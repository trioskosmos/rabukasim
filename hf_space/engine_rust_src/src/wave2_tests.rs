...
fn add_test_member(db: &mut CardDatabase, mut member: MemberCard) {
    ...
    if logic_id < db.members_vec.len() {
        ...
        println!(
            "DEBUG: Member ID {} added to members_vec at index {}.",
            id, logic_id
        );
    } else {
        println!(
            "DEBUG: Member ID {} OUT OF RANGE for members_vec (len: {}).",
            id,
            db.members_vec.len()
        );
    }
...
fn test_granted_ability_propagation_score() {
    ...
    let res = state
        ...
        .get(&0u8)
        .expect("Last performance results for P0 missing!");
    println!("DEBUG: test_score: res JSON: {:?}", res);
    let total = res
        .get("total_score")
        .expect("total_score missing from res object")
        .as_u64()
