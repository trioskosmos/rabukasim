...
## Matrix-First Gate
Before writing or modifying any QA test, you must consult `qa_test_matrix.md` first.

Required workflow:
1. Find the target QID in `qa_test_matrix.md`.
2. Confirm the row is still uncovered (`ℹ️`) before writing a **new** test.
3. If the row is already `✅`, do **not** write another coverage test for that QID unless the task is explicitly weak-test remediation or root-cause bug investigation for an inadequate existing test.
4. After adding a test, re-run `tools/gen_full_matrix.py` and verify that the specific uncovered row was converted by the new test.

Default behavior when the user says to “continue QA tests”:
- Treat this as **new coverage work**.
- Prioritize only uncovered `ℹ️` rows from the matrix.
- Do not spend time replacing weak existing tests unless the user explicitly asks for remediation or the matrix/coverage audit shows the existing test does not count.
...
7. **Document Findings**: Record engine issues or assumptions discovered during testing
