...
def run_batch_audit():
    ...
    for cid, data in cards.items():
        ...
        for m in meanings:
            ...
            elif tag == "HAND_DELTA" and all(
                x not in p_text for x in ["HAND", "DRAW", "RECOVER_LIVE", "RECOVER_MEMBER"]
            ):
                error = "MISSING DRAW/HAND EFFECT"

            if error:
                mismatches.append({"id": cid, "error": error, "text": text, "implemented": p_text})
