...
class SemanticOracleV2:
    ...
    def __init__(self, cards_path=None):
        ...
        if not cards_path or not os.path.exists(cards_path):
            raise FileNotFoundError("Could not find cards.json")
    ...
    def interpret_card(self, card_id):
        ...
        for trigger, text in abilities:
            ...
            for seg in segments:
                ...
                if expects:
                    seg_expects.append({"text": seg, "deltas": expects})

            if seg_expects:
                card_expectations.append({"trigger": trigger, "sequence": seg_expects})

        return {"id": card_id, "abilities": card_expectations}
    ...
    def interpret_pseudocode(self, card_id, pc):
        ...
        for block in blocks:
                    ...
                    if "DRAW(" in line:
                        ...
                        if v:
                            sequence.append(
                                {
                                    "text": line.strip(),
                                    "deltas": [{"tag": tag_prefix + "HAND_DELTA", "value": int(v.group(1))}],
                                }
                            )
                        ...
                        if v:
                            sequence.append(
                                {
                                    "text": line.strip(),
                                    "deltas": [{"tag": tag_prefix + "HAND_DISCARD", "value": int(v.group(1))}],
                                }
                            )
                        ...
                        if v:
                            sequence.append(
                                {
                                    "text": line.strip(),
                                    "deltas": [{"tag": tag_prefix + "SCORE_DELTA", "value": int(v.group(1))}],
                                }
                            )
                        ...
                        sequence.append(
                            {
                                "text": line.strip(),
                                "deltas": [{"tag": "HAND_DELTA", "value": 1}, {"tag": "DISCARD_DELTA", "value": -1}],
                            }
                        )
                        ...
                        if v:
                            sequence.append(
                                {"text": line.strip(), "deltas": [{"tag": "HEART_DELTA", "value": int(v.group(1))}]}
                            )
                    elif "ADD_BLADES(" in line:
                        v = re.search(r"ADD_BLADES\((\d+)\)", line)
                        if v:
                            sequence.append(
                                {"text": line.strip(), "deltas": [{"tag": "BLADE_DELTA", "value": int(v.group(1))}]}
                            )
                    elif "ENERGY_CHARGE(" in line or "ACTIVATE_ENERGY(" in line:
                        v = re.search(r"(?:ENERGY_CHARGE|ACTIVATE_ENERGY)\((\d+)\)", line)
                        if v:
                            sequence.append(
                                {"text": line.strip(), "deltas": [{"tag": "ENERGY_DELTA", "value": int(v.group(1))}]}
                            )
                    elif "PAY_ENERGY(" in line:
                        v = re.search(r"PAY_ENERGY\((\d+)\)", line)
                        if v:
                            sequence.append(
                                {"text": line.strip(), "deltas": [{"tag": "ENERGY_COST", "value": int(v.group(1))}]}
                            )
                    elif "MOVE_TO_DISCARD" in line:
                        sequence.append(
                            {
                                "text": line.strip(),
                                "deltas": [{"tag": "STAGE_DELTA", "value": -1}, {"tag": "DISCARD_DELTA", "value": 1}],
                            }
                        )
            ...
            if sequence:
                card_expectations.append({"trigger": trigger, "sequence": sequence})
