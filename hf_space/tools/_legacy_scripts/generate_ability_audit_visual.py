...
def decompile(bytecode):
    ...
    lines = []
    if costs:
        lines.append(f"<span class='logic-cost'>COST:</span> {'; '.join(costs)}")
    if conditions:
        lines.append(f"<span class='logic-cond'>COND:</span> {', '.join(conditions)}")
    if effects:
        lines.append(f"<span class='logic-eff'>EFF:</span> {'; '.join(effects)}")
...
def generate_report():
    ...
    for c in compiled_cards:
        ...
        if no in card_map:
            ...
            ai_index[no] = {
                "id": c.get("card_id"),
                "abilities": [
                    {"raw_text": ab.get("raw_text", ""), "logic": decompile(ab.get("bytecode", []))}
    ...
    final_html = html_template.format(
        gen_time=datetime.now().strftime("%Y-%m-%d %H:%M:%S"), total_count=len(card_map), rows="".join(rows)
    )
    ...
    print("Success! Report saved to reports/ability_audit_visual.html")
    print("Success! AI Index saved to reports/ability_index.json")
