...
def run_repro():
    print("=" * 60)
    print("REPRODUCTION: PL!-sd1-001-SD (高坂 穂乃果)")
    print("=" * 60)
    ...
    # --- CUSTOM SETUP START ---
    target_card_no = "PL!-sd1-001-SD"
    card_type = "MEMBER"

    # Setup God Mode
    helper.set_energy(0, 12, tapped=False)

    # 1. Add 2 success lives to satisfy condition
    p0 = gs.get_player(0)
    p0.success_lives = [plinth_cid, plinth_cid]  # Use anything
    # 2. Add a LIVE card to discard to be recovered
    p0.discard = [l for l in l_db.keys()][:3]  # Add 3 live cards
    gs.set_player(0, p0)
