...
def audit():
    ...
    for name, op in metadata["opcodes"].items():
        if name in ["NOP", "RETURN", "JUMP", "JUMP_IF_FALSE"]:
            continue
    ...
    # Categorize tokens
    master_names = (
        set(metadata["opcodes"].keys())
        | set(metadata["conditions"].keys())
        | set(metadata["costs"].keys())
        | set(metadata["triggers"].keys())
    )
    ...
    dsl_params = {
        ...
        "UNTIL_LIVE_END",
        "LIVE_END",
        "DID_ACTIVATE_ENERGY_THIS_TURN",
        "DID_ACTIVATE_MEMBER_THIS_TURN",
        "MOVED_THIS_TURN",
        "REVEALED_THIS",
        "PLAYED",
        "DISCARDED",
    ...
    prefix_pattern = re.compile(
        r"^(?:HAS|COUNT|MATCH|SUM|PER|NOT|ON|DECK|TOTAL|REDUCE|IS|PLAY|DISCARD|REMOVE|RESET|REVEALED|LOOK|OTHER|NEXT|PREVENT|SET|UNIQUE|TYPE|HAND|MOVE|BATON|SCORE|ENERGY|HEART|BLADE|ALL|ANY|BASE|OPPONENT|TRIGGER|NO|CHARGED|FIXED)_"
    )
    ...
    for token in sorted_tokens:
        ...
        if token in master_names:
            ...
            or token.startswith("HEART_")
            or token.startswith("NAME_")
            or token.startswith("TARGET_")
        ):
            status = "🎨 PARAM"
        elif (
            "_OR_" in token
