...
class LintWarning:
    def __init__(
        self, card_id: str, card_no: str, card_name: str, ability_idx: int, instr_idx: int, severity: str, message: str
    ):
    ...
    def __str__(self):
        return (
            f"[{self.severity}] Card {self.card_no} (ID={self.card_id}) "
                ...
                if filter_info["unit_enable"] and filter_info["unit_id"] > MAX_UNIT_ID:
                    warnings.append(
                        LintWarning(
                            ...
                            i,
                            "WARN",
                            f"LOOK_AND_CHOOSE unit ID {filter_info['unit_id']} > {MAX_UNIT_ID}. Attr=0x{a:X}",
                        )
                    )
                if filter_info["group_enable"] and filter_info["group_id"] > MAX_GROUP_ID:
                    warnings.append(
                        LintWarning(
                            ...
                            i,
                            "WARN",
                            f"LOOK_AND_CHOOSE group ID {filter_info['group_id']} > {MAX_GROUP_ID}. Attr=0x{a:X}",
                        )
                    )
                ...
                if s not in {0, 1, 2, 4, 6, 7, 8, 13}:
                    warnings.append(
                        LintWarning(
                            ...
                            i,
                            "WARN",
                            f"MOVE_TO_DISCARD has unexpected slot={s}. Bytecode: [{op},{v},{a},{s}]",
                        )
                    )
            ...
            if op == O_RETURN and i == 0:
                warnings.append(
                    LintWarning(
                        ...
                        i,
                        "INFO",
                        "Ability starts with RETURN — effectively a no-op.",
                    )
                )
            ...
            if op not in OPCODE_NAMES and op < 200:
                warnings.append(
                    LintWarning(
                        ...
                        i,
                        "WARN",
                        f"Unknown opcode {op} (0x{op:02X}). Bytecode: [{op},{v},{a},{s}]",
                    )
                )
