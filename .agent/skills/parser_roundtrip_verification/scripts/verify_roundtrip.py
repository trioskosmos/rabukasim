...
def verify_single_ability(card_no: str, ab_idx: int, ability_data: Dict) -> Tuple[bool, str]:
    ...
    # This is valid for trigger-only abilities, but if raw_text has effects, it's a parser bug
    if len(bytecode) == 4 and bytecode[0] == 1:
        # RETURN only - check if raw_text has actual effects
        has_effects = any(kw in raw_text.upper() for kw in 
                         ["EFFECT:", "BOOST", "DRAW", "HEAL", "DAMAGE", "DESTROY", "RETURN", 
                          "SWAP", "SHUFFLE", "SEARCH", "ENERGY", "SCORE", "COST:"])
        if not has_effects:
            return True, "Trigger-only ability (OK)"
        # Has effects in text but not in bytecode - parser bug
        return False, f"Parser bug: effects in text but bytecode is RETURN only"
