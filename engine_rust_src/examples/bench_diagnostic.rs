...
impl DiagnosticTimer {
    ...
    fn print(&self, db: &CardDatabase) {
        ...
        println!(
            "{:<20} {:>6} {:>10} {:>10} {:>10}",
            "Metric", "N", "Avg(μs)", "Max(μs)", "Total(μs)"
        );
        println!("{}", "-".repeat(65));
    ...
    fn print_slow_states(
        ...
        println!(
            "{:<10} {:<30} {:<30} {:<30}",
            "Time(μs)", "Stage Cards", "Live Cards", "Yell Cards"
        );
        println!("{}", "-".repeat(100));

        // Get indices of slowest calls
        let mut indexed: Vec<(usize, u64)> = cards
            .iter()
            .enumerate()
            .map(|(i, (t, _))| (i, *t))
            .collect();
        ...
        for (idx, time_ns) in indexed.iter().take(10) {
            ...
            let stage_names: Vec<String> = stage
                .iter()
                .map(|&cid| {
                    if cid < 0 {
                        "Empty".to_string()
            ...
            let live_names: Vec<String> = live
                ...
                .map(|&cid| {
                    if cid < 0 {
                        ...
                        db.get_live(cid)
                            .map(|l| l.name.clone())
                            .unwrap_or_else(|| format!("Live{}", cid))
                    }
                })
                .collect();

            let yell_names: Vec<String> = yell
                .iter()
                .map(|&cid| {
                    db.get_member(cid)
                        .map(|m| m.name.clone())
                        .unwrap_or_else(|| format!("Yell{}", cid))
                })
                .collect();
    ...
    loop {
        ...
        match state.phase {
            ...
            Phase::LiveResult => {
                ...
                timer
                    .times
                    .get_mut(&Metric::LiveResultFinalize)
                    .unwrap()
                    .push(elapsed);
            }
            Phase::Active => {
