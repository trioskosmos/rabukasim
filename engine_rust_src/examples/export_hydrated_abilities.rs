...
use engine_rust::core::logic::models::{AbilityFrame, AbilityTraceView};
use serde::Serialize;
...
struct HydratedAbilityEntry {
    ...
    card_name: String,
    ability_index: usize,
    trigger: engine_rust::core::enums::TriggerType,
...
fn main() {
    let (db, input_path) = match load_db() {
        Ok(result) => result,
        Err(error) => {
    ...
    let export = HydratedAbilityExport {
        metadata: ExportMetadata {
            ...
            compiled_cards_source: input_path.display().to_string(),
        },
        summary,
        abilities,
    };

    if let Err(error) = write_export(&output_path, &export) {
        eprintln!("[export_hydrated_abilities] {}", error);
        std::process::exit(1);
    }
