...
fn create_proper_decks(
    ...
    println!("Deck sizes:");
    println!(
        "P0: {} members + {} lives = {} total",
        p0_members.len(),
        p0_lives.len(),
        p0_members.len() + p0_lives.len()
    );
    println!(
        "P1: {} members + {} lives = {} total",
        p1_members.len(),
        p1_lives.len(),
        p1_members.len() + p1_lives.len()
    );
...
fn main() {
    ...
    let db = load_db();
    let (p0_members, p1_members, p0_energy, p1_energy, p0_lives, p1_lives) =
        create_proper_decks(&db);
    ...
    println!(
        "P0 Energy Deck: {} cards",
        state.core.players[0].energy_deck.len()
    );
    println!(
        "P1 Energy Deck: {} cards",
        state.core.players[1].energy_deck.len()
    );
    ...
    let p0_live_in_hand = state.core.players[0]
        .hand
        .iter()
        .filter(|&&cid| cid >= 4000 && cid < 5000)
        .count();
    let p1_live_in_hand = state.core.players[0]
        .hand
        .iter()
        .filter(|&&cid| cid >= 4000 && cid < 5000)
        .count();
    let p0_member_in_hand = state.core.players[0]
        .hand
        .iter()
        .filter(|&&cid| cid >= 1000 && cid < 4000)
        .count();
    let p1_member_in_hand = state.core.players[0]
        .hand
        .iter()
        .filter(|&&cid| cid >= 1000 && cid < 4000)
        .count();
    ...
    println!("\n=== CARD TYPE ANALYSIS ===");
    println!(
        "P0 Hand: {} live cards, {} member cards",
        p0_live_in_hand, p0_member_in_hand
    );
    println!(
        "P1 Hand: {} live cards, {} member cards",
        p1_live_in_hand, p1_member_in_hand
    );
