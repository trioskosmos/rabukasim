...
fn test_q103_catchu_dynamic_condition() {
    ...
    for _ in 0..10 { state.core.players[p1].energy_zone.push(100); }
    ...
    for ability in &abilities {
        engine_rust::core::logic::interpreter::resolve_bytecode(&mut state, &db, std::sync::Arc::new(ability.bytecode.clone()), &mut ctx_q97);
        #[cfg(feature = "gpu")]
    ...
    assert_eq!(state.core.players[p1].live_score_bonus, 1, "Q97: Score bonus is 1 because all energy active, despite missing 2 CatChu members");
    ...
    // First proc (CatChu 1)
    let mut ctx_q103_1 = engine_rust::core::logic::AbilityContext { player_id: p1 as u8, source_card_id: catchu_live_id, ..Default::default() };
    for ability in &abilities {
        engine_rust::core::logic::interpreter::resolve_bytecode(&mut state, &db, std::sync::Arc::new(ability.bytecode.clone()), &mut ctx_q103_1);
        #[cfg(feature = "gpu")]
        harness.assert_bytecode_parity(&db, &state, &ability.bytecode, &ctx_q103_1, "Q103 CatChu Proc 1");
    }

    // The "2 Catchu members" condition passes. ACTIVATE_ENERGY untaps up to 6 energy.
    // 6 energy readied. 1 still tapped.
    assert_eq!(state.core.players[p1].tapped_energy_mask.count_ones(), 1, "First capacity readying (6 untaps)");
    assert_eq!(state.core.players[p1].live_score_bonus, 0, "Not all active yet, so no +1 bonus");

    // Second proc (CatChu 2)
    let mut ctx_q103_2 = engine_rust::core::logic::AbilityContext { player_id: p1 as u8, source_card_id: catchu_live_id, ..Default::default() };
    for ability in &abilities {
        engine_rust::core::logic::interpreter::resolve_bytecode(&mut state, &db, std::sync::Arc::new(ability.bytecode.clone()), &mut ctx_q103_2);
        #[cfg(feature = "gpu")]
        harness.assert_bytecode_parity(&db, &state, &ability.bytecode, &ctx_q103_2, "Q103 CatChu Proc 2");
    }

    // 1 remaining energy readied. 0 tapped.
    // The SCORE_RULE now evaluates to True, so +1 bonus is applied.
    assert_eq!(state.core.players[p1].tapped_energy_mask, 0, "All active now");
    assert_eq!(state.core.players[p1].live_score_bonus, 1, "Score +1 applies NOW");
    ...
    assert_eq!(state.core.players[p1].live_score_bonus, 1, "Score remains despite later taps");
}
