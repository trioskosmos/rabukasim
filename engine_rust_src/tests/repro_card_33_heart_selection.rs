...
fn test_card_33_ayase_eri_color_selection() {
    ...
    println!(
        "Success lives (P0): {}",
        state.core.players[p0].success_lives.len()
    );
    ...
    if let Some(card) = db.members.get(&33) {
        ...
        if let Some(ability) = card.abilities.first() {
            ...
            let color_select_effect = ability
                .effects
                .iter()
                ...
                if let Some(card_33) = member_db.get("33") {
                    if let Some(abilities) = card_33.get("abilities").and_then(|a| a.as_array()) {
                        if let Some(ability_0) = abilities.first() {
                            let choice_count =
                                ability_0.get("choice_count").and_then(|v| v.as_u64());
                            println!("Compiled choice_count for card 33: {:?}", choice_count);
                            assert_eq!(choice_count, Some(3), "Compiled choice_count should be 3");

                            // Verify the choices in effect params
                            if let Some(effects) =
                                ability_0.get("effects").and_then(|e| e.as_array())
                            {
                                if let Some(effect_0) = effects.first() {
                                    if let Some(params) = effect_0.get("params") {
                                        if let Some(choices) =
                                            params.get("choices").and_then(|c| c.as_array())
                                        {
                                            let choice_arr: Vec<i64> =
                                                choices.iter().filter_map(|c| c.as_i64()).collect();
                                            println!("Compiled choices: {:?}", choice_arr);
                                            assert_eq!(
                                                choice_arr,
                                                vec![1, 3, 6],
                                                "Compiled choices should be [1, 3, 6]"
                                            );
                                        }
                                    }
                                }
