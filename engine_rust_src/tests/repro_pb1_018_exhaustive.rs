...
fn test_pb1_018_exhaustive() {
    ...
    assert_eq!(
        state.core.players[p1].stage[0], kota_id,
        "P1 Kota in Slot 0"
    );
    // In our engine, WAIT is represented by being Tapped and having Moved flag.
    assert!(
        state.core.players[p1].is_tapped(0),
        "P1 Kota should be Tapped (WAIT)"
    );

    // Check P2 side
    println!("[TEST] P2 Stage: {:?}", state.core.players[p2].stage);
    assert_eq!(
        state.core.players[p2].stage[2], kanata_id,
        "P2 Kanata in Slot 2"
    );
    assert!(
        state.core.players[p2].is_tapped(2),
        "P2 Kanata should be Tapped (WAIT)"
    );
    ...
    assert!(
        !triggered_kanata,
        "Q188: Kanata should NOT have triggered ON_SELF_TAPPED"
    );
    ...
    assert!(
        res.is_err(),
        "Q169: Baton Pass to locked slot must be BLOCKED"
    );
    assert!(
        res.unwrap_err().contains("restriction"),
    ...
    assert!(
        res.is_ok(),
        "Q181: Play to empty slot should SUCCEED even with lock mask: {:?}",
        res.err()
    );
    assert_eq!(
        state.core.players[p1].stage[0], kota_id,
        "Kota should be played successfully to Slot 0"
    );
    ...
    assert!(
        res.is_ok(),
        "Second Nico play should succeed: {:?}",
        res.err()
    );
    ...
    assert_eq!(
        state.phase,
        Phase::Main,
        "Q168: Phase should return to Main immediately if there are no targets in discard"
    );
