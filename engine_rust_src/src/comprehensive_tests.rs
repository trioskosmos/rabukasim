...
fn test_triggers_group_a_action() {
    ...
    state.step(&db, Action::PlayMember { hand_idx: 0, slot_idx: 0 }.id() as i32).unwrap();
    ...
    state.resolve_bytecode_cref(&db, &vec![O_MOVE_TO_DISCARD, 1, 0, 0, 4, O_RETURN, 0, 0, 0, 0], &AbilityContext { player_id: 0, area_idx: 0, source_card_id: 5911, ..Default::default() });
    ...
    state.core.players[0].live_zone[0] = 15002; state.core.players[0].set_revealed(0, false);
    state.core.players[0].yell_count_reduction = 100; // Prevent Yell consumption for test stability
    state.do_performance_phase(&db);
    assert_eq!(state.core.players[0].hand.len(), 3, "OnReveal should trigger draw");
...
fn test_triggers_group_b_phase() {
    ...
    state.current_player = 0; state.do_draw_phase(&db);
    assert_eq!(state.core.players[0].hand.len(), 1, "TurnStart should trigger draw");
...
fn test_triggers_exhaustive() {
    ...
    state.core.players[0].stage[0] = 5914; state.core.players[0].deck.extend(vec![5950]); // Updated deck card
    ...
    state.resolve_bytecode_cref(&db, &vec![O_MOVE_MEMBER, 0, 0, 0, 0, O_RETURN, 0, 0, 0, 0], &AbilityContext { player_id: 0, area_idx: 0, target_slot: 1, ..Default::default() });
...
fn test_triggers_group_c_persistent() {
    ...
    state.core.players[0].stage[0] = 5912; state.core.players[0].deck = vec![5900].into(); // Updated deck card
...
fn test_conditions_group_ab_state() {
    ...
    assert!(state.check_condition(&db, 0, &Condition { condition_type: ConditionType::CountHand, value: 3, ..Default::default() }, &ctx, 0));

    // [ConditionType::IsCenter]
    state.core.players[0].stage[1] = 5901; // Updated stage card
    assert!(state.check_condition(&db, 0, &Condition { condition_type: ConditionType::IsCenter, ..Default::default() }, &AbilityContext { area_idx: 1, ..ctx }, 0));
    ...
    assert!(state.check_condition(&db, 0, &Condition { condition_type: ConditionType::HasMember, value: 5901, ..Default::default() }, &ctx, 0));

    // [ConditionType::TypeCheck] (Attr 1 = Live)
    assert!(state.check_condition(&db, 0, &Condition { condition_type: ConditionType::TypeCheck, attr: 1, ..Default::default() }, &AbilityContext { source_card_id: 55001, ..ctx }, 0));
}
...
fn test_conditions_exhaustive() {
    ...
    assert!(state.check_condition(&db, 0, &Condition { condition_type: ConditionType::DeckRefreshed, ..Default::default() }, &ctx, 0));

    // [ConditionType::HandHasNoLive]
    state.core.players[0].hand = vec![5901].into(); // Member with ID 5901
    assert!(state.check_condition(&db, 0, &Condition { condition_type: ConditionType::HandHasNoLive, ..Default::default() }, &ctx, 0));
    ...
    assert!(state.check_condition(&db, 0, &Condition { condition_type: ConditionType::HasMoved, ..Default::default() }, &AbilityContext { area_idx: 0, ..ctx }, 0));

    // [ConditionType::Baton]
    state.core.players[0].baton_touch_count = 1;
    assert!(state.check_condition(&db, 0, &Condition { condition_type: ConditionType::Baton, ..Default::default() }, &ctx, 0));
}
...
fn test_conditions_group_cd_context_input() {
    ...
    state.core.players[0].success_lives = vec![10].into(); state.core.players[1].success_lives = vec![].into();
    assert!(state.check_condition(&db, 0, &Condition { condition_type: ConditionType::LifeLead, ..Default::default() }, &ctx, 0));

    // [ConditionType::ScoreCompare] (Hearts GE value)
    state.core.players[0].score = 5;
    assert!(state.check_condition(&db, 0, &Condition { condition_type: ConditionType::ScoreCompare, value: 5, ..Default::default() }, &ctx, 0));
}
...
fn test_effects_group_ab_stats_zone() {
    ...
    let ctx = AbilityContext { player_id: 0, area_idx: 0, source_card_id: 60001, ability_index: 0, ..Default::default() };
    ...
    let mut m_rec_901 = m_rec.clone(); m_rec_901.card_id = 5901;
    db.members.insert(5901, m_rec_901.clone()); db.members_vec[5901 & LOGIC_ID_MASK as usize] = Some(m_rec_901);
    ...
    state.resolve_bytecode_cref(&db, &vec![O_MOVE_TO_DECK, 1, 2, 0, 0, O_RETURN, 0, 0, 0, 0], &ctx); // From Hand
...
fn test_effects_group_ce_info_modal() {
    ...
    state.resolve_bytecode_cref(&db, &vec![O_LOOK_AND_CHOOSE, 2, 0, 0, 0, O_RETURN, 0, 0, 0, 0], &ctx);
...
fn test_effects_exhaustive() {
    ...
    state.resolve_bytecode_cref(&db, &vec![O_REDUCE_YELL_COUNT, 2, 0, 0, 0, O_RETURN, 0, 0, 0, 0], &ctx);
    ...
    state.resolve_bytecode_cref(&db, &vec![O_SWAP_AREA, 0, 1, 0, 0, O_RETURN, 0, 0, 0, 0], &ctx);
    ...
    state.resolve_bytecode_cref(&db, &vec![O_TRANSFORM_COLOR, 0, 1, 0, 1, O_RETURN, 0, 0, 0, 0], &ctx);
}
...
fn test_effects_group_f_system() {
    ...
    state.resolve_bytecode_cref(&db, &vec![O_ENERGY_CHARGE, 1, 0, 0, 0, O_RETURN, 0, 0, 0, 0], &AbilityContext { player_id: 0, ..Default::default() });
    assert_eq!(state.core.players[0].energy_zone.len(), initial_energy + 1);

    // [EffectType::Immunity]
    state.resolve_bytecode_cref(&db, &vec![O_IMMUNITY, 1, 0, 0, 0, O_RETURN, 0, 0, 0, 0], &AbilityContext { player_id: 0, ..Default::default() });
...
fn test_costs_all_groups() {
    let db = create_test_db();
    let mut state = create_test_state();
    state.core.players[0].stage[0] = 5901; state.core.players[0].hand = vec![5901].into(); // Updated cards

    let ctx = AbilityContext { player_id: 0, area_idx: 0, ..Default::default() };
    // [AbilityCostType::TapSelf]
    let cost_tap = Cost { cost_type: AbilityCostType::TapSelf, ..Default::default() };
    assert!(state.check_cost(&db, 0, &cost_tap, &ctx));

    // [AbilityCostType::DiscardHand]
    let cost_discard = Cost { cost_type: AbilityCostType::DiscardHand, value: 1, ..Default::default() };
    assert!(state.check_cost(&db, 0, &cost_discard, &ctx));

    // [AbilityCostType::ReturnHand] (Discontinued or mapped to ReturnMemberToHand)
    let cost_return = Cost { cost_type: AbilityCostType::ReturnMemberToHand, ..Default::default() };
...
fn test_targets_all_groups() {
    let db = create_test_db();
    let mut state = create_test_state();
    state.core.players[0].deck = vec![5901, 5902, 5903, 5904, 5905, 5906, 5907, 5908, 5909, 5910].into(); // Updated deck cards
    ...
    state.resolve_bytecode_cref(&db, &vec![O_DRAW, 1, 0, 0, 1, O_RETURN, 0, 0, 0, 0], &AbilityContext { player_id: 0, ..Default::default() });
    assert_eq!(state.core.players[0].hand.len(), 1);

    // [TargetType::Opponent] (Target 2)
    state.resolve_bytecode_cref(&db, &vec![O_DRAW, 1, 0, 0, 2, O_RETURN, 0, 0, 0, 0], &AbilityContext { player_id: 0, ..Default::default() });
    assert_eq!(state.core.players[1].hand.len(), 1);

    // [TargetType::AllPlayers] (Target 3)
    // Draw 1 each
    state.resolve_bytecode_cref(&db, &vec![O_DRAW, 1, 0, 0, 3, O_RETURN, 0, 0, 0, 0], &AbilityContext { player_id: 0, ..Default::default() });
...
fn test_mechanics_optional_once_per_turn() {
    ...
    m_once.card_id = 5999; m_once.abilities = vec![ab_once]; // Changed to 5999
    db.members.insert(5999, m_once.clone()); db.members_vec[5999 & LOGIC_ID_MASK as usize] = Some(m_once);
    ...
    // First play
    state.step(&db, Action::PlayMember { hand_idx: 0, slot_idx: 0 }.id() as i32).unwrap();
    assert_eq!(state.core.players[0].hand.len(), 2, "First play should trigger draw (Hand: [5999] -> [5999, 5901])"); // Updated expected card

    // Second play (on same turn)
    state.step(&db, Action::PlayMember { hand_idx: 0, slot_idx: 1 }.id() as i32).unwrap();
    ...
    m_opt.card_id = 5900; m_opt.abilities = vec![ab_opt]; // Changed to 5900
    db.members.insert(5900, m_opt.clone()); db.members_vec[5900 & LOGIC_ID_MASK as usize] = Some(m_opt);
    ...
    assert_eq!(state.core.players[0].hand.len(), 1, "Optional draw should have resolved");
}
...
fn test_pattern_on_play() {
    ...
    m1.card_id = 5916; m1.abilities = vec![ab1];
    db.members.insert(5916, m1.clone()); db.members_vec[5916 & LOGIC_ID_MASK as usize] = Some(m1);
    state.core.players[0].hand = vec![5916].into();
    state.ui.silent = false; // Enable logging
    state.step(&db, Action::PlayMember { hand_idx: 0, slot_idx: 0 }.id() as i32).unwrap();
    assert_eq!(state.core.players[0].hand.len(), 1, "Mandatory Draw pattern failed (Hand: {:?})", state.core.players[0].hand);
    ...
    let cond2 = Condition { condition_type: ConditionType::Turn1, ..Default::default() };
    let ab2 = Ability {
        trigger: TriggerType::OnPlay,
    ...
    m2.card_id = 5917; m2.abilities = vec![ab2];
    db.members.insert(5917, m2.clone()); db.members_vec[5917 & LOGIC_ID_MASK as usize] = Some(m2);
    ...
    m_recov.card_id = 5990; m_recov.abilities = vec![];
    db.members.insert(5990, m_recov.clone()); db.members_vec[5990 & LOGIC_ID_MASK as usize] = Some(m_recov);
    ...
    if state.phase == Phase::Response { state.step(&db, Action::SelectChoice { choice_idx: 0 }.id() as i32).unwrap(); }
    assert_eq!(state.core.players[0].hand.len(), 1, "Conditional Recover pattern failed (should have recovered ID 5990)");
    ...
    let cost3 = Cost { cost_type: AbilityCostType::DiscardHand, value: 1, ..Default::default() };
    let ab3 = Ability {
        trigger: TriggerType::OnPlay,
        costs: vec![cost3],
...
fn test_pattern_performance() {
    ...
    */
    ...
    l2.card_id = 15003; l2.abilities = vec![ab2];
    ...
    state.core.players[0].live_zone[0] = 15003; state.core.players[0].set_revealed(0, false);

    // Simulate Reveal
    state.phase = Phase::PerformanceP1; // Must be in Performance phase to reveal
    state.resolve_bytecode_cref(&db, &vec![O_REVEAL_CARDS, 1, 0, 0, 0, O_RETURN, 0, 0, 0, 0], &AbilityContext { player_id: 0, ..Default::default() });
    ...
    let cond3 = Condition { condition_type: ConditionType::LifeLead, ..Default::default() };
    let ab3 = Ability {
    ...
    let mut m3 = db.members.get(&3000).unwrap().clone();
    m3.card_id = 5963; m3.abilities = vec![ab3]; // Changed to 5963
    db.members.insert(5963, m3.clone()); db.members_vec[5963 & LOGIC_ID_MASK as usize] = Some(m3);
    state.core.players[0].stage[0] = 5963; state.core.players[0].deck = vec![5902].into(); // Updated deck card
    state.core.players[0].success_lives = vec![15001].into(); state.core.players[1].success_lives = vec![].into();
    ...
    assert_eq!(state.core.players[0].hand.len(), 2, "OnLiveSuccess Reward pattern failed");
}
...
fn test_opcode_reveal_until_cost_ge() {
    ...
    if db.members_vec.len() <= (60015 & LOGIC_ID_MASK as usize) { db.members_vec.resize(60020, None); }
    ...
    let bc = vec![O_REVEAL_UNTIL, C_COST_CHECK, 10, 0, 54, O_RETURN, 0, 0, 0, 0];
    state.resolve_bytecode_cref(&db, &bc, &ctx);
...
fn test_pattern_exhaustive_sampling() {
    ...
    let cost1 = Cost { cost_type: AbilityCostType::DiscardLive, value: 1, ..Default::default() };
    let ab1 = Ability {
        trigger: TriggerType::OnPlay,
        costs: vec![cost1],
    ...
    let mut m1 = db.members.get(&3000).unwrap().clone();
    m1.card_id = 5801; m1.abilities = vec![ab1];
    db.members.insert(5801, m1.clone()); db.members_vec[5801 & LOGIC_ID_MASK as usize] = Some(m1);
    ...
    state.resolve_bytecode_cref(&db, &bc2, &AbilityContext { player_id: 0, ..Default::default() });
...
fn test_reproduce_example_draw_discard() {
    ...
    state.step(&db, Action::PlayMember { hand_idx: 0, slot_idx: 0 }.id() as i32).unwrap();
    println!("DEBUG TEST: Phase after PlayMember: {:?}. Hand: {:?}", state.phase, state.core.players[0].hand);
    ...
    assert_eq!(state.phase, Phase::Main, "Should have returned to Main phase after discard choice. Current phase: {:?}", state.phase);
    assert_eq!(state.core.players[0].hand.len(), 1, "Should have 1 card left (10 drawn, 1 discarded). Hand: {:?}", state.core.players[0].hand);
    assert_eq!(state.core.players[0].discard.len(), 1, "Should have 1 card in discard pile. Discard: {:?}", state.core.players[0].discard);
}
