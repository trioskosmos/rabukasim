...
fn test_opcode_select_member() {
    ...
    assert_eq!(state.phase, Phase::Response, "O_SELECT_MEMBER should enter Phase::Response");
    assert_eq!(state.interaction_stack.last().map(|i| i.choice_type.as_str()).unwrap_or(""), "SELECT_MEMBER", "Pending choice type mismatch");
}
...
fn test_opcode_select_live() {
    ...
    assert_eq!(state.interaction_stack.last().map(|i| i.choice_type.as_str()).unwrap_or(""), "SELECT_LIVE", "Pending choice type mismatch");
}
...
fn test_opcode_opponent_choose() {
    ...
    // Updated behavior
    assert_eq!(state.phase, Phase::Response, "O_OPPONENT_CHOOSE should enter Phase::Response");
    assert_eq!(state.interaction_stack.last().map(|i| i.choice_type.as_str()).unwrap_or(""), "OPPONENT_CHOOSE", "Pending choice type mismatch");
    ...
    if let Some(interaction) = state.interaction_stack.last() {
        assert_eq!(interaction.ctx.player_id, 1, "Pending context should be for opponent (player 1)");
    }
}
...
fn test_opcode_prevent_activate() {
    ...
    m.abilities.push(Ability {
        ...
        costs: vec![Cost { cost_type: AbilityCostType::None, value: 0, ..Default::default() }],
        ..Default::default()
    });
    ...
    if let Err(e) = res {
        assert!(e.contains("restriction"), "Error should mention restriction: {}", e);
    }
}
...
fn test_opcode_prevent_baton_touch() {
    ...
    if let Err(e) = res {
        assert!(e.contains("restricted"), "Error should mention restricted: {}", e);
    }
}
...
fn test_opcode_heart_modifiers() {
    ...
    assert!(state.check_live_success(&db, 0, &l, &pass_hearts), "Should pass with modified requirements");
    assert!(!state.check_live_success(&db, 0, &l, &fail_hearts), "Should fail with original requirements");
}
