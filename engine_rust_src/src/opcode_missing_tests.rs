...
fn test_opcode_heart_modifiers() {
    ...
    assert!(state.check_live_success(0, &l, &pass_hearts), "Should pass with modified requirements");
    assert!(!state.check_live_success(0, &l, &fail_hearts), "Should fail with original requirements");
}
