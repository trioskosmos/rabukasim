...
mod tests {
    ...
    // =========================================================================
    // QA: Q139 | Q: メンバーの下にあるエネルギーがある状態でエリアを移動する場合、どうなりますか？
    // A: 他のエリアに移動する場合、メンバーの下にあるエネルギーカードは移動するメンバーと同時にエリアを移動します。
    ...
    fn test_q141_under_member_energy_to_deck_on_baton() {
        // QA: Q141 | Q: メンバーの下にあるエネルギーがあるメンバーとバトンタッチしてメンバーを登場させた場合、どうなりますか？
        // A: メンバーの下にあったエネルギーはエネルギーデッキに移動します。 バトンタッチしたメンバーにはメンバー下にあるエネルギーカードがない状態で登場します。
    ...
    // =========================================================================
    // QA: Q72 | Q: 自分のステージにメンバーカードがない状況です。ライブカードセットフェイズに手札のカードをライブカード置き場に置くことはできますか？
    // A: はい、できます。
    ...
    fn test_q92_energy_cost_must_be_fully_paid() {
        // QA: Q92 | Q: 『 {{live_start.png|ライブ開始時}} {{icon_energy.png|E}} {{icon_energy.png|E}} 支払わないかぎり、自分の手札を2枚控え室に置く。』について。 アクティブ状態のエネルギーが1枚以下の場合、 {{icon_energy.png|E}} {{icon_energy.png|E}} を支払うことはできますか？また、アクティブ状態のエネルギーが2枚以上の場合、 {{icon_energy.png|E}} {{icon_energy.png|E}} を支払わないことはできますか？
        // A: コストはすべて支払う必要があります。アクティブ状態のエネルギーが1枚以下の場合、 {{icon_energy.png|E}} {{icon_energy.png|E}} を支払うことはできません。1枚だけ支払うということもできません。 コストを支払うかどうかは選択できます。 {{icon_energy.png|E}} {{icon_energy.png|E}} を支払える状況であったとしても、支払わないことを選択できます。 コストを支払わなかった場合、「自分の手札を2枚控え室に置く。」の効果を解決します。
    ...
    fn test_q93_partial_effect_resolution() {
        // QA: Q93 | Q: 『 {{live_start.png|ライブ開始時}} {{icon_energy.png|E}} {{icon_energy.png|E}} 支払わないかぎり、自分の手札を2枚控え室に置く。』について。 {{icon_energy.png|E}} {{icon_energy.png|E}} を支払わず、自分の手札が1枚以下の場合、どうなりますか？
        // A: 効果や処理は実行可能な限り解決し、一部でも実行可能な場合はその一部を解決します。まったく解決できない場合は何も行いません。 手札が1枚の場合、その1枚を控え室に置きます。手札が0枚の場合、特に何も行いません。
    ...
    fn test_q94_ability_activation_on_appearance_and_move() {
        // QA: Q94 | Q: 『 {{jidou.png|自動}} このメンバーが登場か、エリアを移動するたび、ライブ終了時まで、ブレードブレードを得る。』について。 例えば、このメンバーカードが登場して、その後、このメンバーカードが別のエリアに移動した場合、この自動能力は合わせて2回発動しますか？
    ...
    fn test_q99_member_name_count_one_not_two() {
        // QA: Q99 | Q: 『 {{live_start.png|ライブ開始時}} 自分のステージにいる、このターン中に登場、またはエリアを移動した『5yncri5e!』のメンバー1人につき、このカードを成功させるための必要ハートを {{heart_00.png|heart0}} 減らす。』について。 この自動能力の効果を解決する時点で、ステージにいる「このターンに登場、かつエリアを移動した『5yncri5e!』のメンバー」は2人分として数えますか？
    ...
    fn test_q106_void_ability_cannot_void_again() {
        // QA: Q106 | Q: 『 {{toujyou.png|登場}} 自分のステージにいる『Liella!』のメンバー1人のすべての {{live_start.png|ライブ開始時}} 能力を、ライブ終了時まで、無効にしてもよい。これにより無効にした場合、自分の控え室から『Liella!』のカードを1枚手札に加える。』について。 すべての {{live_start.png|ライブ開始時}} 能力が無効になっているメンバーを選んで、もう一度無効にすることで、自分の控え室から『Liella!』のカードを1枚手札に加えることはできますか？
        // A: いいえ、できません。 無効である能力がさらに無効にはならないため、「無効にした場合」の条件を満たしていません。
    ...
    fn test_q108_activated_ability_source_card() {
        // QA: Q108 | Q: 『 {{kidou.png|起動}} {{turn1.png|ターン1回}} 手札のコスト4以下の『Liella!』のメンバーカードを1枚控え室に置く：これにより控え室に置いたメンバーカードの {{toujyou.png|登場}} 能力1つを発動させる。( {{toujyou.png|登場}} 能力がコストを持つ場合、支払って発動させる。)』について。 この {{kidou.png|起動}} 能力の効果で発動する {{toujyou.png|登場}} 能力は、この {{kidou.png|起動}} 能力を使ったカードが持っている能力として扱いますか？
        // A: いいえ、控え室に置いたメンバーカードが持つ {{toujyou.png|登場}} 能力として扱います。 （例）「[PL!SP-pb1-009]鬼塚夏美」の『 {{toujyou.png|登場}} 自分のステージにほかの『5yncri5e!』のメンバーがいる場合、カードを1枚引く。』を発動した場合、この能力を持つ「鬼塚夏美」のほかに自分のステージに『5yncri5e!』のメンバーがいる場合、カードを引きます。 （例）「[PL!SP-bp1-002]唐 可可」の『 {{toujyou.png|登場}} {{icon_energy.png|E}} {{icon_energy.png|E}} 支払ってもよい：ステージの左サイドエリアに登場しているなら、カードを2枚引く。』を発動した場合、この能力を持つ「唐 可可」が左サイドエリアに登場していないため、カードは引きません。
    ...
    fn test_q113_aura_requires_actual_yell() {
        // QA: Q113 | Q: 『 {{jidou.png|自動}} {{turn1.png|ターン1回}} エールにより公開された自分のカードの中にブレードハートを持つカードがないとき、ライブ終了時まで、 {{heart_06.png|heart06}} を得る。』などについて。 ブレードがないなど何らかの理由でエールを行わなかった場合、この能力は発動しますか？
        // A: いいえ、発動しません。
    ...
    fn test_q114_member_location_not_turn_of_appearance() {
        // QA: Q114 | Q: 『 {{live_start.png|ライブ開始時}} 自分のステージに「徒町小鈴」が登場しており、かつ「徒町小鈴」よりコストの大きい「村野さやか」が登場している場合、このカードを成功させるための必要ハートを {{heart_00.png|heart0}} {{heart_00.png|heart0}} {{heart_00.png|heart0}} 減らす。』について。 「徒町小鈴」と「村野さやか」はこの能力を使うターンに登場して、自分のステージにいる必要がありますか？
    ...
    fn test_q116_heart_reduction_independent_of_yell() {
        // QA: Q116 | Q: 『 {{live_start.png|ライブ開始時}} 自分のステージにいるメンバーが持つブレードの合計が10以上の場合、このカードのスコアを＋１する。』について。 ブレードの合計が10以上で、エールによって公開される自分のカードの枚数が減る効果が有効なため、公開される枚数が9枚以下になる場合であっても、このカードのスコアを＋１することはできますか？
    ...
    fn test_q119_ability_effect_time_snapshot() {
        // QA: Q119 | Q: 『 {{live_success.png|ライブ成功時}} 自分の手札の枚数が相手より多い場合、このカードのスコアを＋１する。』について。 この能力を使用して効果を解決したあと、手札の枚数が増減しました。この能力を持つカードのスコアも増減しますか？
        // A: いいえ、増減しません。 この能力を使用して効果を解決する時点の手札の枚数を参照して、「このカードのスコアを＋１する」の効果が有効になるかどうかが決まります。この能力の効果を解決したあとに手札の枚数が増減したとしても、「このカードのスコアを＋１する」の効果が、有効から無効、または、無効から有効にはなりません。
    ...
    fn test_q45_single_ability_per_trigger_timing() {
        // QA: Q45 | Q: エールのチェックで公開された {{icon_b_all.png|ALLブレード}} は、どのような効果を発揮しますか？
        // A: パフォーマンスフェイズで、必要ハートを満たしているかどうかを確認する時に、 {{icon_b_all.png|ALLブレード}} 1つにつき、任意の色（ {{heart_01.png|heart01}} 、 {{heart_04.png|heart04}} 、 {{heart_05.png|heart05}} 、 {{heart_02.png|heart02}} 、 {{heart_03.png|heart03}} 、 {{heart_06.png|heart06}} ）のハートアイコン1つとして扱います。
    ...
    fn test_q59_turn_once_reset_per_turn() {
        // QA: Q59 | Q: ステージにいるメンバーが {{turn1.png|ターン1回}} である能力を使い、その後、ステージから控え室に置かれました。同じターンに、そのメンバーがステージに置かれました。このメンバーは {{turn1.png|ターン1回}} である能力を使うことができますか？
        // A: はい、使うことができます。領域を移動（ステージ間の移動を除きます）したカードは、新しいカードとして扱います。
    ...
    fn test_q69_group_name_matching() {
        // QA: Q69 | Q: 『 {{live_start.png|ライブ開始時}} 手札の「上原歩夢」と「澁谷かのん」と「日野下花帆」を、好きな組み合わせで合計3枚、控え室に置いてもよい：ライブ終了時まで、「 {{jyouji.png|常時}} ライブの合計スコアを＋３する。」を得る。』について。 『「上原歩夢」を3枚』や『「澁谷かのん」を2枚と「日野下花帆」を1枚』という組み合わせでコストを支払うことはできますか？
    ...
    fn test_q84_simultaneous_ability_priority() {
        // QA: Q84 | Q: アクティブプレイヤーのAさんのカードが持つ自動能力1と自動能力2、非アクティブプレイヤーのBさんのカードが持つ自動能力3と自動能力4が同時に発動しました。これらの自動能力はどのような順番で使いますか？
        // A: 以下の手順で処理をします。〈【1】Aさんは自動能力1か自動能力2のいずれかを使います。【2】Aさんは残りの自動能力を使います。【3】Bさんは自動能力3か自動能力4のいずれかを使います。【4】Bさんは残りの自動能力を使います。〉 自動能力が2つ以上同時に発動した場合、そのプレイヤーの好きな順番で使うことができます。また、両方のプレイヤーの自動能力が同時に発動した場合、アクティブプレイヤーが自動能力をすべて使ったあと、非アクティブプレイヤーが自動能力使います。 手番があるフェイズ中（通常フェイズ、パフォーマンスフェイズ）は、手番プレイヤーがアクティブプレイヤーになり、手番がないフェイズ中（ライブカードセットフェイズ、ライブ勝敗判定フェイズ）は、先攻プレイヤーがアクティブプレイヤーになります。
    ...
    fn test_q87_partial_effect_resolution() {
        // QA: Q87 | Q: 同じターンに「バトンタッチ」を複数回行うことはできますか？
        // A: はい、条件を満たしていれば、同じターンに「バトンタッチ」は複数回行うことができます。例えば、左サイドエリアとセンターエリアに、前のターンからメンバーがいる場合、これらの2つのエリアのメンバーとそれぞれバトンタッチすることができます。 ただし、同じターンに登場したメンバーがいるエリアに新しくメンバーを登場させたりバトンタッチすることはできませんので、同じターンにバトンタッチで登場したメンバーとさらにバトンタッチすることはできません。
