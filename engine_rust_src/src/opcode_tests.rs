...
fn test_opcode_look_and_choose_filter_cost_ge() {
    ...
    let bc = vec![O_LOOK_AND_CHOOSE, 2, 0x95000000u32 as i32, 0, 0, O_RETURN, 0, 0, 0, 0];
    state.resolve_bytecode_cref(&db, &bc, &ctx);
    ...
    assert!(actions.contains(&(ACTION_BASE_CHOICE + 1)), "Card 15 (cost 15) should be legal for COST_GE=10");
    assert!(!actions.contains(&(ACTION_BASE_CHOICE + 0)), "Card 10 (cost 5) should NOT be legal for COST_GE=10");
}
...
fn test_look_and_choose_source_zone_fix() {
    ...
    let bc = vec![O_LOOK_AND_CHOOSE, 2, 0, (1u64 << 31) as i32, 6, O_RETURN, 0, 0, 0, 0];
    ...
    assert!(actions.contains(&(ACTION_BASE_CHOICE + 0)), "Should allow choosing first looked card");
    assert!(actions.contains(&(ACTION_BASE_CHOICE + 1)), "Should allow choosing second looked card");
