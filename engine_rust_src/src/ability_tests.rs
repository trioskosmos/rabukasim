...
fn test_opcode_draw_discard() {
    ...
    let bc = vec![O_MOVE_TO_DISCARD, 1, 2, 6, O_RETURN, 0, 0, 0];
    state.resolve_bytecode(&db, &bc, &ctx);
...
fn test_look_and_choose_remainder() {
    ...
    let bc = vec![O_LOOK_DECK, 4, 0, 0, O_LOOK_AND_CHOOSE, 1, 0, 6, O_RETURN, 0, 0, 0];
    ...
    ctx2.choice_index = 1;
    state2.resolve_bytecode(&db, &bc, &ctx2);
    ...
    state3.resolve_bytecode(&db, &bc, &ctx3);
