...
mod tests {
    ...
    fn debug_q203_trace() {
        ...
        println!("get_member({}) = {:?}", niji_member_id, member.map(|m| (&m.name, &m.groups, m.card_id)));
        
        let live = db.get_live(live_id);
        println!("=== LIVE LOOKUP ===");
        println!("get_live({}) = {:?}", live_id, live.map(|l| (&l.name, l.card_id)));
        if let Some(l) = live {
            ...
            if !l.abilities.is_empty() {
                ...
                println!("Ability 0 conditions count: {}", l.abilities[0].conditions.len());
            }
        }
        ...
        println!("activated_energy_group_mask = {} (binary: {:b})", 
            state.core.players[0].activated_energy_group_mask,
            state.core.players[0].activated_energy_group_mask);
        println!("Expected: bit 2 set = 4 (binary: 100)");
        assert!((state.core.players[0].activated_energy_group_mask & (1 << 2)) != 0, 
            "FAIL: Energy mask does NOT have bit 2 set! Actual: {}", state.core.players[0].activated_energy_group_mask);
        ...
        println!("\n=== RESULT ===");
        println!("live_score_bonus = {} (expected: 1)", state.core.players[0].live_score_bonus);
        assert_eq!(state.core.players[0].live_score_bonus, 1, "live_score_bonus should be 1");
    }
}
