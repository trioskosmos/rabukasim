...
mod vanilla_encoding_tests {
    use crate::core::alphazero_encoding_vanilla::{
        AlphaZeroVanillaEncoding, AZ_VANILLA_TOTAL_INPUT,
    };
    ...
    fn test_vanilla_encoding_size() {
        ...
        assert_eq!(
            tensor.len(),
            AZ_VANILLA_TOTAL_INPUT,
            "Vanilla tensor size mismatch"
        );
    ...
    fn test_portfolio_oracle_logic() {
        ...
        let high_hearts =
            crate::core::hearts::HeartBoard::from_array(&[10, 10, 10, 10, 10, 10, 10]);
        for i in 0..3 {
        ...
        assert!(
            tensor[10] > 0.0,
            "With real cards and 30 hearts, Raw EV must be positive!"
        );
    }
}
