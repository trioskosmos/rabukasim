...
fn test_phase_auto_advance_from_mulligan() {
    ...
    state
        ...
        .unwrap(); // P1 choice: Scissors
    assert_eq!(state.phase, Phase::TurnChoice);

    // 2. TurnChoice
    state
        .step(&db, Action::ChooseTurnOrder { first: true }.id() as i32)
        .unwrap(); // P0 chooses first
    ...
    assert_eq!(
        state.phase,
        Phase::Main,
        "Should have auto-advanced to Main phase"
    );
