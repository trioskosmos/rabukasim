...
fn test_yell_persistence_and_selection() {
    ...
    let discard_idx = state.core.players[0]
        .hand
        .iter()
        .position(|&c| c == 500)
        .unwrap();
    ...
    if state.phase == Phase::Response && !state.core.players[0].looked_cards.is_empty() {
        ...
        assert!(
            state.core.players[0].looked_cards.contains(&101),
            "looked_cards should contain 101"
        );
        assert!(
            state.core.players[0].looked_cards.contains(&102),
    ...
    assert!(
        state.core.players[0].discard.contains(&111),
