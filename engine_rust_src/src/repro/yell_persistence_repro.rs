...
fn test_yell_persistence_and_selection() {
    ...
    state.players[0].yell_cards = smallvec![101, 102];

    // Also ensure these cards are in the energy zone (since picking from YELL removes from energy)
    state.players[0].stage_energy[0].push(101);
    state.players[0].stage_energy[1].push(102);
    state.players[0].sync_stage_energy_count(0);
    state.players[0].sync_stage_energy_count(1);
    ...
    state
        .step(&db, ACTION_BASE_CHOICE + discard_idx as i32)
        .unwrap();
    ...
    if state.phase == Phase::Response && !state.players[0].looked_cards.is_empty() {
        // Check that looked_cards contains our yelled cards
        assert!(
            state.players[0].looked_cards.contains(&101),
            "looked_cards should contain 101"
        );
        assert!(
            state.players[0].looked_cards.contains(&102),
            "looked_cards should contain 102"
        );
    } else {
    ...
    // Verification:
    println!("DEBUG: Final hand: {:?}", state.players[0].hand);
    println!("DEBUG: Final discard: {:?}", state.players[0].discard);
    ...
    assert!(
        state.players[0].discard.contains(&111),
        "Card 111 should be in discard after performance"
    );
