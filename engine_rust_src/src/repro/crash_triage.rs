...
impl StateDelta {
    ...
    fn to_string(&self) -> String {
        ...
        if self.hand != 0 {
            parts.push(format!("Hand{:+}", self.hand));
        }
        if self.deck != 0 {
            parts.push(format!("Deck{:+}", self.deck));
        }
        if self.discard != 0 {
            parts.push(format!("Discard{:+}", self.discard));
        }
        if self.energy != 0 {
            parts.push(format!("Energy{:+}", self.energy));
        }
        if self.tapped != 0 {
            parts.push(format!("Tapped{:+}", self.tapped));
        }
        if self.looked != 0 {
            parts.push(format!("Looked{:+}", self.looked));
        }
        if self.stage_changed {
            parts.push("StageChanged".to_string());
        }
        if self.live_changed {
            parts.push("LiveChanged".to_string());
        }
        if self.suspended {
...
fn test_state_delta_verification() {
        ...
        if let Some(card) = db.members.get(card_id) {
            for ability in &card.abilities {
                ...
                seen_signatures.insert(
                    sig.clone(),
                    (card.card_no.clone(), jp_text.clone(), pseudo.clone()),
                );
                all_entries.push((
                    card.card_no.clone(),
                    jp_text,
    ...
    report_lines.push(format!(
        "Total unique ability signatures: {}\n",
        all_entries.len()
    ));
    report_lines
        .push("| # | Card | Trigger | JP Text | Pseudocode | State Delta | Flags |".to_string());
    report_lines
        .push("|---|------|---------|---------|------------|-------------|-------|".to_string());
        ...
        all_entries.iter().enumerate()
    {
        ...
        let jp_short = if jp_text.len() > 60 {
            format!(
                "{}...",
                &jp_text[..jp_text.char_indices().nth(30).map(|(i, _)| i).unwrap_or(60)]
            )
        } else {
            jp_text.clone()
        };
        let pseudo_short = if pseudo.len() > 40 {
            format!(
                "{}...",
                &pseudo[..pseudo.char_indices().nth(20).map(|(i, _)| i).unwrap_or(40)]
            )
        } else {
            pseudo.clone()
        };
        ...
        match result {
            ...
            Err(panic_info) => {
                ...
                let panic_short = if panic_msg.len() > 80 {
                    &panic_msg[..80]
                } else {
                    panic_msg
                };
    ...
    let mut file = std::fs::File::create("../reports/state_delta_verification.md")
    ...
    if !crashed_cards.is_empty() {
        ...
        let mut f = std::fs::File::create("../reports/crashed_abilities.txt")
            .expect("Failed to create crash report");
    ...
    assert!(
        crash_rate < 0.10,
        "Crash rate {:.1}% exceeds 10% threshold ({} of {} crashed)",
        crash_rate * 100.0,
        crash_count,
        all_entries.len()
    );
