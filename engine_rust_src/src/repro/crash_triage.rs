...
fn test_state_delta_verification() {
    ...
    let mut all_entries: Vec<(String, String, String, Vec<i32>, TriggerType, u16)> = Vec::new(); // (card_no, jp_text, pseudo, bytecode, trigger, card_id)

    let mut card_ids: Vec<u16> = db.members.keys().cloned().collect();
    card_ids.sort();
    ...
    for (idx, (card_no, jp_text, pseudo, bytecode, trigger, card_id)) in all_entries.iter().enumerate() {
        ...
        state.core.players[0].stage[0] = *card_id as i16;
        ...
        state.core.players[0].energy_zone = vec![40000u16; 8].into(); // 8 energy cards
        ...
        // Execute
        let bytecode_slice: &[i32] = bytecode;
    ...
    // Summary
    report_lines.insert(2, format!("- **Flagged**: {} | **No-ops**: {} | **Suspended**: {}\n",
        flagged_count, noop_count, suspend_count));
