...
fn benchmark_random_moves() {
    ...
    println!(
        "Starting Random Moves Profiler (Target: {} steps)...",
        target_steps
    );
    ...
    println!("--------------------------");
    println!(
        "Action Generation: {:.3}s ({:>5.1}%)",
        total_action_gen_time.as_secs_f64(),
        (total_action_gen_time.as_secs_f64() / overall_secs) * 100.0
    );
    println!(
        "Step Execution:   {:.3}s ({:>5.1}%)",
        total_execution_time.as_secs_f64(),
        (total_execution_time.as_secs_f64() / overall_secs) * 100.0
    );

    let other_time =
        overall_secs - total_action_gen_time.as_secs_f64() - total_execution_time.as_secs_f64();
    println!(
        "Other Overlay:    {:.3}s ({:>5.1}%)",
        other_time,
        (other_time / overall_secs) * 100.0
    );
