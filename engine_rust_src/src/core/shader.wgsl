struct GpuPlayerState {
    heart_buffs: array<u32, 8>,
    heart_req_reductions: array<u32, 2>,
    deck: array<u32, 32>,
    hand: array<u32, 32>,
    discard_pile: array<u32, 32>,
    stage: array<u32, 2>,
    live_zone: array<u32, 2>,
    score: u32,
    flags: u32,
    hand_added_mask: u32,
    hand_len: u32,
    deck_len: u32,
    discard_pile_len: u32,
    current_turn_volume: u32,
    baton_touch_count: u32,
    energy_count: u32,
    pending_draws: u32,
    tapped_energy_count: u32,
    used_abilities_mask: u32,
    board_blades: u32,
    lives_cleared_count: u32,
    avg_hearts: array<u32, 7>,
    mcts_reward: f32,
    baton_touch_limit: u32,
    moved_flags: u32,
    energy_deck_len: u32,
    _pad_inner: u32,
}

struct GpuGameState {
    players: array<GpuPlayerState, 2>,
    current_player: u32,
    phase: i32,
    turn: u32,
    active_count: u32,
    winner: i32,
    prev_card_id: i32,
    forced_action: i32,
    is_debug: u32,
    rng_state_lo: u32,
    rng_state_hi: u32,
    first_player: u32,
}

struct GpuCardStats {
    ability_flags_lo: u32,
    ability_flags_hi: u32,
    cost: u32,
    blades: u32,
    card_type: u32,
    bytecode_start: u32,
    bytecode_len: u32,
    volume_icons: u32,
    extra_val: u32,
    hearts_lo: u32,
    hearts_hi: u32,
    blade_hearts_lo: u32,
    blade_hearts_hi: u32,
    groups: u32,
    units: u32,
    char_id: u32,
    synergy_flags: u32,
    _pad_card: u32,
}

@group(0) @binding(0) var<storage, read_write> states: array<GpuGameState>;
@group(0) @binding(1) var<storage, read> card_stats: array<GpuCardStats>;
@group(0) @binding(2) var<storage, read> bytecode: array<i32>;

var<private> local_state: GpuGameState;
var<private> g_board_hearts: array<array<u32, 8>, 2>;
var<private> g_rng: vec2<u32>;
var<private> g_gid: u32;

// --- HEURISTIC WEIGHTS ---
const W_LIVE_SCORE: f32 = 10000.0;
const W_SUCCESS_BONUS: f32 = 20000.0;
const W_MEMBER_COST: f32 = 10.0;
const W_HEART: f32 = 500.0;
const W_SLOT_BONUS: f32 = 100.0;
const W_SLOT_PENALTY: f32 = 50.0;
const W_BLADE: f32 = 100.0;
const W_STAGE_ABILITY: f32 = 100.0;
const W_UNTAPPED_BONUS: f32 = 1.05;
const W_SYNERGY_CENTER: f32 = 2.0; // Multiplier
const W_SYNERGY_GROUP: f32 = 0.5;

// --- ABILITY FLAGS (lo) ---
const F_DRAW: u32 = 1u;
const F_SEARCH: u32 = 2u;
const F_RECOVER: u32 = 4u;
const F_BUFF: u32 = 8u;
const F_CHARGE: u32 = 16u;
const F_TEMPO: u32 = 32u;
const F_REDUCE: u32 = 64u;
const F_BOOST: u32 = 128u;
const F_TRANSFORM: u32 = 256u;
const F_WIN_COND: u32 = 512u;

// --- SYNERGY FLAGS ---
const SYN_CENTER: u32 = 1u;
const SYN_GROUP: u32 = 2u;

// --- OPCODES ---
const O_RETURN: i32 = 1;
const O_JUMP: i32 = 2;
const O_JUMP_F: i32 = 3;
const O_DRAW: i32 = 10;
const O_HEARTS: i32 = 12;
const O_REDUCE_COST: i32 = 13;
const O_BOOST: i32 = 16;
const O_CHARGE: i32 = 23;
const O_SELECT_MODE: i32 = 30;
const O_LOOK_AND_CHOOSE: i32 = 41;
const O_ACTIVATE_ENERGY: i32 = 81;

const C_TR1: i32 = 200;
const C_HAS_MEMBER: i32 = 201;
const C_CLR: i32 = 202;
const C_STG: i32 = 203;
const C_HND: i32 = 204;
const C_LLD: i32 = 207;
const C_GRP: i32 = 208;
const C_COST_CHK: i32 = 215;

fn check_condition_opcode(p_idx: u32, op: i32, v: i32, a: u32, target_slot: u32) -> bool {
    switch (op) {
        case 200: { // C_TR1
            return local_state.turn == 1u;
        }
        case 201: { // C_HAS_MEMBER
            var count = 0u;
            for (var s = 0u; s < 3u; s = s + 1u) {
                if (get_stage_card(p_idx, s) == u32(a)) { count = count + 1u; }
            }
            return count >= u32(v);
        }
        case 203: { // C_STG
            var count = 0u;
            for (var s = 0u; s < 3u; s = s + 1u) {
                if (get_stage_card(p_idx, s) > 0u) { count = count + 1u; }
            }
            return count >= u32(v);
        }
        case 204: { // C_HND
            return local_state.players[p_idx].hand_len >= u32(v);
        }
        case 202: { // C_CLR
            let color_idx = a;
            if (color_idx > 0u && color_idx < 7u) {
                for (var s = 0u; s < 3u; s = s + 1u) {
                    let s_cid = get_stage_card(p_idx, s);
                    if (s_cid > 0u && s_cid < arrayLength(&card_stats)) {
                        let hearts_lo = card_stats[s_cid].hearts_lo;
                        let hearts_hi = card_stats[s_cid].hearts_hi;
                        var val = 0u;
                        if (color_idx < 5u) { val = (hearts_lo >> ((color_idx - 1u) * 8u)) & 0xFFu; }
                        else { val = (hearts_hi >> ((color_idx - 5u) * 8u)) & 0xFFu; }
                        if (val > 0u) { return true; }
                    }
                }
            }
            return false;
        }
        case 206: { // C_CTR
            return target_slot == 1u;
        }
        case 207: { // C_LLD
            let o_idx = 1u - p_idx;
            return local_state.players[p_idx].score > local_state.players[o_idx].score;
        }
        case 208: { // C_GRP
            var count = 0u;
            let group_id = a;
            for (var s = 0u; s < 3u; s = s + 1u) {
                let s_cid = get_stage_card(p_idx, s);
                if (s_cid > 0u && s_cid < arrayLength(&card_stats)) {
                    let groups = card_stats[s_cid].groups;
                    if (((groups >> 0u) & 0xFFu) == group_id) { count = count + 1u; }
                    else if (((groups >> 8u) & 0xFFu) == group_id) { count = count + 1u; }
                    else if (((groups >> 16u) & 0xFFu) == group_id) { count = count + 1u; }
                    else if (((groups >> 24u) & 0xFFu) == group_id) { count = count + 1u; }
                }
            }
            return count >= u32(v);
        }
        case 211: { // C_SLF_GRP
            var cid = 0u;
            if (target_slot < 3u) {
                cid = get_stage_card(p_idx, target_slot);
            }
            if (cid > 0u && cid < arrayLength(&card_stats)) {
                let groups = card_stats[cid].groups;
                if (((groups >> 0u) & 0xFFu) == a) { return true; }
                if (((groups >> 8u) & 0xFFu) == a) { return true; }
                if (((groups >> 16u) & 0xFFu) == a) { return true; }
                if (((groups >> 24u) & 0xFFu) == a) { return true; }
            }
            return false;
        }
        case 213: { // C_ENR
            return local_state.players[p_idx].energy_count >= u32(v);
        }
        case 215: { // C_COST_CHK
            let avail = local_state.players[p_idx].energy_count - local_state.players[p_idx].tapped_energy_count;
            return avail >= u32(v);
        }
        case 220: { // C_CMP
            var my_val = i32(local_state.players[p_idx].score);
            var opp_val = i32(local_state.players[1u - p_idx].score);
            let comp_op = (target_slot >> 4u) & 0x0Fu;
            switch (comp_op) {
                case 0u: { return my_val >= opp_val; } // GE
                case 1u: { return my_val <= opp_val; } // LE
                case 2u: { return my_val > opp_val; }  // GT
                case 3u: { return my_val < opp_val; }  // LT
                case 4u: { return my_val == opp_val; } // EQ
                default: { return my_val > opp_val; }
            }
        }
        case 223: { // C_HRT
            var total = 0u;
            for (var j = 0u; j < 7u; j = j + 1u) { total += g_board_hearts[p_idx][j]; }
            return total >= u32(v);
        }
        case 224: { // C_BLD
            return local_state.players[p_idx].board_blades >= u32(v);
        }
        case 231: { // C_BATON
            if (local_state.prev_card_id >= 0) {
                let pcid = u32(local_state.prev_card_id);
                if (pcid < arrayLength(&card_stats)) {
                    return card_stats[pcid].char_id == u32(v);
                }
            }
            return local_state.players[p_idx].baton_touch_count >= u32(v);
        }
        default: {
            return false;
        }
    }
}

fn count_bits(v: u32) -> u32 {
    var x = v;
    x = x - ((x >> 1u) & 0x55555555u);
    x = (x & 0x33333333u) + ((x >> 2u) & 0x33333333u);
    return (((x + (x >> 4u)) & 0x0F0F0F0Fu) * 0x01010101u) >> 24u;
}

// Get card from hand (packed u16 pairs)
fn get_hand_card(p_idx: u32, h_idx: u32) -> u32 {
    let word_idx = h_idx / 2u;
    let shift = (h_idx % 2u) * 16u;
    var word = 0u;
    if (p_idx == 0u) {
        word = local_state.players[0].hand[word_idx];
    } else {
        word = local_state.players[1].hand[word_idx];
    }
    return (word >> shift) & 0xFFFFu;
}

// Get card from discard (packed u16 pairs)
fn get_discard_card(p_idx: u32, idx: u32) -> u32 {
    if (idx >= local_state.players[p_idx].discard_pile_len) { return 0u; }
    let word_idx = idx / 2u;
    let shift = (idx % 2u) * 16u;
    var word = 0u;
    if (p_idx == 0u) {
        word = local_state.players[0].discard_pile[word_idx];
    } else {
        word = local_state.players[1].discard_pile[word_idx];
    }
    return (word >> shift) & 0xFFFFu;
}

// Get card from stage (packed u16 pairs)
fn get_live_card(p_idx: u32, slot: u32) -> u32 {
    let word_idx = slot / 2u;
    let shift = (slot % 2u) * 16u;
    return (local_state.players[p_idx].live_zone[word_idx] >> shift) & 0xFFFFu;
}

fn remove_from_hand(p_idx: u32, hand_idx: u32) {
    let p_hand_len = local_state.players[p_idx].hand_len;
    if (p_hand_len == 0u || hand_idx >= p_hand_len) { return; }
    for (var i = hand_idx; i < 63u; i = i + 1u) {
        if (i >= p_hand_len - 1u) { break; }
        let next_card = get_hand_card(p_idx, i + 1u);
        let t_shift = (i % 2u) * 16u;
        local_state.players[p_idx].hand[i / 2u] = (local_state.players[p_idx].hand[i / 2u] & ~(0xFFFFu << t_shift)) | (next_card << t_shift);
    }
    let last_idx = p_hand_len - 1u;
    local_state.players[p_idx].hand[last_idx / 2u] &= ~(0xFFFFu << ((last_idx % 2u) * 16u));
    local_state.players[p_idx].hand_len -= 1u;
}

fn get_stage_card(p_idx: u32, s_idx: u32) -> u32 {
    let word_idx = s_idx / 2u;
    let shift = (s_idx % 2u) * 16u;
    return (local_state.players[p_idx].stage[word_idx] >> shift) & 0xFFFFu;
}

// Get card from deck (packed u16 pairs)
fn get_deck_card(p_idx: u32, d_idx: u32) -> u32 {
    let word_idx = d_idx / 2u;
    let shift = (d_idx % 2u) * 16u;
    return (local_state.players[p_idx].deck[word_idx] >> shift) & 0xFFFFu;
}

fn is_moved(p_idx: u32, slot: u32) -> bool {
    return (local_state.players[p_idx].moved_flags & (1u << slot)) != 0u;
}

// Select a random legal action using Reservoir Sampling
fn select_random_legal_action(p_idx: u32) -> u32 {
    var chosen_action = 0u;
    var legal_count = 1u;
    let avail_energy = local_state.players[p_idx].energy_count - local_state.players[p_idx].tapped_energy_count;
    let baton_limit = local_state.players[p_idx].baton_touch_limit;
    let baton_count = local_state.players[p_idx].baton_touch_count;
    let hand_len = local_state.players[p_idx].hand_len;

    for (var h = 0u; h < 60u; h = h + 1u) {
        if (h >= hand_len) { break; }
        let cid = get_hand_card(p_idx, h);
        if (cid == 0u || cid >= arrayLength(&card_stats)) { continue; }
        let stats = card_stats[cid];
        if (stats.card_type != 1u) { continue; }

        for (var slot = 0u; slot < 3u; slot = slot + 1u) {
            if (is_moved(p_idx, slot)) { continue; }
            var cost = stats.cost;
            let existing_cid = get_stage_card(p_idx, slot);
            if (existing_cid > 0u) {
                if (baton_count >= baton_limit) { continue; }
                let ext_cost = card_stats[existing_cid].cost;
                if (ext_cost <= cost) { cost -= ext_cost; } else { cost = 0u; }
            }
            if (cost <= avail_energy) {
                let action = 1u + h * 3u + slot;
                legal_count = legal_count + 1u;
                if ((rng_jump() % legal_count) == 0u) { chosen_action = action; }
            }
        }
    }
    return chosen_action;
}

fn select_random_biased_action(p_idx: u32) -> u32 {
    var chosen_action = 0u;
    var weight_sum = 1f;
    let avail_energy = local_state.players[p_idx].energy_count - local_state.players[p_idx].tapped_energy_count;
    let baton_limit = local_state.players[p_idx].baton_touch_limit;
    let baton_count = local_state.players[p_idx].baton_touch_count;
    let hand_len = local_state.players[p_idx].hand_len;

    for (var h = 0u; h < 32u; h = h + 1u) {
        if (h >= hand_len) { break; }
        let cid = get_hand_card(p_idx, h);
        if (cid == 0u || cid >= arrayLength(&card_stats)) { continue; }
        let stats = card_stats[cid];
        if (stats.card_type != 1u) { continue; }

        for (var slot = 0u; slot < 3u; slot = slot + 1u) {
            if (is_moved(p_idx, slot)) { continue; }
            var cost = stats.cost;
            let existing_cid = get_stage_card(p_idx, slot);
            if (existing_cid > 0u) {
                if (baton_count >= baton_limit) { continue; }
                let ext_cost = card_stats[existing_cid].cost;
                if (ext_cost <= cost) { cost -= ext_cost; } else { cost = 0u; }
            }
            if (cost <= avail_energy) {
                let action = 1u + h * 3u + slot;
                weight_sum += 5.0f;
                if ((f32(rng_jump() % 1000u) / 1000.0f) < (5.0f / weight_sum)) { chosen_action = action; }
            }
        }
    }
    return chosen_action;
}

fn rng_jump() -> u32 {
    g_rng.x ^= g_rng.y << 13u;
    g_rng.y ^= g_rng.x >> 7u;
    g_rng.x ^= g_rng.y << 17u;
    return g_rng.x + g_rng.y;
}

fn add_to_discard(p_idx: u32, card_id: u32) {
    if (card_id == 0u) { return; }
    let disc_len = local_state.players[p_idx].discard_pile_len;
    if (disc_len < 32u) {
        local_state.players[p_idx].discard_pile[disc_len] = card_id;
        local_state.players[p_idx].discard_pile_len = disc_len + 1u;
    }
}

fn add_to_hand(p_idx: u32, card_id: u32) {
    let h_idx = local_state.players[p_idx].hand_len;
    if (h_idx >= 32u) { return; }
    let word_idx = h_idx / 2u;
    let shift = (h_idx % 2u) * 16u;
    local_state.players[p_idx].hand[word_idx] = (local_state.players[p_idx].hand[word_idx] & ~(0xFFFFu << shift)) | (card_id << shift);
    local_state.players[p_idx].hand_len += 1u;
}

fn draw_card(p_idx: u32) {
    let d_len = local_state.players[p_idx].deck_len;
    if (d_len == 0u) { return; }
    let card_id = get_deck_card(p_idx, d_len - 1u);
    local_state.players[p_idx].deck_len -= 1u;
    add_to_hand(p_idx, card_id);
}

fn draw_energy(p_idx: u32) {
    if (local_state.players[p_idx].energy_deck_len > 0u) {
        local_state.players[p_idx].energy_deck_len -= 1u;
        local_state.players[p_idx].energy_count += 1u;
    }
}

fn end_main_phase() {
    let p_idx = local_state.current_player;
    let first_p = local_state.first_player;
    if (p_idx == first_p) {
        // CPU: Main → Active (opponent's turn starts)
        let other_p = 1u - first_p;
        local_state.current_player = other_p;
        local_state.phase = 1; // Active (will auto-advance to Energy → Draw → Main)
    } else {
        // Both players finished their turns → LiveSet
        local_state.phase = 5; // LiveSet
        local_state.current_player = first_p;
    }
}

fn recalculate_board_stats(p_idx: u32) {
    var total_hearts: array<u32, 8>;
    total_hearts[0] = 0u; total_hearts[1] = 0u; total_hearts[2] = 0u; total_hearts[3] = 0u;
    total_hearts[4] = 0u; total_hearts[5] = 0u; total_hearts[6] = 0u; total_hearts[7] = 0u;
    var total_blades = 0u;
    for (var s = 0u; s < 3u; s++) {
        let cid = get_stage_card(p_idx, s);
        if (cid > 0u && cid < arrayLength(&card_stats)) {
            let stats = card_stats[cid];
            let h_lo = stats.hearts_lo; let h_hi = stats.hearts_hi;
            total_hearts[0] += (h_lo >> 0u) & 0xFFu; total_hearts[1] += (h_lo >> 8u) & 0xFFu; total_hearts[2] += (h_lo >> 16u) & 0xFFu; total_hearts[3] += (h_lo >> 24u) & 0xFFu;
            total_hearts[4] += (h_hi >> 0u) & 0xFFu; total_hearts[5] += (h_hi >> 8u) & 0xFFu; total_hearts[6] += (h_hi >> 16u) & 0xFFu;
            total_blades += stats.blades;
        }
    }
    for (var i = 0u; i < 8u; i++) { g_board_hearts[p_idx][i] = total_hearts[i]; }
    local_state.players[p_idx].board_blades = total_blades;
}

fn step_state(action: u32) -> u32 {
    let p_idx = local_state.current_player;
    let phase = local_state.phase;

    if (phase == 9) { return 0u; } // Terminal

    if (phase == 1) { // Active
        local_state.phase = 2; return 1u;
    }
    if (phase == 2) { // Energy
        draw_energy(p_idx); local_state.phase = 3; return 1u;
    }
    if (phase == 3) { // Draw
        draw_card(p_idx); local_state.phase = 4; return 1u;
    }
    if (phase == 5) { // LiveSet
        if (action == 0u) {
            // Rule 8.2.2: Finish Live Set -> Draw pending cards
            var draws = local_state.players[p_idx].pending_draws;
            local_state.players[p_idx].pending_draws = 0u;

            // Draw up to 3 cards
            if (draws > 0u) { draw_card(p_idx); }
            if (draws > 1u) { draw_card(p_idx); }
            if (draws > 2u) { draw_card(p_idx); }

            let first_p = local_state.first_player;
            if (p_idx == first_p) {
                local_state.current_player = 1u - first_p;
            } else {
                local_state.phase = 6; // PerformanceP1
                local_state.current_player = first_p;
            }
            return 1u;
        } else if (action >= 400u && action < 500u) {
            let hand_idx = action - 400u;
            let card_id = get_hand_card(p_idx, hand_idx);
            if (card_id > 0u) {
                var found = false;
                for (var s = 0u; s < 3u; s = s + 1u) {
                    if (get_live_card(p_idx, s) == 0u) {
                        let word_idx = s / 2u;
                        let shift = (s % 2u) * 16u;
                        local_state.players[p_idx].live_zone[word_idx] |= (card_id << shift);
                        found = true; break;
                    }
                }

                if (found) {
                    remove_from_hand(p_idx, hand_idx);
                    local_state.players[p_idx].pending_draws += 1u;
                }
            }
            return 1u;
        }
    }
    if (phase == 6) { // PerformanceP1
        // Auto-transition to P2 for other player
        local_state.phase = 7;
        local_state.current_player = 1u - local_state.first_player;
        return 1u;
    }
    if (phase == 7) { // PerformanceP2
        // Auto-transition to LiveResult
        local_state.phase = 8;
        local_state.current_player = local_state.first_player;
        return 1u;
    }
    if (phase == 8) { // LiveResult
        // Finalize results
        local_state.turn += 1u;
        local_state.first_player = 1u - local_state.first_player;
        local_state.current_player = local_state.first_player;
        local_state.phase = 1;

        // Cleanup live zones
        local_state.players[0].live_zone[0] = 0u;
        local_state.players[0].live_zone[1] = 0u;
        local_state.players[1].live_zone[0] = 0u;
        local_state.players[1].live_zone[1] = 0u;

        recalculate_board_stats(local_state.current_player);
        return 1u;
    }

    if (phase == 4) { // Main
        if (action == 0u) {
            end_main_phase();
            recalculate_board_stats(local_state.current_player);
            return 1u;
        }

        var hand_idx = 99u;
        var slot_idx = 99u;
        var choice = -1i;
        var trigger = 0i; // 1=Play, 2=Ability

        if (action >= 1u && action <= 180u) {
            let adj = action - 1u;
            hand_idx = adj / 3u;
            slot_idx = adj % 3u;
            trigger = 1i;
        } else if (action >= 1000u && action < 2000u) {
            let adj = action - 1000u;
            hand_idx = adj / 100u;
            let rem = adj % 100u;
            slot_idx = rem / 10u;
            choice = i32(rem % 10u);
            trigger = 1i;
        }

        if (trigger == 1i) {
            let card_id = get_hand_card(p_idx, hand_idx);
            let stats = card_stats[card_id];

            var cost = stats.cost;
            let existing_cid = get_stage_card(p_idx, slot_idx);
            if (existing_cid > 0u) {
                let baton_limit = local_state.players[p_idx].baton_touch_limit;
                let baton_count = local_state.players[p_idx].baton_touch_count;
                if (baton_count >= baton_limit) { return 0u; }

                let existing_cost = card_stats[existing_cid].cost;
                if (existing_cost <= cost) { cost = cost - existing_cost; }
                else { cost = 0u; }

                add_to_discard(p_idx, existing_cid);
                local_state.players[p_idx].baton_touch_count += 1u;
            }

            let avail_energy = local_state.players[p_idx].energy_count - local_state.players[p_idx].tapped_energy_count;
            if (cost <= avail_energy) {
                local_state.players[p_idx].tapped_energy_count += cost;
                remove_from_hand(p_idx, hand_idx);

                if (stats.card_type == 1u) { // Member
                    let word_idx = slot_idx / 2u;
                    let shift = (slot_idx % 2u) * 16u;
                    local_state.players[p_idx].stage[word_idx] = (local_state.players[p_idx].stage[word_idx] & ~(0xFFFFu << shift)) | (card_id << shift);
                    local_state.players[p_idx].moved_flags |= (1u << slot_idx);

                    if (stats.bytecode_len > 0u) {
                        resolve_bytecode(p_idx, card_id, slot_idx, 1i, -1i, stats.bytecode_start, stats.bytecode_len, choice);
                    }
                } else { // Event / Spell
                    if (stats.bytecode_len > 0u) {
                        resolve_bytecode(p_idx, card_id, 99u, 1i, -1i, stats.bytecode_start, stats.bytecode_len, choice);
                    }
                    add_to_discard(p_idx, card_id);
                }
                recalculate_board_stats(p_idx);
                return 1u;
            } else { return 0u; }
        }

        if (action >= 550u && action < 850u) {
            let adj = action - 550u;
            let s_idx = adj / 100u;
            let ab_rem = adj % 100u;
            let ab_idx = i32(ab_rem / 10u);
            let c = i32(ab_rem % 10u);
            let card_id = get_stage_card(p_idx, s_idx);
            let stats = card_stats[card_id];
            if (stats.bytecode_len > 0u) {
                resolve_bytecode(p_idx, card_id, s_idx, 2i, ab_idx, stats.bytecode_start, stats.bytecode_len, c);
            }
            recalculate_board_stats(p_idx);
            return 1u;
        }

        if (action >= 200u && action < 230u) {
            let adj = action - 200u;
            let s_idx = adj / 10u;
            let ab_idx = i32(adj % 10u);
            let card_id = get_stage_card(p_idx, s_idx);
            let stats = card_stats[card_id];
            if (stats.bytecode_len > 0u) {
                resolve_bytecode(p_idx, card_id, s_idx, 2i, ab_idx, stats.bytecode_start, stats.bytecode_len, -1i);
            }
            recalculate_board_stats(p_idx);
            return 1u;
        }

        if (action >= 2000u && action < 3000u) {
            let adj = action - 2000u;
            let discard_idx = adj / 10u;
            let ab_idx = i32(adj % 10u);
            let card_id = get_discard_card(p_idx, discard_idx);

            if (card_id > 0u && card_id < arrayLength(&card_stats)) {
                let stats = card_stats[card_id];
                if (stats.bytecode_len > 0u) {
                    resolve_bytecode(p_idx, card_id, 99u, 2i, ab_idx, stats.bytecode_start, stats.bytecode_len, -1i);
                }
            }
            recalculate_board_stats(p_idx);
            return 1u;
        }
    }

    return 3u;
}

fn resolve_bytecode(p_idx: u32, card_id: u32, slot_idx: u32, trigger_filter: i32, ab_filter: i32, start: u32, total_len: u32, choice: i32) {
    if (total_len == 0u) { return; }
    var pool_ip = start;
    let pool_end = start + total_len;
    let num_abilities = u32(bytecode[pool_ip]);
    pool_ip = pool_ip + 1u;
    for (var ab_idx = 0u; ab_idx < num_abilities; ab_idx = ab_idx + 1u) {
        if (pool_ip + 3u >= pool_end) { break; }
        let ab_trigger = bytecode[pool_ip];
        let is_once = bytecode[pool_ip + 1u] == 1;
        let internal_idx = u32(bytecode[pool_ip + 2u]);
        let ab_len = u32(bytecode[pool_ip + 3u]);
        pool_ip = pool_ip + 4u;
        if (trigger_filter >= 0 && ab_trigger != trigger_filter) { pool_ip = pool_ip + ab_len; continue; }
        if (ab_filter >= 0 && i32(ab_idx) != ab_filter) { pool_ip = pool_ip + ab_len; continue; }
        if (is_once) {
            let mask = 1u << (slot_idx * 4u + internal_idx);
            if ((local_state.players[p_idx].used_abilities_mask & mask) != 0u) { pool_ip = pool_ip + ab_len; continue; }
            local_state.players[p_idx].used_abilities_mask |= mask;
        }
        var ip = pool_ip;
        let end = pool_ip + ab_len;
        var cond = true;
        var ctx_choice = choice;
        while (ip < end) {
            if (ip + 4u > end) { break; }
            let op = bytecode[ip]; let v = bytecode[ip + 1]; let a = bytecode[ip + 2]; let s_op = bytecode[ip + 3];
            ip = ip + 4;
            if (op <= 0) { continue; }
            if (op == 1) { break; }
            var real_op = op; var is_negated = false;
            if (real_op >= 1000) { real_op = real_op - 1000; is_negated = true; }
            if (real_op >= 200 && real_op <= 299) {
                let passed = check_condition_opcode(p_idx, real_op, v, u32(a), slot_idx);
                cond = passed; if (is_negated) { cond = !cond; }
                continue;
            }
            if (real_op == 2) { ip = u32(i32(ip) + v * 4); continue; }
            if (real_op == 3) { if (!cond) { ip = u32(i32(ip) + v * 4); } continue; }
            if (real_op >= 10 && real_op < 100) {
                switch (real_op) {
                    case 10: { for (var d = 0; d < v; d = d + 1) { draw_card(p_idx); } }
                    case 11: { local_state.players[p_idx].board_blades += u32(v); }
                    case 12: {
                        var color = u32(a);
                        if (color == 0u) { color = u32(ctx_choice); } // Use selected color
                        if (color < 8u) { g_board_hearts[p_idx][color] += u32(v); }
                    }
                    case 13: { local_state.players[p_idx].baton_touch_limit += u32(v); }
                    case 16: { local_state.players[p_idx].score += u32(v); }
                    case 23: { if (local_state.players[p_idx].tapped_energy_count >= u32(v)) { local_state.players[p_idx].tapped_energy_count -= u32(v); } else { local_state.players[p_idx].tapped_energy_count = 0u; } }
                    case 30: { // SELECT_MODE
                         // Simplified, usually just sets choice context
                    }
                    case 31: { // O_MOVE_TO_DISCARD
                        let t = u32(a) & 0xFDu;
                        let count = u32(v);
                        if (t == 6u) { // Hand
                            if (ctx_choice == -1i) {
                                let hand_len = local_state.players[p_idx].hand_len;
                                if (hand_len > 0u) {
                                    local_state.phase = 11; // Phase::Response
                                    // In shader we can't easily 'return' from nested loop
                                    // but setting phase=11 will cause outer loop check to break
                                }
                            } else {
                                // Choice made, discard it
                                let h_idx = u32(ctx_choice);
                                remove_from_hand(p_idx, h_idx);
                                if (local_state.is_debug > 0u) {
                                    let cid = get_discard_card(p_idx, local_state.players[p_idx].discard_pile_len - 1u);
                                     // Debug log here if needed
                                }
                            }
                        } else if (t == 4u) { // Stage
                             // Not implemented for now, uncommon for discard activation
                        } else {
                            // Deck/Energy (Automatic)
                            for (var k = 0u; k < count; k = k + 1u) {
                                if (t == 8u) { // Deck check
                                    // ... implementation of deck discard
                                }
                            }
                        }
                    }
                    case 32: { // TAP_OPPONENT
                        let o_idx = 1u - p_idx;
                        let target_slot = u32(choice);
                        if (target_slot < 3u) {
                             local_state.players[o_idx].flags |= (1u << (3u + target_slot));
                        }
                    }
                    case 45: { // COLOR_SELECT
                        ctx_choice = choice;
                    }
                    default: {}
                }
            }
        }
        pool_ip = pool_ip + ab_len;
    }
}

fn get_card_total_hearts(s: GpuCardStats) -> u32 {
    let lo = s.hearts_lo; let hi = s.hearts_hi;
    return (lo & 0xFFu) + ((lo >> 8u) & 0xFFu) + ((lo >> 16u) & 0xFFu) + ((lo >> 24u) & 0xFFu) + (hi & 0xFFu) + ((hi >> 8u) & 0xFFu) + ((hi >> 16u) & 0xFFu);
}

fn calculate_card_potential(cid: u32, max_energy: u32) -> f32 {
    if (cid == 0u || cid >= arrayLength(&card_stats)) { return 0.0; }
    let s = card_stats[cid];
    if (s.card_type != 1u) { return f32(s.extra_val) * 0.2; }
    var score: f32 = (f32(s.blades) * 10.0 + f32(get_card_total_hearts(s))) / (f32(s.cost) + 1.0);
    if (s.cost > max_energy) { score -= f32(s.cost - max_energy) * 0.5; }
    let f = s.ability_flags_lo;
    if ((f & F_DRAW) != 0u) { score += 5.0; }
    if ((f & F_SEARCH) != 0u) { score += 5.0; }
    if ((f & F_CHARGE) != 0u) { score += 1.2; }
    if ((f & F_WIN_COND) != 0u) { score += 1.0; }
    if ((s.synergy_flags & SYN_CENTER) != 0u) { score += 0.5; }
    return score;
}

fn calculate_stage_power(p_idx: u32) -> f32 {
    var stage_val: f32 = 0.0; var occupied: u32 = 0u;
    for (var i = 0u; i < 3u; i = i + 1u) {
        let cid = get_stage_card(p_idx, i);
        if (cid > 0u && cid < arrayLength(&card_stats)) {
            occupied += 1u; let s = card_stats[cid]; stage_val += f32(s.cost) * W_MEMBER_COST;
            let f = s.ability_flags_lo; var ab_val: f32 = 0.0;
            if ((f & F_CHARGE) != 0u) { ab_val += W_STAGE_ABILITY; }
            if ((f & F_DRAW) != 0u) { ab_val += W_STAGE_ABILITY * 0.8; }
            if ((f & F_WIN_COND) != 0u) { ab_val += W_STAGE_ABILITY * 1.2; }
            let mask = local_state.players[p_idx].used_abilities_mask;
            let is_tapped = ((mask >> (i * 4u)) & 0xFu) != 0u;
            if (!is_tapped && ab_val > 0.0) { ab_val *= W_UNTAPPED_BONUS; }
            if ((s.synergy_flags & SYN_CENTER) != 0u && i == 1u) { ab_val += W_SYNERGY_CENTER; }
            stage_val += ab_val;
        }
    }
    return stage_val + f32(occupied) * W_SLOT_BONUS - f32(3u - occupied) * W_SLOT_PENALTY;
}

fn calculate_live_success_prob(l_stats: GpuCardStats, board_hearts: array<u32, 8>, p_avg_hearts: array<u32, 7>, red_lo: u32, red_hi: u32, blades: u32) -> f32 {
    var r: array<u32, 7>;
    r[0] = l_stats.hearts_lo & 0xFFu; r[1] = (l_stats.hearts_lo >> 8u) & 0xFFu; r[2] = (l_stats.hearts_lo >> 16u) & 0xFFu; r[3] = (l_stats.hearts_lo >> 24u) & 0xFFu;
    r[4] = l_stats.hearts_hi & 0xFFu; r[5] = (l_stats.hearts_hi >> 8u) & 0xFFu; r[6] = (l_stats.hearts_hi >> 16u) & 0xFFu;

    if ((red_lo & 0xFFu) >= r[0]) { r[0] = 0u; } else { r[0] -= (red_lo & 0xFFu); }
    if (((red_lo >> 8u) & 0xFFu) >= r[1]) { r[1] = 0u; } else { r[1] -= ((red_lo >> 8u) & 0xFFu); }
    if (((red_lo >> 16u) & 0xFFu) >= r[2]) { r[2] = 0u; } else { r[2] -= ((red_lo >> 16u) & 0xFFu); }
    if (((red_lo >> 24u) & 0xFFu) >= r[3]) { r[3] = 0u; } else { r[3] -= ((red_lo >> 24u) & 0xFFu); }
    if ((red_hi & 0xFFu) >= r[4]) { r[4] = 0u; } else { r[4] -= (red_hi & 0xFFu); }
    if (((red_hi >> 8u) & 0xFFu) >= r[5]) { r[5] = 0u; } else { r[5] -= ((red_hi >> 8u) & 0xFFu); }
    if (((red_hi >> 16u) & 0xFFu) >= r[6]) { r[6] = 0u; } else { r[6] -= ((red_hi >> 16u) & 0xFFu); }

    var total_req = f32(r[0]) + f32(r[1]) + f32(r[2]) + f32(r[3]) + f32(r[4]) + f32(r[5]) + f32(r[6]);
    if (total_req == 0.0) { return 1.0; }

    let expected_yells = f32(blades) + 2.0;
    var wildcards = f32(board_hearts[6]) + (f32(p_avg_hearts[6]) * expected_yells / 100.0);
    var satisfied = 0.0;

    // Unrolled loop for 6 colors
    {
        let have = f32(board_hearts[0]) + (f32(p_avg_hearts[0]) * expected_yells / 100.0);
        if (have >= f32(r[0])) { satisfied += f32(r[0]); } else { let u = min(wildcards, f32(r[0]) - have); satisfied += have + u; wildcards -= u; }
    }
    {
        let have = f32(board_hearts[1]) + (f32(p_avg_hearts[1]) * expected_yells / 100.0);
        if (have >= f32(r[1])) { satisfied += f32(r[1]); } else { let u = min(wildcards, f32(r[1]) - have); satisfied += have + u; wildcards -= u; }
    }
    {
        let have = f32(board_hearts[2]) + (f32(p_avg_hearts[2]) * expected_yells / 100.0);
        if (have >= f32(r[2])) { satisfied += f32(r[2]); } else { let u = min(wildcards, f32(r[2]) - have); satisfied += have + u; wildcards -= u; }
    }
    {
        let have = f32(board_hearts[3]) + (f32(p_avg_hearts[3]) * expected_yells / 100.0);
        if (have >= f32(r[3])) { satisfied += f32(r[3]); } else { let u = min(wildcards, f32(r[3]) - have); satisfied += have + u; wildcards -= u; }
    }
    {
        let have = f32(board_hearts[4]) + (f32(p_avg_hearts[4]) * expected_yells / 100.0);
        if (have >= f32(r[4])) { satisfied += f32(r[4]); } else { let u = min(wildcards, f32(r[4]) - have); satisfied += have + u; wildcards -= u; }
    }
    {
        let have = f32(board_hearts[5]) + (f32(p_avg_hearts[5]) * expected_yells / 100.0);
        if (have >= f32(r[5])) { satisfied += f32(r[5]); } else { let u = min(wildcards, f32(r[5]) - have); satisfied += have + u; wildcards -= u; }
    }

    satisfied += min(wildcards, f32(r[6]));
    let p = satisfied / total_req; return p * p;
}

@compute @workgroup_size(64)
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    g_gid = global_id.x;
    if (g_gid >= arrayLength(&states)) { return; }
    local_state = states[g_gid];
    g_rng = vec2<u32>(local_state.rng_state_lo, local_state.rng_state_hi + g_gid);
    if (g_rng.x == 0u && g_rng.y == 0u) { g_rng.x = g_gid + 1u; }

    var live_cleared_mask: array<u32, 2>;
    var tactical_score: array<u32, 2>;
    for (var i = 0u; i < 2u; i = i + 1u) {
        live_cleared_mask[i] = 0u; tactical_score[i] = 0u;
        recalculate_board_stats(i);
    }

    let MAX_STEPS = 32u;
    for (var step = 0u; step < MAX_STEPS; step = step + 1u) {
        if (local_state.phase == 9 || local_state.winner != 0) { break; }
        var action: u32 = 0u;
        if (step == 0u && local_state.forced_action >= 0) { action = u32(local_state.forced_action); }
        else { action = select_random_biased_action(local_state.current_player); }

        let phase_status = step_state(action);
        if (local_state.is_debug != 0u) { break; }

        if (phase_status == 3u) {
            // Adversarial Turn Boundary (Live Phase Simulation)
            let cur_p = local_state.current_player;
            let current_blades = local_state.players[cur_p].board_blades;
            for (var y = 0u; y < current_blades; y = y + 1u) {
                let d_len = local_state.players[cur_p].deck_len;
                if (d_len > 0u) {
                    let drawn_id = get_deck_card(cur_p, d_len - 1u);
                    if (drawn_id > 0u && drawn_id < arrayLength(&card_stats)) {
                        let y_stats = card_stats[drawn_id];
                        g_board_hearts[cur_p][0] += y_stats.blade_hearts_lo & 0xFFu;
                        g_board_hearts[cur_p][1] += (y_stats.blade_hearts_lo >> 8u) & 0xFFu;
                        g_board_hearts[cur_p][2] += (y_stats.blade_hearts_lo >> 16u) & 0xFFu;
                        g_board_hearts[cur_p][3] += (y_stats.blade_hearts_lo >> 24u) & 0xFFu;
                        g_board_hearts[cur_p][4] += y_stats.blade_hearts_hi & 0xFFu;
                        g_board_hearts[cur_p][5] += (y_stats.blade_hearts_hi >> 8u) & 0xFFu;
                        g_board_hearts[cur_p][6] += (y_stats.blade_hearts_hi >> 16u) & 0xFFu;
                        add_to_discard(cur_p, drawn_id);
                    }
                    local_state.players[cur_p].deck_len -= 1u;
                }
            }

            for (var p: u32 = 0u; p < 2u; p = p + 1u) {
                var eval_r: array<u32, 7>;
                for (var l: u32 = 0u; l < 3u; l = l + 1u) {
                    let live_id = get_live_card(p, l);
                    if (live_id > 0u && live_id < arrayLength(&card_stats)) {
                        let ls = card_stats[live_id];
                        let rlo = local_state.players[p].heart_req_reductions[0];
                        let rhi = local_state.players[p].heart_req_reductions[1];

                        eval_r[0] = ls.hearts_lo & 0xFFu; eval_r[1] = (ls.hearts_lo >> 8u) & 0xFFu;
                        eval_r[2] = (ls.hearts_lo >> 16u) & 0xFFu; eval_r[3] = (ls.hearts_lo >> 24u) & 0xFFu;
                        eval_r[4] = ls.hearts_hi & 0xFFu; eval_r[5] = (ls.hearts_hi >> 8u) & 0xFFu;
                        eval_r[6] = (ls.hearts_hi >> 16u) & 0xFFu;

                        if ((rlo & 0xFFu) >= eval_r[0]) { eval_r[0] = 0u; } else { eval_r[0] -= (rlo & 0xFFu); }
                        if (((rlo >> 8u) & 0xFFu) >= eval_r[1]) { eval_r[1] = 0u; } else { eval_r[1] -= ((rlo >> 8u) & 0xFFu); }
                        if (((rlo >> 16u) & 0xFFu) >= eval_r[2]) { eval_r[2] = 0u; } else { eval_r[2] -= ((rlo >> 16u) & 0xFFu); }
                        if (((rlo >> 24u) & 0xFFu) >= eval_r[3]) { eval_r[3] = 0u; } else { eval_r[3] -= ((rlo >> 24u) & 0xFFu); }
                        if ((rhi & 0xFFu) >= eval_r[4]) { eval_r[4] = 0u; } else { eval_r[4] -= (rhi & 0xFFu); }
                        if (((rhi >> 8u) & 0xFFu) >= eval_r[5]) { eval_r[5] = 0u; } else { eval_r[5] -= ((rhi >> 8u) & 0xFFu); }
                        if (((rhi >> 16u) & 0xFFu) >= eval_r[6]) { eval_r[6] = 0u; } else { eval_r[6] -= ((rhi >> 16u) & 0xFFu); }

                        var sat = true; var ex = g_board_hearts[p][6];
                        for (var c: u32 = 0u; c < 6u; c = c + 1u) {
                            if (g_board_hearts[p][c] < eval_r[c]) { sat = false; break; }
                            ex += (g_board_hearts[p][c] - eval_r[c]);
                        }
                        if (sat && ex >= eval_r[6]) {
                            live_cleared_mask[p] |= (1u << l);
                            tactical_score[p] += u32(W_LIVE_SCORE);
                        } else {
                            tactical_score[p] += u32(calculate_live_success_prob(ls, g_board_hearts[p], local_state.players[p].avg_hearts, rlo, rhi, local_state.players[p].board_blades) * 5000.0);
                        }
                    }
                }
            }
            local_state.current_player = 1u - local_state.current_player;
            let next_p = local_state.current_player;
            local_state.players[next_p].energy_count = min(local_state.players[next_p].energy_count + 1u, 12u);
            if (local_state.players[next_p].deck_len > 0u && local_state.players[next_p].hand_len < 32u) { draw_card(next_p); }

            local_state.players[0].tapped_energy_count = 0u; local_state.players[1].tapped_energy_count = 0u;
            local_state.players[0].used_abilities_mask = 0u; local_state.players[1].used_abilities_mask = 0u;
            local_state.players[0].moved_flags = 0u; local_state.players[1].moved_flags = 0u;
            recalculate_board_stats(0u); recalculate_board_stats(1u);
            local_state.turn += 1u;
            local_state.players[0].lives_cleared_count += count_bits(live_cleared_mask[0]);
            local_state.players[1].lives_cleared_count += count_bits(live_cleared_mask[1]);
            live_cleared_mask[0] = 0u; live_cleared_mask[1] = 0u;
            if (local_state.players[0].lives_cleared_count >= 3u || local_state.players[1].lives_cleared_count >= 3u) {
                local_state.phase = 9;
                if (local_state.players[0].lives_cleared_count > local_state.players[1].lives_cleared_count) { local_state.winner = 1; }
                else if (local_state.players[1].lives_cleared_count > local_state.players[0].lives_cleared_count) { local_state.winner = 2; }
                else { local_state.winner = 3; }
                break;
            }
        }
    }

    var final_scores: array<f32, 2>;
    for (var p = 0u; p < 2u; p = p + 1u) {
        var score = f32(tactical_score[p]) + calculate_stage_power(p);
        var total_hearts = 0u; for (var h = 0u; h < 7u; h = h + 1u) { total_hearts += g_board_hearts[p][h]; }
        score += f32(total_hearts) * W_HEART + f32(local_state.players[p].board_blades) * W_BLADE;
        for (var i = 0u; i < local_state.players[p].hand_len; i = i + 1u) {
            score += calculate_card_potential(get_hand_card(p, i), local_state.players[p].energy_count);
        }
        score += f32(local_state.players[p].energy_count) * 0.5 + f32(local_state.players[p].tapped_energy_count) * 5.0;
        final_scores[p] = score;
    }

    var reward = clamp(0.5 + (final_scores[0] - final_scores[1]) * 0.005, 0.01, 0.99);
    if (local_state.players[0].lives_cleared_count >= 3u) { local_state.winner = 1; reward = 1.0; }
    else if (local_state.players[1].lives_cleared_count >= 3u) { local_state.winner = 2; reward = 0.0; }
    let diff = (reward - 0.5) * 2.0;
    if (diff >= 0.0) { local_state.players[0].mcts_reward = diff; local_state.players[1].mcts_reward = 0.0; }
    else { local_state.players[0].mcts_reward = 0.0; local_state.players[1].mcts_reward = -diff; }
    local_state.rng_state_lo = g_rng.x; local_state.rng_state_hi = g_rng.y;
    states[g_gid] = local_state;
}
