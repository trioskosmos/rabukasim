    ...
    pub fn dispatch(
        ...
        match op {
            ...
            O_SWAP_AREA | O_REPEAT_ABILITY | O_SET_TARGET_SELF | O_SET_TARGET_OPPONENT => {
                handle_meta_control(state, db, ctx, op, v, a, s, instr_ip).unwrap_or(HandlerResult::Continue)
            },
            // 2. Draw / Hand
            ...
            O_PLAY_MEMBER_FROM_HAND | O_PLAY_MEMBER_FROM_DISCARD | O_INCREASE_COST => {
                handle_member_state(state, db, ctx, op, v, a, s, instr_ip).unwrap_or(HandlerResult::Continue)
            },
            // 4. Energy
            O_ENERGY_CHARGE | O_PAY_ENERGY | O_ACTIVATE_ENERGY | O_PAY_ENERGY_DYNAMIC |
            O_PLACE_ENERGY_UNDER_MEMBER => {
                handle_energy(state, db, ctx, op, v, a, s, instr_ip).unwrap_or(HandlerResult::Continue)
            },
            // 5. Deck / Zones
            ...
            O_SELECT_CARDS | O_SWAP_ZONE => {
                handle_deck_zones(state, db, ctx, op, v, a, s, instr_ip).unwrap_or(HandlerResult::Continue)
            },
            // 6. Score / Hearts
