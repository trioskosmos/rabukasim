...
pub fn check_condition_opcode(
    ...
    let result = match op {
            ...
            if color_mask != 0 {
                ...
                if color_idx < 7 {
                    player.stage.iter().filter(|&&c| c >= 0).any(|&c| {
                        if let Some(m) = db.get_member(c) { m.hearts[color_idx] > 0 } else { false }
                    })
                } else { false }
            }
        }
        ...
        COST_ENERGY => {
            ...
            (player.energy_zone.len() as u32 - player.tapped_energy_mask.count_ones()) as i32 >= total_cost
        }
        COST_TAP_SELF => {
        ...
        COST_TAP_MEMBER => player.stage.iter().enumerate().any(|(i, &cid)| cid >= 0 && !player.is_tapped(i)),
        COST_TAP_ENERGY => (player.energy_zone.len() as u32 - player.tapped_energy_mask.count_ones()) as i32 >= val,
        COST_DISCARD_ENERGY => player.energy_zone.len() as i32 >= val,
        COST_PLACE_MEMBER_FROM_HAND => player.hand.len() as i32 >= val,
        C_RARITY_CHECK => {
            let cid = get_cid();
            if cid >= 0 { db.get_member(cid).map_or(false, |m| m.rarity == val as u8) } else { false }
        }
        ...
        C_COST_CHECK => {
            if let Some(cid) = state.get_context_card_id(ctx) {
                if let Some(m) = db.get_member(cid) { compare_i32(m.cost as i32, val, slot) } else { false }
            } else { false }
        }
        C_HAND_HAS_NO_LIVE => !player.hand.iter().filter(|&&id| id >= 0).any(|&id| db.get_live(id).is_some()),
        ...
        C_SCORE_COMPARE => {
            ...
            let target_score = if (attr & 0x20) != 0 || (val > 0 && opponent.score == 0) { val }
                else if val > 0 { opponent.score as i32 + val }
                else { opponent.score as i32 };
        ...
        C_HAS_KEYWORD => {
            ...
            if (attr & FILTER_REVEALED_CONTEXT) != 0 {
                if val == 1 { res = player.looked_cards.iter().any(|&cid| db.get_live(cid).is_some()); }
                else if val == 2 { res = player.looked_cards.iter().any(|&cid| db.get_member(cid).is_some()); }
            }
        ...
        C_BATON => {
            ...
            if filter_attr != 0 {
                player.baton_source_ids.iter().any(|&bid| state.card_matches_filter(db, bid, filter_attr)) 
                || (state.prev_card_id >= 0 && state.card_matches_filter(db, state.prev_card_id, filter_attr))
            } else { count_ok }
        }
        C_COUNT_LIVE_ZONE => {
        ...
        C_TYPE_CHECK => {
            ...
            let card_id = player.looked_cards.first().copied().filter(|&cid| cid >= 0).or_else(|| state.get_context_card_id(ctx));
            if let Some(card_id) = card_id {
                if check_val == 1 { db.get_live(card_id).is_some() } else { db.get_member(card_id).is_some() }
            } else { false }
        }
        C_IS_IN_DISCARD => {
        ...
        C_COST_LEAD => {
            let self_cost: i32 = player.stage.iter().filter(|&&id| id >= 0).map(|&id| db.get_member(id).map_or(0, |m| m.cost as i32)).sum();
            let opp_cost: i32 = opponent.stage.iter().filter(|&&id| id >= 0).map(|&id| db.get_member(id).map_or(0, |m| m.cost as i32)).sum();
            let reversed = (attr & 0x01) != 0;
        ...
        C_TOTAL_BLADES => {
            ...
            for slot_idx in 0..3 {
                ...
                if let Some(member) = db.get_member(cid) {
                    let mut slot_total = if player.blade_overrides[slot_idx] != -1 { player.blade_overrides[slot_idx] as i32 } else { member.blades as i32 };
                    slot_total += player.blade_buffs[slot_idx] as i32;
                    if slot_total > 0 { total = total.saturating_add(slot_total as u32); }
                }
            }
            total >= val as u32
        }
        C_COST_COMPARE => {
            ...
            if cid >= 0 {
                if let Some(m) = db.get_member(cid) {
                    ...
                    if is_le { m.cost as i32 <= val } else { m.cost as i32 >= val }
                } else { false }
            } else { false }
        }
        C_BLADE_COMPARE => {
            ...
            let is_le = (attr & 0x40000000) != 0;
            if is_le { blades <= val as u32 } else { blades >= val as u32 }
        }
        C_HEART_COMPARE => {
            let slot = if ctx.area_idx >= 0 && (ctx.area_idx as usize) < 3 { ctx.area_idx as usize } else { 0 };
            ...
            let is_le = (attr & 0x40000000) != 0;
            if is_le { count <= val } else { count >= val }
        }
        ...
        C_COUNT_ENERGY_EXACT => compare_i32(player.tapped_energy_mask.count_ones() as i32, val, slot),
        C_COUNT_BLADE_HEART_TYPES => {
            let mut combined = HeartBoard::from_array(&[0; 7]);
            for &cid in &player.yell_cards {
                if let Some(m) = db.get_member(cid) { combined.add(HeartBoard::from_array(&m.blade_hearts)); }
                else if let Some(l) = db.get_live(cid) { combined.add(HeartBoard::from_array(&l.blade_hearts)); }
            }
            let mut types = 0;
            for i in 0..7 { if combined.get_color_count(i) > 0 { types += 1; } }
            compare_i32(types, val, slot)
        }
        C_OPPONENT_HAS_EXCESS_HEART => opponent.excess_hearts > 0,
        C_SCORE_TOTAL_CHECK => {
            let mut sum = 0;
            for &cid in &player.success_lives { if let Some(l) = db.get_live(cid) { sum += l.score; } }
        ...
        306 => {
            ...
            if area_val == 0 {
                for p in 0..2 {
                    ...
                    if p == 1 && (attr & (1u64 << 40)) == 0 { continue; }
                    check_ids.extend(state.players[player_idx].stage.iter().filter(|&&id| id >= 0));
                }
            } else if area_val >= 1 && area_val <= 3 { check_ids.push(&player.stage[(area_val - 1) as usize]); }
            check_ids.into_iter().any(|&cid| cid >= 0 && state.card_matches_filter(db, cid, attr))
        }
        307 => compare_i32(resolve_count(state, db, op, attr, slot, ctx, depth), val, slot),
        308 => ctx.area_idx >= 0 && player.is_moved(ctx.area_idx as usize),
        309 => {
            let matching_count = if !ctx.selected_cards.is_empty() {
                ctx.selected_cards.iter().copied().filter(|&cid| attr == 0 || state.card_matches_filter(db, cid, attr)).count() as i32
            } else { player.discarded_this_turn as i32 };
            if (val & 0x04) != 0 && !ctx.selected_cards.is_empty() { matching_count == ctx.selected_cards.len() as i32 }
            else { compare_i32(matching_count, val, slot) }
        }
        310 => {
            let mut seen = 0u8;
            let mut count = 0;
            for &cid in &player.yell_cards {
                if let Some(m) = db.get_member(cid) {
                    for i in 0..7 { if m.hearts[i] > 0 && (seen & (1 << i)) == 0 { seen |= 1 << i; count += 1; } }
                }
            }
            compare_i32(count, val, slot)
        }
        311 => {
            let (self_cost, opp_cost) = if area_val >= 1 && area_val <= 3 {
                ...
                let s_cost = if s_cid >= 0 && (attr == 0 || state.card_matches_filter(db, s_cid, attr)) { db.get_member(s_cid).map_or(0, |m| m.cost as i32) } else { 0 };
                let o_cost = if o_cid >= 0 && (attr == 0 || state.card_matches_filter(db, o_cid, attr)) { db.get_member(o_cid).map_or(0, |m| m.cost as i32) } else { 0 };
                (s_cost, o_cost)
            } else {
                let s_cost: i32 = player.stage.iter().filter(|&&id| id >= 0 && (attr == 0 || state.card_matches_filter(db, id, attr))).map(|&id| db.get_member(id).map_or(0, |m| m.cost as i32)).sum();
                let o_cost: i32 = opponent.stage.iter().filter(|&&id| id >= 0 && (attr == 0 || state.card_matches_filter(db, id, attr))).map(|&id| db.get_member(id).map_or(0, |m| m.cost as i32)).sum();
                (s_cost, o_cost)
            };
        ...
        C_TARGET_MEMBER_HAS_NO_HEARTS => {
            ...
            if target_id >= 0 {
                if let Some(slot_idx) = player.stage.iter().position(|&id| id == target_id) {
                    state.get_effective_hearts(p_idx, slot_idx, db, depth + 1).get_color_count(6) == 0
                } else { false }
            } else { false }
        }
        _ => false,
