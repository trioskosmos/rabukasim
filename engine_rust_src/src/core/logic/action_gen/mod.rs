...
impl ActionGeneratorFactory {
    pub fn generate_actions<R: ActionReceiver + ?Sized>(
        state: &GameState,
        db: &CardDatabase,
        p_idx: usize,
        receiver: &mut R,
        ...
        match state.phase {
            ...
            Phase::MulliganP1 | Phase::MulliganP2 => {
                mulligan::MulliganGenerator.generate(db, p_idx, state, receiver)
            }
            Phase::Active | Phase::Draw | Phase::LiveResult => {
                active_draw::ActiveDrawGenerator.generate(db, p_idx, state, receiver)
            }
