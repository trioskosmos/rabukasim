    ...
    fn generate<R: ActionReceiver + ?Sized>(&self, db: &CardDatabase, p_idx: usize, state: &GameState, receiver: &mut R) {
        ...
        for s in 0..3 {
            if player.stage[s] >= 0 {
                 if let Some(prev) = db.get_member(player.stage[s]) {
                     ...
                     slot_prevents_baton_touch[s] = GameState::has_restriction(state, p_idx, s, O_PREVENT_BATON_TOUCH, db);
                 }
            }
        }
            ...
            if let Some(card) = db.get_member(cid) {
                ...
                for slot_idx in 0..3 {
                    ...
                    if player.stage[slot_idx] >= 0 {
                        ...
                        if slot_prevents_baton_touch[slot_idx] { continue; }
                    }
                    ...
                    if cost <= available_energy {
                                ...
                                let has_select_mode = (ab.choice_flags & CHOICE_FLAG_MODE) != 0;
                                    let has_color_select = (ab.choice_flags & CHOICE_FLAG_COLOR) != 0;
                                     ...
                                     if has_color_select {
                                        ...
                                        for c in 0..6 {
                                            let choice_aid = crate::core::logic::ACTION_BASE_HAND_CHOICE + (i * 100) + (slot_idx as i32 * 10) + (c as i32);
                        ...
                        if !has_choice_on_play {
                         let aid = crate::core::logic::ACTION_BASE_HAND + (i * 10) + slot_idx as i32;
                    ...
                    if multi_limit >= 2 && hand_idx < 10 && player.stage[slot_idx] >= 0 {
                        ...
                        for other_slot in 0..3 {
                            ...
                            if slot_prevents_baton_touch[other_slot] { continue; }
                            ...
                            if combined_cost <= available_energy {
                                 ...
                                 let aid = crate::core::logic::ACTION_BASE_HAND + (i * 10) + 3 + combo_idx as i32;
                                 receiver.add_action(aid as usize);
                            }
                        }
                    }
        ...
        if player.prevent_activate == 0 {
            for slot_idx in 0..3 {
                ...
                if cid >= 0 {
                    if let Some(card) = stage_data[slot_idx] {  // USE CACHED STAGE DATA
                        for (ab_idx, ab) in card.abilities.iter().enumerate() {
                            if ab.trigger == TriggerType::Activated {
                                ...
                                if cond_ok && cost_ok && state.check_once_per_turn(p_idx, 0, cid as u32, ab_idx) {
                ...
                if let Some(card) = db.get_member(cid) {
                    for (ab_idx, ab) in card.abilities.iter().enumerate() {
                        if ab.trigger == TriggerType::Activated {
                            ...
                            let cond_ok = ab.conditions.iter().all(|c| state.check_condition(db, p_idx, c, &ctx, 0));
                            let cost_ok = ab.costs.iter().all(|c| state.check_cost(db, p_idx, c, &ctx));
                            if cond_ok && cost_ok && state.check_once_per_turn(p_idx, 1, cid as u32, ab_idx) {
                                let ab_aid = crate::core::logic::ACTION_BASE_HAND_ACTIVATE + (i * 10) + (ab_idx as i32);
                                receiver.add_action(ab_aid as usize);
                            }
                        }
                    }
