...
pub fn suspend_interaction(state: &mut GameState, db: &CardDatabase, ctx: &AbilityContext, instr_ip: usize, effect_opcode: i32, target_slot: i32, choice_type: &str, choice_text: &str, filter_attr: u64, v_remaining: i16) -> bool {
    ...
    if actions.len() <= 1 && (actions.is_empty() || actions.contains(&0)) && choice_type != "OPPONENT_CHOOSE" {
        if state.debug.debug_mode {
            if state.debug.debug_mode { println!("[DEBUG] Softlock prevented: {} has no legal actions. Skipping suspension.", choice_type); }
        }
...
pub fn check_condition_opcode(state: &GameState, db: &CardDatabase, op: i32, val: i32, attr: u64, slot: i32, ctx: &AbilityContext, depth: u32) -> bool {
    ...
    if state.debug.debug_mode {
        println!("[SHADOW-LEGACY] Condition Opcode: {}, Value: {}, Attr: {}, Slot: {}, Source: {:?}", op, val, attr, slot, get_cid());
    }
...
pub fn get_condition_count(state: &GameState, db: &CardDatabase, cond_id: i32, attr: i32, ctx: &AbilityContext) -> i32 {
    ...
    match cond_id {
        ...
        204 => { // COUNT_HAND
             let mut count = 0;
             if !only_opponent { count += get_zone_count(&player.hand); }
             if include_opponent { count += get_zone_count(&opponent.hand); }
             count
        },
        205 => { // COUNT_DISCARD
             let mut count = 0;
             if !only_opponent { count += get_zone_count(&player.discard); }
             if include_opponent { count += get_zone_count(&opponent.discard); }
             count
        },
        213 => { // COUNT_ENERGY
             let mut count = 0;
             if !only_opponent { count += player.energy_zone.len() as i32; }
             if include_opponent { count += opponent.energy_zone.len() as i32; }
             count
        },
        218 => { // COUNT_SUCCESS_LIVE
             let mut count = 0;
             if !only_opponent { count += get_zone_count(&player.success_lives); }
             if include_opponent { count += get_zone_count(&opponent.success_lives); }
             count
        },
        223 => { // COUNT_HEARTS
            let mut count = 0;
            if !only_opponent {
                ...
                count += if attr == 0 { totals.to_array().iter().map(|&x| (x as i32)).sum::<i32>() }
                         else { let color_idx = (attr as usize).saturating_sub(1); if color_idx < 7 { totals.get_color_count(color_idx) as i32 + totals.to_array()[6] as i32 } else { 0 } };
            }
            if include_opponent {
                let totals = state.get_total_hearts(1 - p_idx, db, 0);
                count += if attr == 0 { totals.to_array().iter().map(|&x| (x as i32)).sum::<i32>() }
                         else { let color_idx = (attr as usize).saturating_sub(1); if color_idx < 7 { totals.get_color_count(color_idx) as i32 + totals.to_array()[6] as i32 } else { 0 } };
            }
            count
        ...
        224 => { // COUNT_BLADES
            let mut count = 0;
            if !only_opponent { count += state.get_total_blades(p_idx, db, 0) as i32; }
            if include_opponent { count += state.get_total_blades(1 - p_idx, db, 0) as i32; }
            count
        },
        230 => { // COUNT_LIVE_ZONE
             let mut count = 0;
             if !only_opponent { count += get_zone_count(&player.live_zone); }
             if include_opponent { count += get_zone_count(&opponent.live_zone); }
             count
        },
        _ => 0,
    }
...
pub fn resolve_bytecode(state: &mut GameState, db: &CardDatabase, bytecode: &[i32], ctx_in: &AbilityContext) {
    ...
    while sp > 0 {
        ...
        while ip < len && steps < 1000 {
            ...
            if state.debug.debug_mode {
                if state.debug.debug_mode { println!("[DEBUG] BC_STEP: ip={}, op={}, v={}, a={}, s={}, cond={}", instr_ip, real_op, v, a, s, cond); }
            }
            ...
            if state.debug.debug_mode {
                if state.debug.debug_mode { println!("[DEBUG COND_GATE] ip={}, op={}, cond={}, choice_index={}", instr_ip, real_op, cond, ctx.choice_index); }
            }
            if cond {
                let p_idx = ctx.player_id as usize;
        ...
        if is_live {
            if let Some(live) = db.get_live(cid) {
                if (ab_idx as usize) < live.abilities.len() {
                    state.resolve_bytecode_cref(db, &live.abilities[ab_idx as usize].bytecode, &ctx);
                }
            }
        } else {
            if let Some(member) = db.get_member(cid) {
                if (ab_idx as usize) < member.abilities.len() {
                    state.resolve_bytecode_cref(db, &member.abilities[ab_idx as usize].bytecode, &ctx);
                }
            }
        }
