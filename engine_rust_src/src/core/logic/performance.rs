...
pub fn do_performance_phase(state: &mut GameState, db: &CardDatabase) {
    ...
    for i in 0..3 {
        ...
        if cid >= 0 && db.get_live(cid).is_none() {
            if !state.ui.silent { state.log(format!("Rule 8.3.4: Discarding non-live card #{} from Live Zone.", cid)); }
    ...
    if !state.performance_yell_done[p_idx] {
        if !state.ui.silent { state.log(format!("Rule 8.3.11: Player {} performs Yell ({} blades).", p_idx, total_blades)); }
    ...
    for &cid in state.core.players[p_idx].yell_cards.iter() {
        ...
        for i in 0..7 { adj_bh[i] = bh[i] as u32; }

        for &(src_cid, src_col, dst_col) in &state.core.players[p_idx].color_transforms {
            if src_col == 0 && (dst_col as usize) < 7 {
                let mut sum = 0;
                for i in 0..7 { if i != dst_col as usize { sum += adj_bh[i]; adj_bh[i] = 0; } }
                adj_bh[dst_col as usize] += sum;

                let source_name = if let Some(m) = db.get_member(src_cid) { m.name.clone() }
                                  else if let Some(l) = db.get_live(src_cid) { l.name.clone() }
                                  else { "Effect".to_string() };
    ...
    for i in 0..3 {
        if let Some(cid) = state.core.players[p_idx].live_zone.get(i).copied().filter(|&c| c >= 0) {
            if let Some(live) = db.get_live(cid) {
                ...
                if check_live_success(state, p_idx, live, &remaining_hearts) {
                    ...
                    state.log(format!("    -> FAILED for {} (Hearts or Restrictions)", live.name));
                    any_failed = true;
                }
            }
        }
    ...
    if any_failed {
        ...
        for i in 0..3 {
            if state.core.players[p_idx].live_zone[i] >= 0 {
                state.core.players[p_idx].discard.push(state.core.players[p_idx].live_zone[i]);
                state.core.players[p_idx].live_zone[i] = -1;
...
pub fn do_live_result(state: &mut GameState, db: &CardDatabase) {
    ...
    for p in 0..2 {
        ...
        if has_live && snapshot_success {
            ...
            for i in 0..3 {
                if state.core.players[p].live_zone[i] >= 0 {
                    state.core.players[p].discard.push(state.core.players[p].live_zone[i]);
        ...
        for &(t_cid, s_cid, ab_idx) in &state.core.players[p].granted_abilities {
             if let Some(slot) = state.core.players[p].stage.iter().position(|&cid| cid == t_cid) {
        ...
        for (cid, bonus) in constant_bonuses {
            ...
            let name = db.get_member(cid).map(|m| m.name.clone()).unwrap_or_else(|| format!("Card {}", cid));
            score_breakdown.push(json!({
                "source": name,
                "source_id": cid,
                "value": bonus,
        ...
        for &(cid, bonus) in &state.core.players[p].live_score_bonus_logs {
            let name = if cid >= 0 {
                db.get_member(cid).map(|m| m.name.clone())
        ...
        scores[p] = live_score + total_constant_bonus.max(0) as u32 + state.core.players[p].live_score_bonus.max(0) as u32;
    ...
    if !state.ui.performance_results.is_empty() {
        for p in 0..2u8 {
            let mut map = if let Some(serde_json::Value::Object(m)) = state.ui.performance_results.get(&p).cloned() {
                m
            } else {
            ...
            state.ui.performance_history.push(serde_json::Value::Object(map));
        }
    }
    ...
    if !state.ui.silent {
        state.log(format!("Rule 8.4.6: P0 Score: {} (Success: {} wins: {})", scores[0], has_success[0], p0_wins));
        state.log(format!("Rule 8.4.6: P1 Score: {} (Success: {} wins: {})", scores[1], has_success[1], p1_wins));
    }
    ...
    for i in 0..2 {
        ...
        if wins {
            ...
            let valid_candidates: Vec<usize> = cards_in_zone.iter().cloned()
                .filter(|&i| {
                    let cid = state.core.players[p].live_zone[i];
                    if let Some(card) = db.get_live(cid) {
                        ...
                        if let Some(res) = perf_res {
                            if let Some(lives) = res.get("lives").and_then(|l| l.as_array()) {
                                if let Some(live_res) = lives.get(i) {
                                    return live_res.get("passed").and_then(|v| v.as_bool()).unwrap_or(false);
                                }
                            }
                        }
            ...
            if is_tie_capped {
                ...
                if !choices_pending {
                    ...
                    if !state.ui.silent { state.log(format!("Rule 8.4.7.3: P{} must SELECT a success live card.", p)); }
                }
            }
        }
    }
...
pub fn finalize_live_result(state: &mut GameState) {
    ...
    if state.phase != Phase::Terminal {
        ...
        if s0 && !s1 {
            ...
            if !state.ui.silent { state.log("Rule 8.4.13: P0 obtained Success Live. Now First Player.".to_string()); }
        } else if s1 && !s0 {
            state.first_player = 1;
            if !state.ui.silent { state.log("Rule 8.4.13: P1 obtained Success Live. Now First Player.".to_string()); }
        } else {
