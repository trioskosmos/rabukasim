...
impl GameState {
    pub fn dump_diagnostics(&self, db: &CardDatabase) {
        println!("\n================================================================================");
        println!("DIAGNOSTIC DUMP - Turn: {}, Phase: {:?}, Current Player: {}", self.turn, self.phase, self.current_player);
        println!("================================================================================");
        ...
        for i in 0..2 {
            ...
            for slot in 0..3 {
                ...
                if cid >= 0 {
                    let card_no = db.get_member(cid).map(|m| m.card_no.as_str()).unwrap_or("???");
            ...
            print!("  [ENERGY] ");
            if p.energy_zone.is_empty() { print!("(Empty)"); }
            for (idx, &cid) in p.energy_zone.iter().enumerate() {
                let is_tapped = p.is_energy_tapped(idx);
                print!("{}{}{} ", if is_tapped { "[" } else { "" }, cid, if is_tapped { "]" } else { "" });
            }
            println!();
