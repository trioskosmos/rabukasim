...
impl GameStateEncoding for GameState {
    fn encode_state(&self, db: &CardDatabase) -> Vec<f32> {
        ...
        let p0_tapped = (0..self.core.players[0].energy_zone.len())
            .filter(|&i| self.core.players[0].is_energy_tapped(i))
            .count();
        let p1_tapped = (0..self.core.players[1].energy_zone.len())
            .filter(|&i| self.core.players[1].is_energy_tapped(i))
            .count();
        ...
        for i in 0..3 {
            self.encode_live_slot(&mut feats, 0, i, db, true);
        }
        for i in 0..3 {
        ...
        for i in 0..3 {
            ...
            for c in 0..7 {
                p0_stage_hearts[c] += h_arr[c] as u32;
            }
        }
        ...
        while feats.len() < TOTAL_SIZE - 10 {
            feats.push(0.0);
        }
        while feats.len() < TOTAL_SIZE {
            feats.push(0.0);
        }
        if feats.len() > TOTAL_SIZE {
            feats.truncate(TOTAL_SIZE);
        }
        feats
    }
    ...
    fn encode_member_slot(
        ...
        if cid >= 0 {
            if let Some(m) = db.get_member(cid) {
                ...
                for h in hearts.to_array() {
                    feats.push(h as f32 / 5.0);
                }
                for _ in 0..7 {
                    feats.push(0.0);
                }
                for &bh in &m.blade_hearts {
                    feats.push(bh as f32 / 5.0);
                }
                feats.push(m.note_icons as f32 / 5.0);
                feats.push(if (m.semantic_flags & 0x01) != 0 {
                    1.0
                } else {
                    0.0
                });
                feats.push(if (m.semantic_flags & 0x04) != 0 {
                    1.0
                } else {
                    0.0
                });
                feats.push(if (m.semantic_flags & 0x02) != 0 {
                    1.0
                } else {
                    0.0
                });
                feats.push(if self.core.players[p_idx].is_tapped(slot) {
                    1.0
                } else {
                    0.0
                });
                while (feats.len() % 48) != 0 {
                    feats.push(0.0);
                }
                return;
            }
    ...
    fn encode_live_slot(
        ...
        if cid >= 0 && visible {
            if let Some(l) = db.get_live(cid) {
                ...
                for &h in &l.required_hearts {
                    feats.push(h as f32 / 5.0);
                }
                for &bh in &l.blade_hearts {
                    feats.push(bh as f32 / 5.0);
                }
                feats.push(l.note_icons as f32 / 5.0);
                feats.push(if (l.semantic_flags & 0x01) != 0 {
                    1.0
                } else {
                    0.0
                });
    ...
    fn encode_card_features(&self, feats: &mut Vec<f32>, cid: i32, db: &CardDatabase) {
        ...
        if let Some(m) = db.get_member(cid) {
            ...
            for _ in 0..7 {
                feats.push(0.0);
            } // Padding for blade hearts if 7 colors
            feats.push(m.note_icons as f32 / 5.0);
            feats.push(if (m.semantic_flags & 0x01) != 0 {
                1.0
            } else {
