// @generated by tools/codegen_abilities.py on 2026-03-19T16:16:28.693988
...
pub fn execute_hardcoded_ability(state: &mut GameState, _db: &CardDatabase, card_id: i32, ab_idx: usize, ctx: &AbilityContext) -> bool {
    ...
    match (card_id, ab_idx) {
        ...
        (24, 1) => {
            // pseudocode: TRIGGER: CONSTANT EFFECT: COUNT_MEMBER {FILTER="OPPONENT, STATUS=TAPPED"} -> COUNT_VAL; ADD_BLADES(1, PER_CARD=COUNT_VAL) -> SELF
        ...
        (120, 1) => {
            // pseudocode: TRIGGER: CONSTANT EFFECT: COUNT_CARDS(ZONE="SUCCESS_PILE", PLAYER) -> COUNT; EFFECT: ADD_BLADES(1, PER_CARD=COUNT) -> SELF
            state.players[p_idx].blade_buffs[268487424 as usize] += 1;
            true
        },
        (157, 0) => {
        ...
        (193, 1) => {
            // pseudocode: TRIGGER: CONSTANT EFFECT: COUNT_MEMBER(PLAYER) {FILTER="UNIT_MIRAKURA, NOT_SELF"} -> COUNT_VAL; ADD_BLADES(1, PER_CARD=COUNT_VAL) -> SELF
            state.players[p_idx].blade_buffs[268487424 as usize] += 1;
            true
        },
        (4289, 1) => {
        ...
        (617, 0) => {
            // pseudocode: TRIGGER: ON_PLAY EFFECT: COUNT_ENERGY(PLAYER) -> COUNT_VAL; DIV_VAL(COUNT_VAL, 6) -> DRAW_COUNT; DRAW(DRAW_COUNT)
            state.draw_cards(p_idx, 1 as u32);
            true
        },
        (626, 0) => {
