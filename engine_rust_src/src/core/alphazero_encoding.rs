...
impl AlphaZeroEncoding for GameState {
    fn to_alphazero_tensor(&self, db: &CardDatabase) -> Vec<f32> {
        ...
        tensor.push(if self.core.performance_yell_done[opp] {
            1.0
        } else {
            0.0
        });
        tensor.push(if self.core.live_result_selection_pending {
            1.0
        } else {
            0.0
        });
...
fn append_entity_vector(
    ...
    if let Some(m) = db.get_member(template_id) {
        ...
        for i in 0..7 {
            if m.hearts[i] > 0 {
                attr_mask += (1 << i) as f32;
            }
        }
        tensor.push(attr_mask);
        tensor.push(if m.blade_hearts.iter().any(|&h| h > 0) {
            1.0
        } else {
            0.0
        });
        ...
        tensor.push(attr_mask);
        tensor.push(if l.blade_hearts.iter().any(|&h| h > 0) {
            1.0
        } else {
            0.0
        });
    ...
    if let Some(abs) = abilities {
        for (ab_idx, ab) in abs.iter().enumerate() {
            ...
            let is_spent = if ab.is_once_per_turn {
                ...
                if player.used_abilities.contains(&uid) {
                    1.0
                } else {
                    0.0
                }
