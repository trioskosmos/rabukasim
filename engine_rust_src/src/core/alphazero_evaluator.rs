...
use std::sync::Arc;
use crate::core::heuristics::{HeuristicConfig, Heuristic};
...
impl AlphaZeroEvaluator for HeuristicBaselineEvaluator {
    fn evaluate_batch(&self, states: &[GameState], db: &CardDatabase) -> Vec<AlphaZeroOutput> {
        ...
        states.iter().map(|s| {
            let val = h.evaluate(s, db, s.players[0].score, s.players[1].score, EvalMode::Normal, None, None);
...
impl AlphaZeroEvaluator for PyAlphaZeroEvaluator {
    fn evaluate_batch(&self, states: &[GameState], db: &CardDatabase) -> Vec<AlphaZeroOutput> {
        Python::with_gil(|py| {
            ...
            let result = self.model.call_method1(py, "predict_batch", (py_tensors,))
                .expect("Python AlphaZero model call failed! Ensure model has predict_batch(tensors).");
