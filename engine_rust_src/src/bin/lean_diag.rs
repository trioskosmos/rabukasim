...
fn main() {
    ...
    println!(
        "[ACTION GENERATION] {} legal actions generated in {:.3}ms",
        legal.len(),
        action_gen_time.elapsed().as_secs_f32() * 1000.0
    );
    ...
    println!(
        "[STATE CLONE] Single clone took {:.3}ms",
        clone_cost.as_secs_f32() * 1000.0
    );
    ...
    println!(
        "[STATE STEP] Single step took {:.3}ms\n",
    ...
    println!(
        "  Est. clones: {} (if each node cloned once)",
        seq_count * 2
    );

    // Plan turn (our optimized version)
    let plan_start = Instant::now();
    let (_seq, _val, _brk, evals) =
        engine_rust::core::logic::turn_sequencer::TurnSequencer::plan_full_turn(&state, &db);
    ...
    println!(
        "  Without counting: {:.3}ms",
        plan_time.as_secs_f32() * 1000.0
    );
    println!(
        "  With counting (current simple_game): {:.3}ms",
        (count_time.as_secs_f32() + plan_time.as_secs_f32()) * 1000.0
    );
    println!(
        "  Overhead from counting: {:.3}ms ({:.1}% of total)",
