...
fn analyze_state(label: &str, state: &GameState, db: &engine_rust::core::logic::CardDatabase, time_limits: &[f32]) {
    println!("\n=== {} ===", label);
    println!("Phase: {:?}, Turn: {}, Legal Actions: {}", state.phase, state.turn, {
    ...
    for &timeout in time_limits {
        ...
        let (stats, profiler) = mcts.search(state, db, 0, timeout, SearchHorizon::TurnEnd(), &heuristic);
        ...
        println!("  - Sims per Second:   {:.0}", (total_visits as f64) / elapsed);
...
fn main() {
    ...
    // State 2: Mid-game
    analyze_state("State 2: Mid-game (After 20 random actions)", &state, &db, &time_limits);
}
