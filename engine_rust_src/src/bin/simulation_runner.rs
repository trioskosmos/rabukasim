use engine_rust::core::heuristics::{Heuristic, OriginalHeuristic};
...
fn main() {
    ...
    let p0_main: Vec<i32> = p0_deck
        .iter()
        .filter(|&&id| db.get_member(id).is_some() || db.get_live(id).is_some())
        .cloned()
        .collect();
    let p1_main: Vec<i32> = p1_deck
        .iter()
        .filter(|&&id| db.get_member(id).is_some() || db.get_live(id).is_some())
        .cloned()
        .collect();
    ...
    while !state.is_terminal() && move_count < 1000 {
        ...
        println!(
            "\n--- [MOVE {}] Turn {} | Phase {:?} | Player {} ---",
            move_count, state.turn, state.phase, current_p
        );

        // Log current probability as seen by the heuristic
        let current_eval = heuristic.evaluate(
            &state,
            &db,
            0,
            0,
        ...
        println!(
            "[AI View] Evaluation Score: {:.4} (higher favors P0)",
            current_eval
        );
        ...
        if let Some((action, q_val, visits)) = suggestions.first() {
            println!(
                "[AI Decision] Action {} | Q-Value: {:.4} | Visits: {}",
                action, q_val, visits
            );
