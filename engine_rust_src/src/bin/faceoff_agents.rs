...
fn get_action(agent: Agent, state: &GameState, db: &engine_rust::core::logic::CardDatabase, mcts: &mut MCTS, heuristic: &OriginalHeuristic, rng: &mut rand::rngs::ThreadRng) -> i32 {
    ...
    if valid_actions.is_empty() { return 0; }
    if valid_actions.len() == 1 { return valid_actions[0]; }
    ...
    match agent {
        ...
        Agent::Greedy => {
            ...
            for &action in &valid_actions {
                ...
                let mut eval = if next_sim.is_terminal() {
                    ...
                    let h_val = heuristic.evaluate(&next_sim, db, state.players[0].score, state.players[1].score, engine_rust::core::heuristics::EvalMode::Normal, None, None);
                    if p_idx == 1 { 1.0 - h_val } else { h_val } // Convert P0 win rate to Current Player win rate
                };
                
                // Tiebreaker: avoid passing instantly unless it's strictly better
                if action == 0 { eval -= 0.0001; }
        ...
        Agent::Mcts(timeout) => {
            let (stats, _) = mcts.search(state, db, 0, timeout, SearchHorizon::GameEnd(), heuristic);
            if stats.is_empty() { valid_actions[0] } else { stats[0].0 }
        }
    }
}
...
fn run_matchup(agent0: Agent, agent1: Agent, num_games: usize) {
    ...
    println!("\n--- Results for {} vs {} ---", agent0.name(), agent1.name());
