use engine_rust::core::logic::{GameState, CardDatabase, ACTION_SPACE};
...
fn run_single_thread(initial_state: &GameState, db: &CardDatabase, duration: Duration, seed: u64) -> (u64, u64) {
    ...
    while start.elapsed() < duration {
        ...
        while !sim.is_terminal() && steps < 1000 {
            ...
            for (i, &b) in mask.iter().enumerate() {
                if b {
                    if first_valid < 0 { first_valid = i as i32; }
                    valid_count += 1;
                }
            }
...
fn main() {
    ...
    let p_main = if p_deck.is_empty() {
        println!("Warning: Could not find deck at {}, using fallback.", deck_path);
    ...
    println!("Avg Steps/g: {:.1}", st_steps as f64 / st_games.max(1) as f64);
    ...
    println!("Avg Steps/g: {:.1}", mt_steps as f64 / mt_games.max(1) as f64);
    println!("Speedup:     {:.2}x", mt_steps as f64 / st_steps.max(1) as f64);
