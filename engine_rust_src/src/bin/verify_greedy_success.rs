...
fn main() {
    ...
    while !sim.is_terminal() && steps < 200 {
        // Fewer steps for debug
        mask.fill(false);
        ...
        for &action in &valid_actions {
            ...
            let chances = PerformanceProbabilitySolver::analyze_current_permissible_lives(
                &next_sim,
                &db,
                current_player,
            );
            let max_prob = chances
                .iter()
                .map(|c| c.1.success_probability)
                .fold(0.0, f32::max);
            eval += max_prob * 1000.0;

            if next_sim.phase == sim.phase
                && next_sim.players[current_player].score == sim.players[current_player].score
                && action == 0
            {
                eval -= 5000.0;
            }
