...
fn test_refresh_on_empty_draw() {
    ...
    state.core.players[0].discard = vec![121, 124, 122].into(); // Real IDs
    ...
    assert!(state.core.players[0].get_flag(PlayerState::FLAG_DECK_REFRESHED));
}

#[test]
fn test_refresh_on_look_at_top_x() {
    let mut state = create_test_state();
    state.core.players[0].deck = vec![120].into(); // Only 1 card in deck
    state.core.players[0].discard = vec![121, 124, 122].into();
    ...
    assert_eq!(state.core.players[0].discard.len(), 0);
    assert!(state.core.players[0].get_flag(PlayerState::FLAG_DECK_REFRESHED));
    ...
    assert_eq!(state.core.players[0].looked_cards[0], 120);
}

#[test]
fn test_refresh_order_preservation() {
    let mut state = create_test_state();
    state.core.players[0].deck = vec![121, 120].into(); // 120 is on top
    state.core.players[0].discard = vec![124, 122, 137].into();
    ...
    assert_eq!(state.core.players[0].deck.pop(), Some(120));
    assert_eq!(state.core.players[0].deck.pop(), Some(121));
...
fn test_refresh_on_look_and_choose() {
    let mut state = create_test_state();
    state.core.players[0].deck = vec![120].into(); // Only 1 card (Honoka)
    state.core.players[0].discard = vec![121, 124, 122].into();
    ...
    // assert_eq!(state.core.players[0].hand.len(), 1);
    assert_eq!(state.core.players[0].hand.last(), Some(&120));
    assert_eq!(state.core.players[0].deck.len(), 3);
}
