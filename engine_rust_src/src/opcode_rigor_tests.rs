...
fn test_opcode_swap_cards_deck_refresh() {
    ...
    assert_eq!(state.core.players[0].hand.len(), 2, "Hand size should be 2 after moving 2 cards from deck (including refresh)"); 
    assert_eq!(state.core.players[0].deck.len(), 2, "Deck size should be 2 after moving 2 cards from deck+discard (total 4)"); 
}
...
fn test_opcode_increase_cost_ripple() {
    ...
    assert_eq!(new_cost, base_cost + 2, "Cost should increase by 2 (Baseline: {}, Actual: {})", base_cost, new_cost);
}
...
fn test_opcode_select_live_rigor() {
    ...
    assert_eq!(state.interaction_stack.last().map(|i| i.choice_type.as_str()).unwrap_or(""), "SELECT_LIVE");
    ...
    assert!(state.interaction_stack.is_empty(), "Interaction stack should be empty after response");
}
...
fn test_opcode_opponent_choose_rigor() {
    ...
    let ctx = AbilityContext { player_id: 0, area_idx: 0, source_card_id: 3001, ..Default::default() };
    ...
    println!("DEBUG: interaction.ctx.player_id = {}", interaction.ctx.player_id);
    assert_eq!(interaction.ctx.player_id, 1, "Opponent (P1) should be the chooser");

    let _ = state.step(&db, ACTION_BASE_CHOICE + 0); // P1 chooses index 0

    println!("DEBUG: P0 hand len = {}, P1 hand len = {}", state.core.players[0].hand.len(), state.core.players[1].hand.len());
    assert_eq!(state.phase, Phase::Main);
    // Note: O_OPPONENT_CHOOSE flips ctx.player_id during resumption.
    // Subsequent opcodes (O_DRAW) use the flipped context.
    assert_eq!(state.core.players[1].hand.len(), 1, "Player 1 should have drawn the card due to context flip");
}
...
fn test_opcode_reduce_yell_count_functional() {
    ...
    assert_eq!(revealed.len(), 1, "Should reveal only 1 card due to reduction");
}
...
fn test_opcode_prevent_activate_rigor() {
    ...
    assert_eq!(state.core.players[0].prevent_activate, 1, "prevent_activate flag should be set");
