...
def enrich_label(lbl, player_obj, flattened_db):
    ...
    if match_hand:
        ...
        if idx < len(player_obj.hand):
            card_cid = player_obj.hand[idx]

    match_stage = re.search(r"Member Slot (\d+)", lbl)
    if match_stage:
        idx = int(match_stage.group(1))
        if idx < len(player_obj.stage):
            card_cid = player_obj.stage[idx]

    match_discard = re.search(r"Discard(?: Index)? (\d+)", lbl)
    if match_discard:
        idx = int(match_discard.group(1))
        if idx < len(player_obj.discard):
            card_cid = player_obj.discard[idx]

    if card_cid != -1:
        # Mask UID to get base card
...
def run_audit():
    print("--- MCTS 10s Stability Audit (Real Decks) ---")
    results = []
    ...
    print("Advancing game to first Live success...")
    max_turns = 100
    while not state.is_terminal() and state.turn < max_turns:
        ...
        if len(p0.success_lives) > 0 or len(p1.success_lives) > 0:
            print(f"Moment reached at Turn {state.turn}!")
            break
    ...
    # Display full action list first
    results.append("\nLEGAL ACTIONS:")
    print("\nLEGAL ACTIONS:")
    for aid in legal_ids:
        lbl = state.get_action_label(aid)
        lbl = enrich_label(lbl, active_player_obj, flattened_db)
        action_line = f"  - {lbl} (ID: {aid})"
        print(action_line)
        results.append(action_line)

    results.append("\nMonitoring best move stability for 10 seconds...")
    print("\nMonitoring best move stability for 10 seconds...")
    ...
    # Audit Log
    results.insert(0, f"Audit at T{state.turn} {phase_name} for Player {state.current_player}")

    while time.time() - start_time < 10:
        # Increase sims gradually to simulate time passage
        sugg = state.get_mcts_suggestions(sims, 0.0, engine_rust.SearchHorizon.GameEnd(), engine_rust.EvalMode.Normal)
        if not sugg:
            sims += 100
            continue
        ...
        # Accelerate simulation count
        sims = int(sims * 1.25) + 100
        # throttle slightly so we don't spam too fast in early tiny sims
        time.sleep(0.1)

    print("\nAudit Complete.")
    with open("mcts_audit_log.txt", "w", encoding="utf-8") as f:
        f.write("\n".join(results))
