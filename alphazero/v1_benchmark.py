...
from alphazero.alphanet import NUM_ACTIONS
...
def get_player_move(player_type, state, db, model, device, sims=128):
    ...
    if not legal_ids:
        ...
        if pi:
            print(
                f"      [DEBUG] Interaction: type={pi.choice_type}, filter={pi.filter_attr}, ctx={pi.ctx}", flush=True
            )
    ...
    if player_type == "Heuristic_Greedy":
        ...
        res = state.get_mcts_suggestions(
            sims, 0.0, engine_rust.SearchHorizon.GameEnd(), engine_rust.EvalMode.TerminalOnly
        )
        move = res[0][0] if res else random.choice(legal_ids)
...
def get_state_str(state):
    try:
        lines = []
        lines.append("--- Board State Snapshot ---")
...
def play_match(p0_type, p1_type, decks, db_engine, model, device, sims=128):
    ...
    while not state.is_terminal() and state.turn < 120 and step_count < 1000:
        ...
        if state.phase == 0:
            ...
            phase_name = "SelectMode"
        elif state.phase == 4:
            phase_name = "SelectColor"
        elif state.phase == 5:
            phase_name = "SelectStageSlot"
        elif state.phase == 6:
            phase_name = "SelectMember"
        elif state.phase == 7:
            phase_name = "SelectChoice"

        action_history.append(
            f"Step {step_count} (T-Step {turn_steps}) | Turn {state.turn} | Phase {phase_name} ({state.phase}) | P{state.current_player} ({p_type}) | Move {move}: {action_label} | Legals: {legal_labels}"
        )
...
def run_stress_test(db_engine, decks, model, device, duration_secs, sims):
    ...
    while time.time() - start_time < duration_secs:
        ...
        if (
            winner == -1 and turns < 100
        ):  # -1 usually means softlock or terminal without winner? Actually engine uses -1 for draws/stalls
            stalls += 1
    ...
    for s_idx, (label, p0_type, p1_type) in enumerate(scenarios):
        ...
        for i in range(games_per_scenario):
            ...
            try:
                ...
                print(
                    f"  Game {i + 1}/{games_per_scenario} DONE. Result: {winner}, Turns: {turns}. Stats: ({p0_wins}-{p1_wins}-{draws})",
                    flush=True,
                )
            except Exception as e:
