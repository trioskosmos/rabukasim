...
def run_compare():
    ...
    while not state.is_terminal() and state.turn < max_turns:
        phase_names = ["MullP1", "MullP2", "Active", "Energy", "Draw", "Main", "LiveSet", "PerfP1", "PerfP2", "LiveRes", "Terminal", "Resp", "Setup"]
    ...
    s_orig = state.search_mcts(1000, 0.0, "original", engine_rust.SearchHorizon.GameEnd(), engine_rust.EvalMode.Normal, None)
    
    print("Running MCTS with Legacy Heuristic...")
    s_legacy = state.search_mcts(1000, 0.0, "legacy", engine_rust.SearchHorizon.GameEnd(), engine_rust.EvalMode.Normal, None)
