...
def benchmark():
    ...
    for i in range(games):
        ...
        while not state.is_terminal() and state.turn < 100:
            ...
            suggestions = state.get_mcts_suggestions(sims, 0, engine_rust.SearchHorizon.GameEnd(), engine_rust.EvalMode.TerminalOnly)
            
            policy_target = np.zeros(16384, dtype=np.float32)
            total_visits = sum(s[2] for s in suggestions)
            if total_visits > 0:
                for aid, q, v in suggestions: policy_target[aid] = v / total_visits
    ...
    print(f"Running 15 training epochs...")
    ...
    for epoch in range(15):
        ...
        if epoch == 0: initial_loss = stats['loss']
