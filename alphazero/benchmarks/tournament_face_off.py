...
def get_player_move(player_type, state, db, model, device):
    ...
    if player_type == "Greedy_Heuristic":
        ...
        # Teacher at 0.1s
        res = state.search_mcts(0, 0.1, "original", engine_rust.SearchHorizon.GameEnd(), engine_rust.EvalMode.TerminalOnly, None, None)
        ...
        res = state.search_mcts(0, 0.1, "original", engine_rust.SearchHorizon.GameEnd(), engine_rust.EvalMode.Normal, None, None)
        return res[0][0] if res else random.choice(legal_ids)
...
def run_face_off():
    ...
    print(f"--- 4-WAY FACE OFF TOURNAMENT ---")
