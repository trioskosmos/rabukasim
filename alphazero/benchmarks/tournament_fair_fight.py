...
def get_player_move(player_type, state, db, model, device):
    ...
    if player_type == "Greedy":
        ...
        res = state.search_mcts(
            0, 0.01, "original", engine_rust.SearchHorizon.GameEnd(), engine_rust.EvalMode.TerminalOnly, None, None
        )
        ...
        res = state.search_mcts(
            0, 0.01, "original", engine_rust.SearchHorizon.GameEnd(), engine_rust.EvalMode.Normal, None, None
        )
        num_sims = sum(s[2] for s in res) if res else 0
        move = res[0][0] if res else random.choice(legal_ids)
...
def run_face_off():
    ...
    print("--- THE ALPHAZERO FAIR FIGHT (0.1s Limit) ---")
