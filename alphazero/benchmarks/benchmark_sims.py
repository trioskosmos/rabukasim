...
def play_match(sims_p0, sims_p1, decks, db_engine):
    ...
    while not state.is_terminal() and state.turn < 100:
        ...
        res = state.get_mcts_suggestions(sims, 1.41, engine_rust.SearchHorizon.GameEnd(), engine_rust.EvalMode.TerminalOnly)
...
def analyze_sims():
    ...
    for s0, s1 in matchups:
        ...
        print(f"Avg Time/Game: {total_time/games_per_matchup:.1f}s | Avg Turns/Game: {total_turns/games_per_matchup:.1f}")
