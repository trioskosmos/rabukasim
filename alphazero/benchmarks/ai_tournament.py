    ...
    with open(path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if card_no in card_map:
                ...
                if ctype == "energy":
                    deck_energy.extend([cid] * count)
                elif ctype == "live":
                    deck_lives.extend([cid] * count)
                else:
                    deck_members.extend([cid] * count)
...
def run_single_match(args):
    ...
    state.initialize_game_with_seed(
        d0["members"], d1["members"], d0["energy"], d1["energy"], d0["lives"], d1["lives"], seed
    )
    ...
    if p0_name == "random" or p1_name == "random":
        ...
        while state.phase != 9 and turns < 2000:  # Phase::Terminal = 9
            ...
            if debug:
                ...
                print(
                    f"  Score: P0={len(state.get_player(0).success_lives)}, P1={len(state.get_player(1).success_lives)}"
                )
            ...
            if (acting_p == 0 and p0_name == "random") or (acting_p == 1 and p1_name == "random"):
                ...
                if acting_sims > 0:
                    ...
                    if debug:
                        print("  Greedy Results (All Legal):")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="original",
        choices=["original", "legacy", "simple", "random"],
        help="Heuristic for P0",
    )
    parser.add_argument(
        "--p1_type",
        type=str,
        default="original",
        choices=["original", "legacy", "simple", "random"],
        help="Heuristic for P1",
    )
    parser.add_argument(
        "--decks", type=str, nargs="*", help="Specific deck files to use (recursive search in ai/decks if omitted)"
    )
    ...
    for i, df in enumerate(deck_files):
        ...
        if p and len(p["members"]) >= 40:
            ...
            if i == 0:
                print(
                    f"Sample Deck ({df}): Members={len(p['members'])}, Energy={len(p['energy'])}, Lives={len(p['lives'])}"
                )
    ...
    for i in range(len(parsed_decks)):
        ...
        for g in range(count):
            ...
            tasks.append((args.p0_type, args.p1_type, args.sims, args.sims, p0_h, p1_h, (d, d), seed, args.debug))
