...
# --- Buffer ---
MAX_BUFFER_SIZE = 500000    # AlphaGo Zero scale enabled by disk_buffer.py using bfloat16 compression of observations on disk
...
def get_minimal_state(state):
    ...
    try:
        ...
        def get_id(card):
            ...
            if isinstance(card, dict): return card.get("id", -1)
            return card
...
def play_one_game(d0, d1, sims_per_move, dirichlet_alpha, dirichlet_eps):
    ...
    for t in game_history:
        ...
        tn = t["turn"]
        if tn <= 5: efficiency = 1.0
        else: efficiency = max(0.0, (15 - tn) / 10.0) ** 2
...
def train_fixed_steps(model, buffer, optimizer, scaler, device, num_steps, batch_size):
    ...
    train_fixed_steps._ts = 0; train_fixed_steps._tt = 0
    train_fixed_steps._tf = 0; train_fixed_steps._tb = 0
    ...
    for step in range(num_steps):
        ...
        with torch.amp.autocast('cuda' if device.type == 'cuda' else 'cpu', dtype=amp_dtype):
            ...
            sub_loss = F.kl_div(log_sub.reshape(batch_size, -1), sub_tgt_3d.reshape(batch_size, -1), reduction='batchmean')
            
            # Scale loss for gradient accumulation
            policy_loss = type_loss + sub_loss
            loss = (value_loss + policy_loss + (ENTROPY_LAMBDA * -log_type.exp().mul(log_type).sum(1).mean())) / ACCUM_STEPS
        ...
        time_sample  = getattr(train_fixed_steps, '_ts', 0) + (t1 - t0); train_fixed_steps._ts = time_sample
        time_transfer = getattr(train_fixed_steps, '_tt', 0) + (t2 - t1); train_fixed_steps._tt = time_transfer
        time_forward  = getattr(train_fixed_steps, '_tf', 0) + (t3 - t2); train_fixed_steps._tf = time_forward
        time_backward = getattr(train_fixed_steps, '_tb', 0) + (t4 - t3); train_fixed_steps._tb = time_backward
...
def main():
    ...
    if checkpoint_path.exists():
        ...
        try:
            ...
            if 'model' in ckpt:
                ...
                compatible = {k: v for k, v in ckpt['model'].items() if k in model_state and model_state[k].shape == v.shape}
                model.load_state_dict(compatible, strict=False)
                
                # Restore optimizer/scheduler states
                if 'optimizer' in ckpt:
                    try: optimizer.load_state_dict(ckpt['optimizer'])
                    except: print("  Warning: Optimizer state mismatch, starting fresh.")
                if 'scheduler' in ckpt:
                    try: scheduler.load_state_dict(ckpt['scheduler'])
                    except: print("  Warning: Scheduler state mismatch.")
                if 'scaler' in ckpt:
                ...
                if 'it' in ckpt:
                    ...
                    print(f"Successfully resumed training state (Iter ???)")
            else:
    ...
    master_buffer = PersistentBuffer(
        buffer_dir, 
    ...
    if not log_exists:
        log_file.write("iter,loss,value_loss,policy_loss,accuracy,buffer_size,gen_time,train_time,avg_turns,p0_wins,p1_wins\n")
    ...
    try:
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers, initializer=init_worker, initargs=(str(db_path),)) as executor:
            for it in range(start_it, NUM_ITERATIONS):
                ...
                try:
                    for future in concurrent.futures.as_completed(futures, timeout=600):
                        ...
                        print(f"  [Game {game_num:2d}/{GAMES_PER_ITER}] {winner_str} | "
                              f"Turns: {gstats['turns']} | "
                              f"Lives: {gstats['p0_lives']}-{gstats['p1_lives']}", flush=True)
                except concurrent.futures.TimeoutError:
                    print(f"  [WARN] Game timed out after 600s — skipping remaining futures for this iteration.", flush=True)
                    for f in futures:
                        f.cancel()
                ...
                if not gen_game_stats:
                    ...
                    continue
                if master_buffer.count < BATCH_SIZE: continue
                ...
                stats = train_fixed_steps(
                    model, master_buffer, optimizer, scaler, device,
                ...
                print(f"It {it:3d} | Games: [P0:{p0_wins} P1:{p1_wins} {draw_str}] | AvgT: {avg_turns:.1f} | "
                ...
                log_file.write(f"{it},{stats['loss']},{stats['value']},{stats['policy']},{pol_acc},{master_buffer.count},{gen_time:.1f},{train_time:.1f},{avg_turns:.1f},{p0_wins},{p1_wins}\n")
                log_file.flush()
