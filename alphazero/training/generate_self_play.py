...
def load_tournament_decks(db_json):
    ...
    for deck_file in decks_dir.glob("*.txt"):
        ...
        if len(members) >= 48 and len(lives) >= 12:
            ...
            loaded_decks.append(
                {"name": deck_file.stem, "members": final_members, "lives": final_lives, "energy": final_energy}
            )
        else:
...
def generate_trajectories(num_games=10, sims_per_move=100, output_file="trajectories.json"):
    ...
    for g_idx in tqdm(range(num_games), desc="Self-Play Games"):
        ...
        while not state.is_terminal() and state.turn < 100:
            ...
            suggestions = state.get_mcts_suggestions(
                sims_per_move, 1.41, engine_rust.SearchHorizon.GameEnd(), engine_rust.EvalMode.Normal
            )
            ...
            if log_file:
                ...
                stats = (
                    f"S:{p0.score}, L:{len(p0.success_lives)}"
                    if state.current_player == 0
                    else f"S:{p1.score}, L:{len(p1.success_lives)}"
                )
    ...
    np.savez_compressed(output_path, obs=obs_batch, policy=policy_batch, mask=mask_batch, value=value_batch)
