        ...
        if hand_idx < len(hand):
            ...
            if initial_deck and card_id in initial_deck:
                try:
                    ...
                    if deck_idx < 60:
                        if 1 <= slot <= 3: return 68 + (slot - 1) * 60 + deck_idx
                        return 8 + deck_idx
    ...
    if 400 <= engine_id < 500:
        ...
        if hand_idx < len(hand):
            ...
            if initial_deck and card_id in initial_deck:
                try:
                    deck_idx = initial_deck.index(card_id)
                    if deck_idx < 60: return 8 + deck_idx
                except ValueError: pass
        return -1
    if 600 <= engine_id <= 602: return 248 + (engine_id - 600)
    if 8200 <= engine_id <= 8204: return 251 + (engine_id - 8200)
    if 20000 <= engine_id <= 20002: return 240 + (engine_id - 20000)
    if 21000 <= engine_id <= 21002: return 240 + (engine_id - 21000)
    return -1
...
def action_256_to_engine_action(state, player_idx, action_idx, phase, initial_deck):
    ...
    if 1 <= action_idx <= 6: return 300 + (action_idx - 1)
    if action_idx == 7: return 11000 if phase == 4 else 0
    if 248 <= action_idx <= 250: return 600 + (action_idx - 248)
    if 251 <= action_idx <= 255: return 8200 + (action_idx - 251)
    if 8 <= action_idx < 68:
        ...
        if deck_idx < len(initial_deck):
            ...
            if cid in hand:
                ...
                if phase == 4: return 1000 + h_idx * 10 + 1
                if phase == 5: return 400 + h_idx
    if 68 <= action_idx < 248:
        ...
        if deck_idx < len(initial_deck):
            ...
            if cid in hand:
                h_idx = hand.index(cid)
                if phase == 4: return 1000 + h_idx * 10 + (slot_idx + 1)
                if phase == 5: return 400 + h_idx
    return None
                ...
                if db_json:
                    ...
                    if field_costs:
                        ...
                        for f_energy in field_costs:
                            if energy > f_energy: bonus += 10
                            elif energy == f_energy: bonus += 3
                            else: bonus += 5
        ...
        if self.temperature > 0.01:
            if self.use_noise and self.dirichlet_epsilon > 0:
                ...
                if len(legal_vids) > 0:
                    ...
                    policy_248_masked[legal_vids] = (1 - self.dirichlet_epsilon) * policy_248_masked[legal_vids] + self.dirichlet_epsilon * noise
...
def run_benchmark(model_path=None, sims=50, model=None, db=None):
    ...
    if db is None:
        ...
        with open(db_path, "r", encoding="utf-8") as f: full_db = json.load(f)
        stripped = json.loads(json.dumps(full_db))
        for cat in ["member_db", "live_db"]:
            for cid, data in stripped.get(cat, {}).items():
                data["abilities"] = []; data["ability_flags"] = 0
                if "synergy_flags" in data: data["synergy_flags"] &= 1
    ...
    for deck in all_decks:
        for seed in FIXED_SEEDS:
            ...
            while not state.is_terminal() and state.turn < 25 and moves < 500:
                ...
                if not legal: state.auto_step(db); legal = state.get_legal_action_ids(); 
                if not legal: break
                ...
                if cp == -4: 
                    if 0 in legal: state.step(0); state.auto_step(db); moves += 1; continue
                if cp in [-3, -2]:
                    state.step(random.choice(legal)); state.auto_step(db); moves += 1; continue
                ...
                if action == -1:
                    ...
                    for aid in legal:
                        ...
                        if 0 <= vid < ACTION_SPACE: mask[0, vid] = True; v_to_e[vid] = aid
                ...
                state.step(action); state.auto_step(db); moves += 1
