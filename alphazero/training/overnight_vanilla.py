...
def init_worker(db_json_str, model_path_str=None, device_str="cpu"):
    ...
    if model_global:
        evaluator_global = engine_rust.PyAlphaZeroEvaluator(model_global, engine_rust.AlphaZeroTensorType.Vanilla)
    else:
        evaluator_global = None
...
def play_one_game(d0, d1, sims_per_move, mirror_seed=None, iteration=0):
    ...
    try:
        ...
        while not state.is_terminal() and state.turn < 40 and moves_taken < max_moves:
            ...
            if use_dfs:
                ...
                if sequence:
                    ...
                    for aid in legal_ids:
                        vid = map_engine_to_vanilla(
                            curr_p_data, aid, initial_decks[state.current_player], current_phase
                        )
                    ...
                    if evals:
                        ...
                        if valid_evs:
                            ...
                            target_vid = map_engine_to_vanilla(
                                curr_p_data, seq_action, initial_decks[state.current_player], current_phase
                            )
            ...
            if total_visits > 0:
                for engine_id, q, visits in suggestions:
                    vid = map_engine_to_vanilla(
                        curr_p_data, engine_id, initial_decks[state.current_player], current_phase
                    )
                    if 0 <= vid < ACTION_SPACE:
            ...
            if model_global:
                with torch.no_grad():
                    ...
                    if np.isnan(probs).any():
                        print("CRITICAL: NaNs in policy probabilities! Resetting to uniform mask.")
                    ...
                    try:
                        ...
                        action_engine = random.choice(legal_ids)
                        action_vid = map_engine_to_vanilla(
                            state_json["players"][state.current_player],
                            action_engine,
                            initial_decks[state.current_player],
                        )
                    else:
                ...
                if p_sum > 1e-11:
                    ...
                    print(
                        f"DEBUG: MCTS visits yielded zero-sum policy. total_visits={total_visits}, suggestions={len(suggestions)}"
                    )
                    action_engine = random.choice(legal_ids)
                    action_vid = map_engine_to_vanilla(
                        state_json["players"][state.current_player], action_engine, initial_decks[state.current_player]
                    )
            else:
                action_engine = random.choice(legal_ids)
                action_vid = map_engine_to_vanilla(
        ...
        if exit_reason != "Terminal" and state.turn == 0:
            print(
                f"[DEBUG] 0-Turn Game! Reason: {exit_reason}, Phase: {state.phase}, Legal: {len(state.get_legal_action_ids())}"
            )

        # Debug: Check final state
        print(
            f"[DEBUG] Game ending - is_terminal: {state.is_terminal()}, turn: {state.turn}, winner: {state.get_winner()}"
        )
        ...
        for h in game_history:
            ...
            new_transitions.append(
                (h["obs"], _dense_to_sparse(h["policy"]), h["mask"], np.array([float(val)], dtype=np.float32))
            )
...
def train_fixed_steps(model, buffer, optimizer, scaler, device, num_steps, batch_size):
    ...
    for _ in range(num_steps):
        ...
        pol_t[torch.from_numpy(row_v).long().to(device), torch.from_numpy(col_v).long().to(device)] = (
            torch.from_numpy(val_v).float().to(device)
        )
    ...
    try:
        with concurrent.futures.ProcessPoolExecutor(
            ...
            initargs=(
                ...
                else (str(checkpoint_path) if checkpoint_path.exists() else None),
                "cpu",
            ),
        ) as executor:
            for it in range(start_it, NUM_ITERATIONS):
                ...
                for future in concurrent.futures.as_completed(futures):
                    ...
                    print(
                        f"  [Game done] Winner: {stat['winner']}, Turns: {stat['turns']}, P0 Lives: {stat['p0_lives']}, P1 Lives: {stat['p1_lives']}"
                    )
                ...
                print(
                    f"[SUMMARY] It {it:3d} | Loss: {loss:.4f} | Gen: {gen_time:.1f}s | P0 Wins: {wins.count(0)} | P1 Wins: {wins.count(1)} | Ties: {ties} | Avg Lives - P0: {p0_lives / len(game_stats):.1f} P1: {p1_lives / len(game_stats):.1f} | Avg Turns: {avg_turns:.1f}"
                )
                ...
                log_file.write(
                    f"{it},{loss:.4f},{avg_turns:.1f},{wins.count(0)},{wins.count(1)},{buffer.count},{bench_turns:.1f},{bench_score:.1f},{gen_time:.1f},{train_time:.1f}\n"
                )
                log_file.flush()
