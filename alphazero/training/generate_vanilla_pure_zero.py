...
def generate_vanilla_trajectories(
    num_games=10, sims_per_move=128, output_file="vanilla_trajectories.npz", mirror=False, verbose=False
):
    ...
    for g_idx in tqdm(range(num_games), desc="Vanilla Self-Play", disable=verbose):
        ...
        if mirror:
            ...
            state.initialize_game_with_seed(
                d0["members"] + d0["lives"], d1["members"] + d1["lives"], d0["energy"], d1["energy"], [], [], seed
            )
        else:
        ...
        while not state.is_terminal() and state.turn < 100:
            ...
            suggestions = state.get_mcts_suggestions(
                sims_per_move, 1.41, engine_rust.SearchHorizon.GameEnd(), engine_rust.EvalMode.TerminalOnly
            )
            ...
            game_history.append(
                {"obs": obs, "policy": policy_target.copy(), "player": state.current_player, "mask": mask}
            )
        ...
        if verbose:
            ...
            print(
                f"Game {g_idx + 1:3d} | Winner: P{winner} | Final Turn: {state.turn:2d} | Final Scores: P0:{p0.score} P1:{p1.score}",
                flush=True,
            )
