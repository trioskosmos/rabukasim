...
untested_specific.sort(key=lambda x: int(x['id'][1:]), reverse=True)

untested_generic = [qa for qa in untested if not qa.get('related_cards')]
untested_generic.sort(key=lambda x: int(x['id'][1:]), reverse=True)
        ...
        if cards:
            ...
            for c in cards:
                ...
                if 'ability_text' in card_data:
                    f.write(f"  - **Ability:** {''.join(card_data['ability_text']).strip()}\n")
            f.write("\n")
...
print(f"Generated detailed plans for {len(untested_sorted)} untested QAs in qa_test_plans.md")
