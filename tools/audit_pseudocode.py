...
def audit_pseudocode():
    ...
    for card_no, data in cards_data.items():
        ...
        if group_id == 99:
            if card_no.startswith("PL!-"): group_id = 0
            elif card_no.startswith("PL!S-"): group_id = 1
            elif card_no.startswith("PL!N-"): group_id = 2 # N is usually Nijigasaki
            elif card_no.startswith("PL!SP-"): group_id = 3 # SP is Liella
            elif card_no.startswith("PL!HS-"): group_id = 4 # HS is Hasuno
    ...
    for card_no, p_entry in pseudo_data.items():
        ...
        for m in matches:
            ...
            if card_no.startswith("PL!N-") and found_id in (3, 4):
                 errors.append(f"[GROUP_MISMATCH] {card_no}: Pseudocode has GROUP_ID={found_id}, but card is likely Nijigasaki (ID 2).")
            elif card_no.startswith("PL!SP-") and found_id != 3:
                 errors.append(f"[GROUP_MISMATCH] {card_no}: Pseudocode has GROUP_ID={found_id}, but card is Liella (ID 3).")
