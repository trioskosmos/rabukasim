            ...
            for live in lives:
                ...
                if passed:
                    ...
                    if winner >= 0 and pid != winner:
                        ...
                        output.append(f"      [EXPLANATION] TIE: Checking Rule 8.4.7.1 (Catch-up).")
                    elif len(lives) > 1 and result == "DISCARDED":
                        output.append(f"      [EXPLANATION] SELECTION: Player had multiple passed lives, only one moved.")
                    elif result == "DISCARDED":
                        output.append(f"      [EXPLANATION] UNKNOWN: Passed alone but discarded. (Hearts re-check failure?)")
