...
class PseudocodeNormalizer:
    ...
    INTERNAL_KEYS = {
        "raw_effect", "raw_cond", "raw_val", "target_name", "val", "type",
        "options_text", "multiplier_value", "cost_type_name", "opponent_trigger_allowed",
        "has_multiplier", "raw_instructions"
    }
                ...
                for instr in opt_list:
                    ...
                    if hasattr(instr, 'effect_type'): # It's an Effect object
                        ...
                        if "effect_type" in instr:
                            formatted_opts.append(f"{EFFECT_DISPLAY.get(instr['effect_type'], 'UNK')}({instr.get('value')})")
                        elif "type" in instr:
...
def main():
    ...
    cards = {}
    if "member_db" in data: cards.update(data["member_db"])
    if "live_db" in data: cards.update(data["live_db"])
