...
def run_repro():
    print("=" * 60)
    print("REPRODUCTION: PL!S-bp2-001-P (高海千歌)")
    print("=" * 60)
    ...
    p0_lives = []
    p1_lives = [l for l in l_db.keys()][:1]

    print(f"Initializing game with Plinth CID: {plinth_cid}")
    print(f"Lives: {p0_lives}")
    ...
    print(f"  Blade Buffs: {p0.blade_buffs}")
    eff_blades = gs.get_effective_blades(0, 0)
    print(f"  Effective Blades (Slot 0): {eff_blades}")
    print("  Rule Log:")
    for line in gs.rule_log[-5:]:
        print(f"    {line}")
