...
TEST_FN_PATTERN = re.compile(
    r'#\[test\]\s*\n\s*fn (test_opcode_\w+)\s*\([^)]*\)\s*\{'
)
BYTECODE_PATTERN = re.compile(
    r'let\s+bc\s*=\s*vec!\[([^\]]+)\]'
)
BYTECODE_ALT_PATTERN = re.compile(
    r'vec!\[([O_\w]+,\s*\d+,\s*\d+,\s*\d+,[^\]]+)\]'
)
OPCODE_PATTERN = re.compile(r'O_[A-Z_]+')
RESOLVE_BYTECODE_PATTERN = re.compile(
    r'state\.resolve_bytecode\s*\(\s*&db\s*,\s*&bc\s*,\s*&ctx\s*\)'
)
CREATE_TEST_DB_PATTERN = re.compile(r'create_test_db\(\)')
LOAD_REAL_DB_PATTERN = re.compile(r'load_real_db\(\)')
SUSPEND_INTERACTION_PATTERN = re.compile(
    r'suspend_interaction|interaction_stack|choice_index'
)
...
def generate_parity_test_code(test: ExtractedTest, card_id: int) -> str:
    ...
    if test.has_interaction:
        ...
        lines.append(f"    // TODO: Implement choice propagation")
        return '\n'.join(lines)
    
    # Skip tests using real DB for now
    if test.uses_real_db:
        lines.append(f"    // {test.name}: SKIPPED (uses real DB)")
        lines.append(f"    // TODO: Map card IDs for production test")
        return '\n'.join(lines)
    ...
    lines.append(f'    add_card(&mut unit_db, {card_id}, "{test.name}", vec![], vec![(TriggerType::OnPlay, bc_{card_id}, vec![])]);')
...
def generate_test_runner(test: ExtractedTest, card_id: int, test_num: int) -> str:
    ...
    lines.append(f"    let mut state = create_test_state();")
    lines.append(f"    state.core.players[0].hand = vec![{card_id}].into();")
    lines.append(f"    state.core.players[0].energy_zone = (10..30).collect::<Vec<i32>>().into();")
    lines.append(f'    if !run_parity_check(&unit_manager, &unit_db, &state, Action::PlayMember {{ hand_idx: 0, slot_idx: 0 }}.id(), "U{test_num} {test.name}") {{ mismatch_count += 1; }}')
