...
def sync():
    ...
    with open(rust_const_path, "w", encoding="utf-8") as f:
        ...
        for key, val in metadata.get("extra_constants", {}).items():
            const_type = "i32"
            if key.startswith("FLAG_") or key.startswith("FILTER_"):
                const_type = "u64"
    ...
    with open(rust_enum_path, "w", encoding="utf-8") as f:
        ...
        f.write("        match self {\n")
        f.write("            Phase::Main | Phase::LiveSet | Phase::MulliganP1 | Phase::MulliganP2 | Phase::Response | Phase::LiveResult | Phase::Energy => true,\n")
        f.write("            _ => false,\n")
        f.write("        }\n")
        f.write("    }\n")
        f.write("}\n")
    ...
    wgsl_implemented_opcodes = {
        ...
        # Basic game actions
        "DRAW", "ADD_BLADES", "ADD_HEARTS", "REDUCE_COST", "BOOST_SCORE",
        "ENERGY_CHARGE", "SELECT_MODE", "MOVE_TO_DECK", "TAP_OPPONENT", 
        "COLOR_SELECT", "MOVE_TO_DISCARD", "LOOK_AND_CHOOSE", "ACTIVATE_ENERGY",
        # State manipulation
        "RESTRICTION", "BATON_TOUCH_MOD", "BUFF_POWER",
        "IMMUNITY", "SET_BLADES", "SET_HEARTS", "MOVE_MEMBER", "SWAP_CARDS",
        "SEARCH_DECK", "LOOK_DECK", "REVEAL_UNTIL", "RECOVER_LIVE", "RECOVER_MEMBER",
        "DRAW_UNTIL", "PAY_ENERGY", "SET_SCORE", "ACTIVATE_MEMBER", "TAP_MEMBER",
        # Heart and cost manipulation
        "SET_TAPPED", "REDUCE_HEART_REQ", "INCREASE_HEART_COST", "SET_HEART_COST",
        "INCREASE_COST", "REDUCE_YELL_COUNT", "REDUCE_LIVE_SET_LIMIT",
        "PREVENT_ACTIVATE", "PREVENT_SET_TO_SUCCESS_PILE", "PREVENT_BATON_TOUCH",
        # Card movement and play
        "PLAY_MEMBER_FROM_HAND", "PLAY_MEMBER_FROM_DISCARD", "GRANT_ABILITY",
        "TRANSFORM_HEART", "PLAY_LIVE_FROM_DISCARD", "TRANSFORM_COLOR",
        "ADD_TO_HAND", "SELECT_MEMBER", "SELECT_LIVE", "SELECT_PLAYER", "SELECT_CARDS",
        "SWAP_AREA", "SWAP_ZONE",
        # Additional opcodes (marked as unused but included for completeness)
        "NOP", "FLAVOR", "FORMATION_CHANGE", "ADD_CONTINUOUS",
        # Remaining opcodes (to be implemented in WGSL)
        "NEGATE_EFFECT", "ORDER_DECK", "META_RULE", "PLACE_UNDER",
        "REVEAL_CARDS", "CHEER_REVEAL", "TRIGGER_REMOTE", "MODIFY_SCORE_RULE",
        "ADD_STAGE_ENERGY", "PREVENT_PLAY_TO_SLOT", "OPPONENT_CHOOSE",
    ...
    wgsl_implemented_conditions = {
        ...
        "LIFE_LEAD", "COUNT_GROUP", "COUNT_ENERGY", "COST_CHECK",
        "SCORE_COMPARE", "COUNT_HEARTS", "COUNT_BLADES", "BATON",
        # Additional conditions from shader_rules.wgsl
        "GROUP_FILTER", "OPPONENT_HAS", "SELF_IS_GROUP", "RARITY_CHECK",
        "COUNT_SUCCESS_LIVE", "TYPE_CHECK", "IS_IN_DISCARD",
        # Additional conditions (to be implemented or marked as implemented)
        "COUNT_DISCARD", "IS_CENTER", "MODAL_ANSWER", "HAS_LIVE_CARD",
        "OPPONENT_HAND_DIFF", "HAS_KEYWORD", "DECK_REFRESHED", "COUNT_LIVE_ZONE",
        "AREA_CHECK", "OPPONENT_ENERGY_DIFF", "HAS_MOVED", "HAND_INCREASED",
        "OPPONENT_CHOICE", "HAS_CHOICE", "COST_LEAD", "SCORE_LEAD", "HEART_LEAD",
        "HAS_EXCESS_HEART", "NOT_HAS_EXCESS_HEART", "TOTAL_BLADES", "COST_COMPARE",
        "BLADE_COMPARE", "HEART_COMPARE", "OPPONENT_HAS_WAIT",
        # Final conditions for 100% coverage
        "HAND_HAS_NO_LIVE", "IS_TAPPED", "IS_ACTIVE", "LIVE_PERFORMED",
        "IS_PLAYER", "IS_OPPONENT"
    }
    ...
    with open(wgsl_path, "w", encoding="utf-8") as f:
        ...
        f.write("\n// --- CONDITION TYPES ---\n")
        for key, val in metadata["conditions"].items():
            impl_status = " // IMPLEMENTED" if key in wgsl_implemented_conditions else " // TODO: Not implemented in WGSL"
            if key in unused_list:
                impl_status = " // [UNUSED]"
    ...
    print(f"WGSL Parity: {len(wgsl_implemented_opcodes)}/{len(metadata['opcodes'])} opcodes, {len(wgsl_implemented_conditions)}/{len(metadata['conditions'])} conditions")
