import datetime
import json
import os
import hashlib


def calculate_hash(path):
    if not os.path.exists(path):
        return None
    with open(path, "rb") as f:
        return hashlib.sha256(f.read()).hexdigest()


def calculate_sync_state_hash(*paths):
    digest = hashlib.sha256()
    for path in paths:
        file_hash = calculate_hash(path)
        if file_hash is None:
            continue
        digest.update(file_hash.encode("ascii"))
    return digest.hexdigest()


def to_pascal_case(snake_str):
    if not snake_str:
        return ""
    # Handle acronyms or special cases if any
    parts = snake_str.lower().split("_")
    return "".join(x.capitalize() for x in parts)


PYTHON_ENUM_COMPAT_ALIASES = {
    "ConditionType": [
        ("UNIQUE_NAMES_COUNT", "COUNT_UNIQUE_COLORS"),
        ("ALL_CARDS_MATCH", "DISCARDED_CARDS"),
    ],
    "AbilityCostType": [
        ("SELECT_CARDS", 10),
        ("SELECT_MEMBER", 11),
    ],
}


def write_python_enum_aliases(handle, enum_name):
    aliases = PYTHON_ENUM_COMPAT_ALIASES.get(enum_name, [])
    if not aliases:
        return

    for alias_name, alias_target in aliases:
        handle.write(f"    {alias_name} = {alias_target}\n")


def resolve_extra_constants(metadata):
    extra_constants = dict(metadata.get("extra_constants", {}))
    packed_layout = metadata.get("packed_layout", {})
    layout_fields = packed_layout.get("A", {}).get("standard", {})

    shift_mappings = {
        "FILTER_TYPE_SHIFT": "card_type",
        "FILTER_GROUP_SHIFT": "group_id",
        "FILTER_UNIT_SHIFT": "unit_id",
        "FILTER_COST_SHIFT": "value_threshold",
        "FILTER_COLOR_SHIFT": "color_mask",
        "FILTER_SPECIAL_SHIFT": "special_id",
        "FILTER_ZONE_MASK_SHIFT": "zone_mask",
    }
    for constant_name, field_name in shift_mappings.items():
        if field_name in layout_fields:
            extra_constants[constant_name] = layout_fields[field_name][0]

    bit_mappings = {
        "FILTER_SETSUNA": "is_setsuna",
        "DYNAMIC_VALUE": "compare_accumulated",
        "FILTER_IS_OPTIONAL": "is_optional",
        "KEYWORD_ACTIVATED_ENERGY_BY_GROUP": "keyword_energy",
        "KEYWORD_ACTIVATED_MEMBER_BY_GROUP": "keyword_member",
    }
    for constant_name, field_name in bit_mappings.items():
        if field_name in layout_fields:
            extra_constants[constant_name] = 1 << layout_fields[field_name][0]

    return extra_constants


def analyze_layout_fields(fields, layout_name):
    groups = []

    for field_name, bits in fields.items():
        start, end = bits
        if end < start:
            raise ValueError(f"Invalid bit range for {layout_name}.{field_name}: [{start}, {end}]")

        matched_group = None
        for group in groups:
            if start == group["start"] and end == group["end"]:
                matched_group = group
                break

            overlaps = not (end < group["start"] or start > group["end"])
            if overlaps:
                raise ValueError(
                    "Overlapping packed layout fields detected: "
                    f"{layout_name}.{field_name}[{start},{end}] overlaps with "
                    f"{layout_name}.{group['canonical']}[{group['start']},{group['end']}]"
                )

        if matched_group is None:
            mask = (1 << (end - start + 1)) - 1
            groups.append(
                {
                    "canonical": field_name,
                    "aliases": [],
                    "start": start,
                    "end": end,
                    "mask": mask,
                }
            )
        else:
            matched_group["aliases"].append(field_name)

    return groups


def sync():
    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
    metadata_path = os.path.join(project_root, "data", "metadata.json")
    if not os.path.exists(metadata_path):
        print(f"Error: {metadata_path} not found")
        return

    # --- Parity Check ---
    hash_file = os.path.join(project_root, "data", ".metadata_sync_hash")
    current_hash = calculate_sync_state_hash(metadata_path, __file__)
    
    if os.path.exists(hash_file):
        with open(hash_file, "r") as f:
            stored_hash = f.read().strip()
        if stored_hash == current_hash:
            # Check if one of the critical generated files exists. If not, we must sync anyway.
            if os.path.exists(os.path.join(project_root, "engine_rust_src/src/core/generated_constants.rs")):
                print("Metadata is up to date (O(1) match). Skipping sync.")
                return

    with open(metadata_path, "r", encoding="utf-8") as f:
        metadata = json.load(f)

    unused_list = metadata.get("unused", [])
    resolved_extra_constants = resolve_extra_constants(metadata)

    # --- Generate Rust Constants (Generated Constants file) ---
    # This file remains the shared constant surface for packed-word helpers.
    rust_const_path = "engine_rust_src/src/core/generated_constants.rs"
    with open(rust_const_path, "w", encoding="utf-8") as f:
        f.write(f"//! @generated by tools/sync_metadata.py on {datetime.datetime.now().isoformat()}\n")
        f.write("#![allow(dead_code)]\n\n")

        f.write("// Opcodes\n")
        for key, val in metadata["opcodes"].items():
            comment = " // [UNUSED]" if key in unused_list else ""
            f.write(f"pub const O_{key}: i32 = {val};{comment}\n")

        f.write("\n// Action ID Bases\n")
        for key, val in metadata["action_bases"].items():
            f.write(f"pub const ACTION_BASE_{key}: i32 = {val};\n")

        f.write("\n// Condition Types\n")
        for key, val in metadata["conditions"].items():
            comment = " // [UNUSED]" if key in unused_list else ""
            f.write(f"pub const C_{key}: i32 = {val};{comment}\n")

        f.write("\n// Cost Types\n")
        for key, val in metadata["costs"].items():
            f.write(f"pub const COST_{key}: i32 = {val};\n")

        f.write("\n// Choice Types\n")
        for key, val in metadata.get("choices", {}).items():
            f.write(f"pub const CHOICE_{key}: i32 = {val};\n")

        f.write("\n// Extra Constants\n")
        for key, val in resolved_extra_constants.items():
            const_type = "i32"
            if (
                key.startswith("FLAG_")
                or key.startswith("FILTER_")
                or key.startswith("KEYWORD_")
                or key == "DYNAMIC_VALUE"
            ):
                const_type = "u64"
            elif key.startswith("CHOICE_FLAG_"):
                const_type = "u8"
            elif key.startswith("SYN_FLAG_") or key.startswith("COST_FLAG_"):
                const_type = "u32"

            # Write with literal suffix for u64/u32 to avoid warnings/errors
            suffix = "u64" if const_type == "u64" else "u32" if const_type == "u32" else ""
            f.write(f"pub const {key}: {const_type} = {val}{suffix};\n")

        f.write("\n// Zone Types\n")
        for key, val in metadata.get("zones", {}).items():
            f.write(f"pub const ZONE_{key}: i32 = {val};\n")

        f.write("\n// Comparisons\n")
        for key, val in metadata.get("comparisons", {}).items():
            f.write(f"pub const COMP_{key}: i32 = {val};\n")

        f.write("\n// Heart Color Map\n")
        for key, val in metadata.get("heart_color_map", {}).items():
            f.write(f"pub const HEART_COLOR_{key}: i32 = {val};\n")

        f.write("\n// Meta Rule Types\n")
        for key, val in metadata.get("meta_rule_types", {}).items():
            f.write(f"pub const META_RULE_{key}: i32 = {val};\n")

        f.write("\n// Multiplier Count Sources\n")
        for key, val in metadata.get("multiplier_count_sources", {}).items():
            f.write(f"pub const COUNT_SOURCE_{key}: i32 = {val};\n")

        f.write("\n// Card Types\n")
        for key, val in metadata.get("card_types", {}).items():
            f.write(f"pub const CARD_TYPE_{key}: i32 = {val};\n")

        f.write("\n// Slot Indices\n")
        for key, val in metadata.get("slot_indices", {}).items():
            f.write(f"pub const SLOT_{key}: i32 = {val};\n")

        f.write("\n// Choice Indices\n")
        for key, val in metadata.get("choice_indices", {}).items():
            f.write(f"pub const CHOICE_IDX_{key}: i32 = {val};\n")

        f.write("\n// Target Players\n")
        for key, val in metadata.get("target_players", {}).items():
            f.write(f"pub const TARGET_PLAYER_{key}: i32 = {val};\n")

        f.write("\n// Character IDs\n")
        for key, val in metadata.get("character_ids", {}).items():
            f.write(f"pub const CHAR_{key}: i32 = {val};\n")

        f.write("\n// Group IDs\n")
        for key, val in metadata.get("group_ids", {}).items():
            f.write(f"pub const GROUP_{key}: i32 = {val};\n")

        f.write("\n// Unit IDs\n")
        for key, val in metadata.get("unit_ids", {}).items():
            f.write(f"pub const UNIT_{key}: i32 = {val};\n")

    # --- Generate Rust Enums (FULL OVERWRITE of enums.rs) ---
    rust_enum_path = "engine_rust_src/src/core/enums.rs"
    with open(rust_enum_path, "w", encoding="utf-8") as f:
        f.write(f"//! @generated by tools/sync_metadata.py on {datetime.datetime.now().isoformat()}\n")
        f.write("#![allow(unused_imports)]\n")
        f.write("use serde::{Deserialize, Serialize};\n")
        f.write("use serde_repr::{Deserialize_repr, Serialize_repr};\n")
        f.write("pub use crate::core::generated_constants::*;\n\n")

        # TriggerType
        f.write("#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize_repr, Deserialize_repr, Default)]\n")
        f.write("#[repr(u8)]\n")
        f.write("pub enum TriggerType {\n")
        f.write("    #[default]\n")
        for key, val in metadata["triggers"].items():
            variant = to_pascal_case(key) if key != "NONE" else "None"
            comment = " /// [UNUSED]" if key in unused_list else ""
            f.write(f"    {variant} = {val},{comment}\n")
        f.write("}\n\n")

        # TargetType
        f.write("#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize_repr, Deserialize_repr, Default)]\n")
        f.write("#[repr(u8)]\n")
        f.write("pub enum TargetType {\n")
        f.write("    #[default]\n")
        for key, val in metadata["targets"].items():
            variant = to_pascal_case(key) if key != "SELF" else "Self_"  # 'Self' is reserved
            f.write(f"    {variant} = {val},\n")
        f.write("}\n\n")

        # EffectType
        f.write("#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize_repr, Deserialize_repr, Default)]\n")
        f.write("#[repr(u8)]\n")
        f.write("pub enum EffectType {\n")
        f.write("    #[default]\n")
        # We need a default. Usually NOP=0, but EffectType start at 10.
        # Let's use first one or NOP if exists.
        if "NOP" in metadata["opcodes"]:
            f.write("    Nop = 0,\n")
        else:
            # Fallback to a safe default if Nop is missing
            first_op = list(metadata["opcodes"].keys())[0]
            f.write(f"    {to_pascal_case(first_op)} = {metadata['opcodes'][first_op]},\n")

        for key, val in metadata["opcodes"].items():
            if key == "NOP":
                continue
            comment = " /// [UNUSED]" if key in unused_list else ""
            f.write(f"    {to_pascal_case(key)} = {val},{comment}\n")
        f.write("}\n\n")

        # ConditionType
        # Check if any condition values exceed u8 range
        max_condition_val = max(metadata["conditions"].values()) if metadata["conditions"] else 0
        condition_repr = "u16" if max_condition_val > 255 else "u8"
        f.write("#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize_repr, Deserialize_repr, Default)]\n")
        f.write(f"#[repr({condition_repr})]\n")
        f.write("pub enum ConditionType {\n")
        f.write("    #[default]\n")
        f.write("    None = 0,\n")
        for key, val in metadata["conditions"].items():
            comment = " /// [UNUSED]" if key in unused_list else ""
            f.write(f"    {to_pascal_case(key)} = {val},{comment}\n")
        f.write("}\n\n")

        # AbilityCostType
        f.write("#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize_repr, Deserialize_repr, Default)]\n")
        f.write("#[repr(u8)]\n")
        f.write("pub enum AbilityCostType {\n")
        f.write("    #[default]\n")
        f.write("    None = 0,\n")
        for key, val in metadata["costs"].items():
            if key == "NONE":
                continue
            f.write(f"    {to_pascal_case(key)} = {val},\n")
        f.write("}\n\n")

        # ChoiceType
        if "choices" in metadata:
            f.write("#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize_repr, Deserialize_repr, Default)]\n")
            f.write("#[repr(u8)]\n")
            f.write("pub enum ChoiceType {\n")
            f.write("    #[default]\n")
            f.write("    None = 0,\n")
            for key, val in metadata["choices"].items():
                if key == "NONE":
                    continue
                f.write(f"    {to_pascal_case(key)} = {val},\n")
            f.write("}\n\n")

            f.write("impl ChoiceType {\n")
            f.write("    pub fn from_str(s: &str) -> Self {\n")
            f.write("        match s {\n")
            for key, val in metadata["choices"].items():
                if key == "NONE":
                    continue
                f.write(f'            "{key}" => ChoiceType::{to_pascal_case(key)},\n')
            f.write("            _ => ChoiceType::None,\n")
            f.write("        }\n")
            f.write("    }\n")
            f.write("    pub fn as_str(&self) -> &'static str {\n")
            f.write("        match self {\n")
            for key, val in metadata["choices"].items():
                if key == "NONE":
                    continue
                f.write(f'            ChoiceType::{to_pascal_case(key)} => "{key}",\n')
            f.write('            ChoiceType::None => "NONE",\n')
            f.write("        }\n")
            f.write("    }\n")
            f.write("}\n\n")
            f.write("impl std::fmt::Display for ChoiceType {\n")
            f.write("    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {\n")
            f.write('        write!(f, "{}", self.as_str())\n')
            f.write("    }\n")
            f.write("}\n\n")

        # Phase
        f.write("#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize_repr, Deserialize_repr, Default)]\n")
        f.write("#[repr(i8)]\n")
        f.write("pub enum Phase {\n")
        f.write("    #[default]\n")
        # Phases are tricky because they include negative values.
        # Sorted by value
        sorted_phases = sorted(metadata["phases"].items(), key=lambda x: x[1])
        for key, val in sorted_phases:
            f.write(f"    {to_pascal_case(key)} = {val},\n")
        f.write("}\n\n")

        # Add is_interactive impl
        f.write("impl Phase {\n")
        f.write("    pub fn is_interactive(&self) -> bool {\n")
        f.write("        match self {\n")
        f.write(
            "            Phase::Main | Phase::LiveSet | Phase::MulliganP1 | Phase::MulliganP2 | Phase::Response | Phase::LiveResult | Phase::Energy => true,\n"
        )
        f.write("            _ => false,\n")
        f.write("        }\n")
        f.write("    }\n")
        f.write("}\n\n")

        # Zone
        if "zones" in metadata:
            f.write("#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize_repr, Deserialize_repr, Default)]\n")
            f.write("#[repr(u8)]\n")
            f.write("pub enum Zone {\n")
            f.write("    #[default]\n")
            f.write("    Default = 0,\n")
            for key, val in metadata["zones"].items():
                if key == "DEFAULT":
                    continue
                f.write(f"    {to_pascal_case(key)} = {val},\n")
            f.write("}\n\n")

        # Display Name Helpers
        for meta_key, func_name in [("group_names", "get_group_name"), ("unit_names", "get_unit_name")]:
            f.write(f"pub fn {func_name}(id: u8, lang: &str) -> &'static str {{\n")
            f.write("    match lang {\n")
            f.write('        "jp" => match id {\n')
            for id_str, names in metadata.get(meta_key, {}).items():
                f.write(f'            {id_str} => "{names["jp"]}",\n')
            f.write('            _ => "-",\n')
            f.write("        },\n")
            f.write("        _ => match id {\n")  # Default to EN
            for id_str, names in metadata.get(meta_key, {}).items():
                f.write(f'            {id_str} => "{names["en"]}",\n')
            f.write('            _ => "-",\n')
            f.write("        }\n")
            f.write("    }\n")
            f.write("}\n\n")

    # --- Generate JS Constants ---
    js_path = "frontend/web_ui/js/generated_constants.js"
    with open(js_path, "w", encoding="utf-8") as f:
        f.write(f"/** @generated by tools/sync_metadata.py on {datetime.datetime.now().isoformat()} */\n\n")
        f.write("export const Opcodes = {\n")
        for key, val in metadata["opcodes"].items():
            comment = " // [UNUSED]" if key in unused_list else ""
            f.write(f"    {key}: {val},{comment}\n")
        f.write("};\n\n")

        f.write("export const TriggerType = {\n")
        for key, val in metadata["triggers"].items():
            f.write(f"    {key}: {val},\n")
        f.write("};\n\n")

        f.write("export const TargetType = {\n")
        for key, val in metadata["targets"].items():
            f.write(f"    {key}: {val},\n")
        f.write("};\n\n")

        f.write("export const ActionBases = {\n")
        for key, val in metadata["action_bases"].items():
            f.write(f"    {key}: {val},\n")
        f.write("};\n\n")

        f.write("export const Phases = {\n")
        for key, val in metadata["phases"].items():
            f.write(f"    {key}: {val},\n")
            f.write(f'    "{val}": "{key}",\n')
        f.write("};\n\n")

        f.write("export const ConditionTypes = {\n")
        for key, val in metadata["conditions"].items():
            comment = " // [UNUSED]" if key in unused_list else ""
            f.write(f"    {key}: {val},{comment}\n")
        f.write("};\n\n")

        f.write("export const CostTypes = {\n")
        for key, val in metadata["costs"].items():
            f.write(f"    {key}: {val},\n")
        f.write("};\n\n")

        f.write("export const ChoiceTypes = {\n")
        for key, val in metadata.get("choices", {}).items():
            f.write(f"    {key}: {val},\n")
        f.write("};\n\n")

        f.write("export const ExtraConstants = {\n")
        for key, val in resolved_extra_constants.items():
            f.write(f"    {key}: {val},\n")
        f.write("};\n\n")

        f.write("export const Zones = {\n")
        for key, val in metadata.get("zones", {}).items():
            f.write(f"    {key}: {val},\n")
        f.write("};\n\n")

        f.write("export const Comparisons = {\n")
        for key, val in metadata.get("comparisons", {}).items():
            f.write(f"    {key}: {val},\n")
        f.write("};\n\n")

        f.write("export const HeartColorMap = {\n")
        for key, val in metadata.get("heart_color_map", {}).items():
            f.write(f"    {key}: {val},\n")
        f.write("};\n\n")

        f.write("export const MetaRuleTypes = {\n")
        for key, val in metadata.get("meta_rule_types", {}).items():
            f.write(f"    {key}: {val},\n")
        f.write("};\n\n")

        f.write("export const CountSources = {\n")
        for key, val in metadata.get("multiplier_count_sources", {}).items():
            f.write(f"    {key}: {val},\n")
        f.write("};\n\n")

        f.write("export const CardTypes = {\n")
        for key, val in metadata.get("card_types", {}).items():
            f.write(f"    {key}: {val},\n")
        f.write("};\n\n")

        f.write("export const SlotIndices = {\n")
        for key, val in metadata.get("slot_indices", {}).items():
            f.write(f"    {key}: {val},\n")
        f.write("};\n\n")

        f.write("export const ChoiceIndices = {\n")
        for key, val in metadata.get("choice_indices", {}).items():
            f.write(f"    {key}: {val},\n")
        f.write("};\n\n")

        f.write("export const TargetPlayers = {\n")
        for key, val in metadata.get("target_players", {}).items():
            f.write(f"    {key}: {val},\n")
        f.write("};\n\n")

        f.write("export const CharacterIds = {\n")
        for key, val in metadata.get("character_ids", {}).items():
            f.write(f"    {key}: {val},\n")
        f.write("};\n\n")

        f.write("export const GroupIds = {\n")
        for key, val in metadata.get("group_ids", {}).items():
            f.write(f"    {key}: {val},\n")
        f.write("};\n\n")

        f.write("export const UnitIds = {\n")
        for key, val in metadata.get("unit_ids", {}).items():
            f.write(f"    {key}: {val},\n")
        f.write("};\n\n")

        f.write("export const GroupNames = {\n")
        for id_str, names in metadata.get("group_names", {}).items():
            f.write(f'    {id_str}: {{ jp: "{names["jp"]}", en: "{names["en"]}" }},\n')
        f.write("};\n\n")

        f.write("export const UnitNames = {\n")
        for id_str, names in metadata.get("unit_names", {}).items():
            f.write(f'    {id_str}: {{ jp: "{names["jp"]}", en: "{names["en"]}" }},\n')
        f.write("};\n")

    # --- Generate Python Metadata ---
    py_path = os.path.join(project_root, "engine", "models", "generated_metadata.py")
    with open(py_path, "w", encoding="utf-8") as f:
        f.write(f"# @generated by tools/sync_metadata.py on {datetime.datetime.now().isoformat()}\n\n")

        sections = [
            ("OPCODES", "opcodes"),
            ("TRIGGERS", "triggers"),
            ("TARGETS", "targets"),
            ("ACTION_BASES", "action_bases"),
            ("CONDITIONS", "conditions"),
            ("COSTS", "costs"),
            ("CHOICES", "choices"),
            ("PHASES", "phases"),
            ("ZONES", "zones"),
            ("EXTRA_CONSTANTS", None),
            ("COMPARISONS", "comparisons"),
            ("HEART_COLOR_MAP", "heart_color_map"),
            ("META_RULE_TYPES", "meta_rule_types"),
            ("COUNT_SOURCES", "multiplier_count_sources"),
            ("CARD_TYPES", "card_types"),
            ("SLOT_INDICES", "slot_indices"),
            ("CHOICE_INDICES", "choice_indices"),
            ("TARGET_PLAYERS", "target_players"),
            ("CHARACTER_IDS", "character_ids"),
            ("GROUP_IDS", "group_ids"),
            ("UNIT_IDS", "unit_ids"),
        ]

        for var_name, meta_key in sections:
            f.write(f"{var_name} = {{\n")
            section_data = resolved_extra_constants if var_name == "EXTRA_CONSTANTS" else metadata.get(meta_key, {})
            for key, val in section_data.items():
                comment = " # [UNUSED]" if key in unused_list else ""
                if isinstance(key, str):
                    f.write(f'    "{key}": {val},{comment}\n')
                else:
                    f.write(f"    {key}: {val},{comment}\n")
            f.write("}\n\n")

    # --- Generate Python Opcode Compatibility Layer ---
    # Keep engine/models/opcodes.py in sync with generated metadata so runtime
    # imports always see real IntEnum members instead of manually assigned attrs.
    py_opcode_path = os.path.join(project_root, "engine", "models", "opcodes.py")
    with open(py_opcode_path, "w", encoding="utf-8") as f:
        f.write(f"# @generated by tools/sync_metadata.py on {datetime.datetime.now().isoformat()}\n\n")
        f.write("from enum import IntEnum\n\n")
        f.write("from .generated_metadata import CONDITIONS, COSTS, OPCODES\n\n")
        f.write("\n")
        f.write("def _build_opcode_enum() -> IntEnum:\n")
        f.write('    """Build a real IntEnum from generated metadata."""\n')
        f.write("    members = dict(OPCODES)\n")
        f.write("    members.update({name: value for name, value in COSTS.items()})\n")
        f.write('    members.update({f"CHECK_{name}": value for name, value in CONDITIONS.items()})\n')
        f.write('    members.update({f"COST_{name}": value for name, value in COSTS.items()})\n')
        f.write('    return IntEnum("Opcode", members, module=__name__)\n\n')
        f.write("\n")
        f.write("Opcode = _build_opcode_enum()\n")

    # --- Generate Python Enums ---
    py_enum_path = os.path.join(project_root, "engine", "models", "generated_enums.py")
    with open(py_enum_path, "w", encoding="utf-8") as f:
        f.write(f"# @generated by tools/sync_metadata.py on {datetime.datetime.now().isoformat()}\n")
        f.write("from enum import IntEnum\n\n")

        # TriggerType
        f.write("class TriggerType(IntEnum):\n")
        f.write("    NONE = 0\n")
        for key, val in metadata["triggers"].items():
            if key == "NONE":
                continue
            comment = " # [UNUSED]" if key in unused_list else ""
            f.write(f"    {key} = {val}{comment}\n")
        f.write("\n\n")

        # TargetType
        f.write("class TargetType(IntEnum):\n")
        for key, val in metadata["targets"].items():
            f.write(f"    {key} = {val}\n")
        f.write("\n\n")

        # EffectType (Opcodes)
        f.write("class EffectType(IntEnum):\n")
        f.write("    NONE = 0\n")
        if "NOP" not in metadata["opcodes"]:
            f.write("    NOP = 0\n")
        for key, val in metadata["opcodes"].items():
            comment = " # [UNUSED]" if key in unused_list else ""
            f.write(f"    {key} = {val}{comment}\n")
        f.write("\n\n")

        # ConditionType
        f.write("class ConditionType(IntEnum):\n")
        f.write("    NONE = 0\n")
        for key, val in metadata["conditions"].items():
            comment = " # [UNUSED]" if key in unused_list else ""
            f.write(f"    {key} = {val}{comment}\n")
        write_python_enum_aliases(f, "ConditionType")
        f.write("\n\n")

        # AbilityCostType
        f.write("class AbilityCostType(IntEnum):\n")
        f.write("    NONE = 0\n")
        for key, val in metadata["costs"].items():
            if key == "NONE":
                continue
            f.write(f"    {key} = {val}\n")
        write_python_enum_aliases(f, "AbilityCostType")
        f.write("\n\n")

        # ChoiceType
        if "choices" in metadata:
            f.write("class ChoiceType(IntEnum):\n")
            f.write("    NONE = 0\n")
            for key, val in metadata["choices"].items():
                if key == "NONE":
                    continue
                f.write(f"    {key} = {val}\n")
            f.write("\n\n")

        # CardType
        f.write("class CardType(IntEnum):\n")
        for key, val in metadata["card_types"].items():
            f.write(f"    {key} = {val}\n")
        f.write("\n\n")

        # Zone
        f.write("class Zone(IntEnum):\n")
        f.write("    DEFAULT = 0\n")
        for key, val in metadata["zones"].items():
            if key == "DEFAULT":
                continue
            f.write(f"    {key} = {val}\n")
        f.write("\n\n")

    # --- Generate Sync Report ---
    report_path = "reports/sync_status.json"
    os.makedirs("reports", exist_ok=True)
    report_data = {
        "status": "Success",
        "last_updated": datetime.datetime.now().isoformat(),
        "_metadata": {
            "generated_by": "tools/sync_metadata.py",
            "generated_at": datetime.datetime.now().isoformat()
        }
    }
    with open(report_path, "w", encoding="utf-8") as f:
        json.dump(report_data, f, indent=2)

    # --- Generate Packed Layout Helpers ---
    layout = metadata.get("packed_layout")
    if layout:

        # 1. Python Generated Packer
        py_packer_path = "engine/models/generated_packer.py"
        with open(py_packer_path, "w", encoding="utf-8") as f:
            f.write(f"# @generated by tools/sync_metadata.py on {datetime.datetime.now().isoformat()}\n\n")
            f.write(
                "def to_signed_32(x):\n    x = int(x) & 0xFFFFFFFF\n    return x - 0x100000000 if x >= 0x80000000 else x\n\n"
            )

            for word_name in ["A", "V", "S"]:
                word_data = layout.get(word_name, {})
                for variant_name, fields in word_data.items():
                    if not isinstance(fields, dict):
                        continue

                    field_groups = analyze_layout_fields(fields, f"{word_name}.{variant_name}")

                    # Packer function
                    func_name = f"pack_{word_name.lower()}_{variant_name}"
                    args = ", ".join([f"{fname}=0" for fname in fields.keys()])
                    f.write(f"def {func_name}({args}):\n")
                    f.write("    res = 0\n")
                    for group in field_groups:
                        merged_names = [group["canonical"], *group["aliases"]]
                        merged_expr = " | ".join(merged_names)
                        f.write(f"    res |= (({merged_expr}) & {hex(group['mask'])}) << {group['start']}\n")
                    f.write("    return res\n\n")

                    # Unpacker function
                    func_name = f"unpack_{word_name.lower()}_{variant_name}"
                    f.write(f"def {func_name}(val):\n")
                    f.write("    return {\n")
                    for group in field_groups:
                        decoded_expr = f"(val >> {group['start']}) & {hex(group['mask'])}"
                        f.write(f"        '{group['canonical']}': {decoded_expr},\n")
                        for alias in group["aliases"]:
                            f.write(f"        '{alias}': {decoded_expr},\n")
                    f.write("    }\n\n")

        # 2. Rust Generated Layout
        rust_layout_path = "engine_rust_src/src/core/generated_layout.rs"
        with open(rust_layout_path, "w", encoding="utf-8") as f:
            f.write(f"//! @generated by tools/sync_metadata.py on {datetime.datetime.now().isoformat()}\n")
            f.write("#![allow(dead_code)]\n\n")

            for word_name in ["A", "V", "S"]:
                word_data = layout.get(word_name, {})
                for variant_name, fields in word_data.items():
                    if not isinstance(fields, dict):
                        continue

                    field_groups = analyze_layout_fields(fields, f"{word_name}.{variant_name}")

                    prefix = f"{word_name}_{variant_name}".upper()
                    f.write(f"// --- {prefix} ---\n")
                    for group in field_groups:
                        const_base = f"{prefix}_{group['canonical'].upper()}"
                        mask_type = "u64" if word_name == "A" else "u32"
                        f.write(f"pub const {const_base}_SHIFT: u32 = {group['start']};\n")
                        f.write(f"pub const {const_base}_MASK: {mask_type} = {hex(group['mask'])};\n")
                        for alias in group["aliases"]:
                            alias_const_base = f"{prefix}_{alias.upper()}"
                            f.write(f"pub const {alias_const_base}_SHIFT: u32 = {group['start']};\n")
                            f.write(f"pub const {alias_const_base}_MASK: {mask_type} = {hex(group['mask'])};\n")
                    f.write("\n")

    print("Successfully synchronized metadata to Rust, JS, and Python!")
    
    # Save hash for parity
    try:
        if current_hash:
            with open(hash_file, "w") as f:
                f.write(current_hash)
    except Exception as e:
        print(f"Warning: Could not save parity hash: {e}")


if __name__ == "__main__":
    sync()
