...
def run_audit():
    ...
    report.append(f"## 1. Coverage Gap: Cards with Abilities but No Manual Pseudocode")
    ...
    report.append(f"## 2. QA Data Without Test Coverage")
    ...
    report.append(f"## 3. Pseudocode Exists but Bytecode is Empty")
    ...
    report.append(f"## 4. Pseudocode Inconsistencies Within Same-Ability Groups")
    ...
    report.append(f"## 5. Variant Cards with Different Ability Text")
    ...
    for card_no, card_data in cards_with_ability.items():
        ...
        trigger_count = ab.count("png|登場") + ab.count("png|起動") + ab.count("png|常時") + ab.count("png|ライブ") + ab.count("png|自動")
    ...
    report.append(f"## 6. Complex (Multi-Trigger) Cards Without Test Coverage")
    ...
    coverage = ((len(cards_with_ability) - len(cards_no_pseudocode)) / len(cards_with_ability) * 100) if cards_with_ability else 0
    ...
    report.append(f"- **{len(empty_bytecode)} cards** have pseudocode that compiles to empty bytecode — these abilities are silently broken")
    report.append(f"- **{len(inconsistent_groups)} ability groups** have multiple conflicting pseudocodes — the consolidated mapping picks one but may pick wrong")
    report.append("")
    report.append("### 🟠 High Risks")
    report.append(f"- **{len(cards_no_pseudocode)} cards** have abilities but no pseudocode at all — these parse raw JP text, likely producing garbage bytecode")
    report.append(f"- **{len(qa_no_test)} QA-referenced cards** have no test coverage — official FAQs document edge cases that aren't verified")
    report.append("")
    report.append("### 🟡 Medium Risks")
    report.append(f"- **{len(complex_untested)} complex multi-trigger cards** have no tests — these are most likely to have subtle bugs")
    report.append(f"- The parser (`parser_v2.py`) at 2066 lines has many alias mappings that can silently map wrong opcode")
    report.append("")
    report.append("### Architectural Weakpoints")
    report.append("1. **Pseudocode is freeform text** → no schema validation, easy to write something the parser silently ignores")
    report.append("2. **Parser alias explosion** → 50+ aliases mean the same intent can be expressed many ways, causing silent divergence")
    report.append("3. **No round-trip verification** → bytecode is never decompiled back to pseudocode to verify semantic equivalence")
    report.append("4. **Consolidated mapping uses longest-wins** → a longer but wrong pseudocode beats a shorter correct one")
    report.append("5. **Variant cards share ability text but may need distinct pseudocode** due to group filters (e.g., Nijigasaki vs μ's)")
