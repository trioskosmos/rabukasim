        ...
        for i, ab in enumerate(compiled.get("abilities", [])):
            ...
            if "残りを控え室" in ability_text or "remainder to waiting room" in ability_text.lower():
                 ...
                 if not any(op in allowed for op in opcodes):
                      issues.append(f"{no}: Text implies 'Remainder to Discard' but uses Opcodes {opcodes} (No known discard handler found).")
            ...
            if has_order_deck:
                ...
                if rem_dest != 7:
                     issues.append(f"{no}: Uses O_LOOK_AND_CHOOSE (Op 41) with s={look_choose_s} (Target={target}, Rem={rem_dest}), expected Rem={7} (Discard).")
            else:
                # Maybe complex logic?
                pass
