...
INITIAL_URL = (
    f"{BASE_URL}/question/searchresults/?keyword=&keyword_type%5B%5D=all&search_type=and&title=&card_kind=&work_title="
)
    ...
    if relation_div:
        ...
        if relation_text_el:
            ...
            for card_no, name in card_matches:
                related_cards.append({"card_no": card_no.strip(), "name": name.strip()})

    return {"id": qa_id, "date": date, "question": question, "answer": answer, "related_cards": related_cards}
