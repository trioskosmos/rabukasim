...
def analyze():
    ...
    if kw_cond_match:
        parser_condition_aliases = extract_aliases('CONDITION_ALIASES').union(set(re.findall(r'"(\w+)"\s*:', kw_cond_match.group(1))))
    else:
        ...
        for line in lines:
            ...
            if 'EFFECT:' in line:
                ...
                if match:
                    for e in match.group(1).split('|'):
                        effects[e] += count
                        for c in cards: keyword_usage_cards['EFFECT'][e].add(c)
            ...
            if 'CONDITION:' in line:
                ...
                if match:
                    for c_kw in match.group(1).split('|'):
                        ...
                        for c in cards: keyword_usage_cards['CONDITION'][c_kw].add(c)
            ...
            if 'TRIGGER:' in line:
                ...
                if match:
                    for t in match.group(1).split('|'):
                        triggers[t] += count
                        for c in cards: keyword_usage_cards['TRIGGER'][t].add(c)
            ...
            if 'COST:' in line:
                ...
                if match:
                    for cost_kw in match.group(1).split('|'):
                        costs[cost_kw] += count
                        for c in cards: keyword_usage_cards['COST'][cost_kw].add(c)
            ...
            for tm in targets_match:
                for target_kw in tm.split('|'):
                    ...
                    for c in cards: keyword_usage_cards['TARGET'][target_kw].add(c)
    ...
    ignored_keywords = {
        'COUNT_VAL', 'PLAYER', 'SELF', 'OPPONENT', 'VARIABLE', 'PER_CARD', 
        'AND', 'OR', 'NOT', 'TRUE', 'FALSE', 'IF', 'THEN', 'ELSE',
        'VALUE_GT', 'VALUE_GE', 'VALUE_LT', 'VALUE_LE', 'VALUE_EQ', 'VALUE_NE'
    }
    ...
    print_gaps('CONDITION', conditions, meta_conditions, parser_condition_aliases, ignored_keywords.union(parser_ignored_conds))
