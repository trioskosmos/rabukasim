...
from engine.models.ability import Ability, EffectType, ConditionType, AbilityCostType, TargetType
...
class CompilerTruthOracle:
    ...
    def map_effect_to_deltas(self, effect, tag_prefix=""):
        ...
        elif etype in [EffectType.PREVENT_ACTIVATE, EffectType.PREVENT_BATON_TOUCH, EffectType.RESTRICTION, EffectType.PREVENT_PLAY_TO_SLOT]:
            deltas.append({"tag": tag_prefix + "ACTION_PREVENTION", "value": 1})
        elif etype in [EffectType.LOOK_AND_CHOOSE, EffectType.SEARCH_DECK, EffectType.ORDER_DECK, EffectType.REVEAL_CARDS, EffectType.LOOK_DECK]:
        ...
        elif etype in [EffectType.BUFF_POWER, EffectType.TRANSFORM_COLOR, EffectType.TRANSFORM_HEART, EffectType.GRANT_ABILITY, EffectType.SELECT_MEMBER]:
    ...
    def interpret_card(self, card_id):
        ...
        for ability in abilities:
            ...
            for item in items:
                deltas = []
                from engine.models.ability import Cost, Effect, Condition
                ...
                if isinstance(item, Cost):
                    ...
                    if not deltas:
                        unmapped_types.append(item.effect_type.name if hasattr(item.effect_type, "name") else item.effect_type)
...
def run_extraction():
    ...
    for cid, (interp, status, source) in id_to_res.items():
        if interp and interp["abilities"]:
            ...
            if status not in skipped: skipped[status] = []
    ...
    for cid in missing_pc:
        ...
        if status not in missing_by_status: missing_by_status[status] = []
