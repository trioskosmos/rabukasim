...
class PseudocodeOracle:
    ...
    def extract_conditions(self, block: str) -> list:
        ...
        if cond_match:
            ...
            conditions.append({"type": "PRECONDITION", "expression": cond_text})
    ...
    def extract_costs(self, block: str) -> list:
        ...
        for effect_name, delta_tag in self.COST_TO_DELTA.items():
            ...
            if match:
                ...
                costs.append({"type": effect_name, "value": value, "delta_tag": delta_tag})

        energy_icons = cost_text.count("icon_energy")
        if energy_icons > 0:
            costs.append({"type": "PAY_ENERGY", "value": energy_icons, "delta_tag": "ENERGY_COST"})

        if "TAP_MEMBER" in cost_text and not re.search(r"TAP_MEMBER\([^)]+\)", cost_text):
            costs.append({"type": "TAP_MEMBER", "value": 1, "delta_tag": "MEMBER_TAP_DELTA"})

        if "MOVE_TO_DISCARD" in cost_text:
            costs.append({"type": "MOVE_TO_DISCARD", "value": 1, "delta_tag": "DISCARD_DELTA"})
        ...
        if effect_section:
            ...
            for part in effect_parts:
                ...
                if match:
                    ...
                    effect_entry = {"type": effect_name, "value": value, "delta_tag": delta_tag}
        ...
        for match in option_matches:
            ...
            for part in effect_parts:
                ...
                if eff_match:
                    ...
                    option_effects.append({"type": effect_name, "value": value, "delta_tag": delta_tag})

            if option_effects:
                options.append({"name": option_name, "effects": option_effects})
    ...
    def interpret_card(self, card_no: str) -> dict:
        ...
        return {"id": card_no, "abilities": abilities}
