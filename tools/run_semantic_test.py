...
class AbilityResult:
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "segment_pass_rate": sum(1 for s in self.segments if s.passed) / len(self.segments)
            if self.segments
            else 1.0,
        }
...
class CardResult:
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "ability_pass_rate": sum(1 for a in self.abilities if a.passed) / len(self.abilities)
            if self.abilities
            else 1.0,
        }
...
class TestGameSetup:
    ...
    def setup_game_for_trigger(self, game: Any, trigger: TriggerType, card_id: int, player_id: int = 0) -> None:
...
def generate_report(
    ...
    abilities_passed = sum(1 for r in results for a in r.abilities if a.passed)

    total_segments = sum(len(a.segments) for r in results for a in r.abilities)
    segments_passed = sum(1 for r in results for a in r.abilities for s in a.segments if s.passed)

    segments_with_deltas = sum(1 for r in results for a in r.abilities for s in a.segments if s.expected_deltas)
...
def print_report_summary(report: TestReport) -> None:
    ...
    print(
        f"  Pass rate:        {report.abilities_passed / report.total_abilities * 100:.1f}%"
        if report.total_abilities > 0
        else "  Pass rate: N/A"
    )
    print()
    ...
    print(
        f"  Pass rate:            {report.segments_passed / report.total_segments * 100:.1f}%"
