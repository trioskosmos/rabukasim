...
for id_str, card in d["member_db"].items():
    ...
    for ab_idx, ab in enumerate(card.get("abilities", [])):
        t = ab.get("trigger", 0)
        if t == 1:  # OnPlay
            on_play_cards.append((int(id_str), card.get("name","?"), card.get("cost",0), ab_idx, str(ab.get("pseudocode",""))[:60]))
        if t == 7:  # Activated
            activated_cards.append((int(id_str), card.get("name","?"), card.get("cost",0), ab_idx, str(ab.get("pseudocode",""))[:60], ab.get("costs",[])))
