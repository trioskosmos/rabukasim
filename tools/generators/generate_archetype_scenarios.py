...
def generate_scenarios():
    ...
    def get_setup(conditions, is_pass, target_card_id, trigger_type):
        ...
        elif trigger_type in [T_ON_LIVE_START, T_ON_LIVE_SUCCESS, T_TURN_START, T_TURN_END, T_ACTIVATED, T_ON_LEAVES, T_ON_POSITION_CHANGE, T_CONSTANT]:
        ...
        for cond in conditions:
            ...
            if isinstance(val, str): val = 1 # Handle string values if any
    ...
    for card_id, card in members.items():
        ...
        for ab_idx, ability in enumerate(card.get("abilities", [])):
            ...
            if unique_key in seen_scenarios: continue
            ...
            # Action Mapping
            action = {"type": "FORCE_TRIGGER", "trigger_type": trigger_type, "slot_idx": target_slot_idx, "ab_idx": ab_idx}
            ...
            def get_setup_v2(conds, pass_mode, t_card_id, t_type, t_slot):
                ...
                elif t_type in [T_ON_LIVE_START, T_ON_LIVE_SUCCESS, T_TURN_START, T_TURN_END, T_ACTIVATED, T_ON_LEAVES, T_ON_POSITION_CHANGE, T_CONSTANT]:
                ...
                for c in conds:
                    ...
                    if ct == C_COUNT_HAND:
                        ...
                        if len(current_hand) < needed:
                            ...
                            # Add dummies
                            for i in range(diff): s_setup["hand"].append(member_ids[(i+10)%len(member_ids)])
            ...
            if is_center_req: fail_slot = 0 if target_slot_idx == 1 else 1 # Move away from required slot
            ...
            scenarios.append({
                ...
                "action": {**action, "slot_idx": pass_slot} if action["type"] != "PLAY_MEMBER" else {**action, "slot_idx": pass_slot}, 
                "choices": [0]*10,
                "expect": {"is_pass": True}
            })
            ...
            if conditions:
                ...
                if fail_action["type"] != "PLAY_MEMBER": fail_action["slot_idx"] = fail_slot
                else: fail_action["slot_idx"] = fail_slot
