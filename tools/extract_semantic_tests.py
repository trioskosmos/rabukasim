...
class SemanticDelta:
    ...
    def _generate_assertion(self, tag: str, val: Any, player_idx: int = 0, prefix: str = "") -> str:
        ...
        if tag == "HAND_DISCARD":
            ...
            return f"// ACTION_PREVENTION flag changed"
...
def load_semantic_truth(path: Path) -> Dict[str, SemanticCardTruth]:
    ...
    return {card_id: SemanticCardTruth.from_json(card_data) 
...
def generate_test_function(card: SemanticCardTruth, ab_idx: int, ability: SemanticAbility) -> str:
    ...
    is_simple = len(ability.sequence) <= 1 and not any(
        d.tag in ('HAND_DISCARD', 'MEMBER_TAP_DELTA') and d.value > 0 
        for d in all_deltas
    )
...
def generate_simple_test(test_name: str, card_id: str, ab_idx: int, 
                         rust_trigger: str, trigger_code: str, 
...
def generate_complex_test(test_name: str, card_id: str, ab_idx: int,
                          rust_trigger: str, trigger_code: str,
...
def generate_gpu_parity_test(card: SemanticCardTruth, ab_idx: int, 
                             ability: SemanticAbility) -> str:
...
def generate_gpu_assertion_for_tag(tag: str, val: Any, player_idx: int) -> str:
    ...
    if tag == "SCORE_DELTA" or tag == "LIVE_SCORE_DELTA":
        ...
        return f'// Energy tap: GPU uses tapped_energy_mask'
        ...
        return f'// Member tap: check tapped_mask in GPU'
    elif tag == "HEART_DELTA":
        return f'// Heart delta: check heart_req_reduction in GPU'
    elif tag == "STAGE_DELTA":
        return f'// Stage delta: check stage slots in GPU'
    else:
...
def main():
    ...
    parser.add_argument('--input', '-i', 
                        default='../reports/semantic_truth_v3.json',
                        help='Input semantic truth JSON file')
    parser.add_argument('--output', '-o',
                        default='../engine_rust_src/tests/generated_semantic_parity_tests.rs',
                        help='Output Rust file')
    parser.add_argument('--format', '-f',
                        choices=['unit', 'gpu_parity', 'both'],
                        default='gpu_parity',
                        help='Output format')
    parser.add_argument('--limit', '-l', type=int, default=0,
                        help='Limit number of tests to generate (0 = all)')
