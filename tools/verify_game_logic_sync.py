...
def find_wgsl_opcode_handlers(wgsl_dir: str) -> Dict[str, str]:
    ...
    opcode_values = {
        0: "NOP", 1: "RETURN", 2: "JUMP", 3: "JUMP_IF_FALSE",
        10: "DRAW", 11: "ADD_BLADES", 12: "ADD_HEARTS", 13: "REDUCE_COST",
        14: "LOOK_DECK", 15: "RECOVER_LIVE", 16: "BOOST_SCORE", 17: "RECOVER_MEMBER",
        18: "BUFF_POWER", 19: "IMMUNITY", 20: "MOVE_MEMBER", 21: "SWAP_CARDS",
        22: "SEARCH_DECK", 23: "ENERGY_CHARGE", 24: "SET_BLADES", 25: "SET_HEARTS",
        26: "FORMATION_CHANGE", 27: "NEGATE_EFFECT", 28: "ORDER_DECK", 29: "META_RULE",
        30: "SELECT_MODE", 31: "MOVE_TO_DECK", 32: "TAP_OPPONENT", 33: "PLACE_UNDER",
        35: "RESTRICTION", 36: "BATON_TOUCH_MOD", 37: "SET_SCORE", 38: "SWAP_ZONE",
        39: "TRANSFORM_COLOR", 40: "REVEAL_CARDS", 41: "LOOK_AND_CHOOSE", 42: "CHEER_REVEAL",
        43: "ACTIVATE_MEMBER", 44: "ADD_TO_HAND", 45: "COLOR_SELECT", 46: "REPLACE_EFFECT",
        47: "TRIGGER_REMOTE", 48: "REDUCE_HEART_REQ", 49: "MODIFY_SCORE_RULE",
        50: "ADD_STAGE_ENERGY", 51: "SET_TAPPED", 52: "ADD_CONTINUOUS", 53: "TAP_MEMBER",
        57: "PLAY_MEMBER_FROM_HAND", 58: "MOVE_TO_DISCARD", 60: "GRANT_ABILITY",
        61: "INCREASE_HEART_COST", 62: "REDUCE_YELL_COUNT", 63: "PLAY_MEMBER_FROM_DISCARD",
        64: "PAY_ENERGY", 65: "SELECT_MEMBER", 66: "DRAW_UNTIL", 67: "SELECT_PLAYER",
        68: "SELECT_LIVE", 69: "REVEAL_UNTIL", 70: "INCREASE_COST", 71: "PREVENT_PLAY_TO_SLOT",
        72: "SWAP_AREA", 73: "TRANSFORM_HEART", 74: "SELECT_CARDS", 75: "OPPONENT_CHOOSE",
        76: "PLAY_LIVE_FROM_DISCARD", 77: "REDUCE_LIVE_SET_LIMIT", 80: "PREVENT_SET_TO_SUCCESS_PILE",
        81: "ACTIVATE_ENERGY", 82: "PREVENT_ACTIVATE", 83: "SET_HEART_COST",
        90: "PREVENT_BATON_TOUCH", 91: "LOOK_DECK_DYNAMIC", 92: "REDUCE_SCORE",
        93: "REPEAT_ABILITY", 94: "LOSE_EXCESS_HEARTS", 95: "SKIP_ACTIVATE_PHASE",
        96: "PAY_ENERGY_DYNAMIC", 97: "PLACE_ENERGY_UNDER_MEMBER",
    }
...
def generate_sync_report(metadata_path: str, rust_dir: str, wgsl_dir: str) -> SyncReport:
    ...
    if report.opcode_coverage < 1.0:
        ...
        if missing:
            report.recommendations.append(f"Implement missing WGSL opcodes: {', '.join(missing[:5])}{'...' if len(missing) > 5 else ''}")
    ...
    if report.condition_coverage < 1.0:
        ...
        if missing:
            report.recommendations.append(f"Implement missing WGSL conditions: {', '.join(missing[:5])}{'...' if len(missing) > 5 else ''}")
...
def print_report(report: SyncReport):
    ...
    print("\n### COVERAGE SUMMARY ###")
    print(f"  Opcodes:    {report.opcode_coverage*100:.1f}% ({sum(1 for o in report.opcodes.values() if o.wgsl_implemented)}/{len(report.opcodes)})")
    print(f"  Conditions: {report.condition_coverage*100:.1f}% ({sum(1 for c in report.conditions.values() if c.wgsl_implemented)}/{len(report.conditions)})")
    print(f"  Phases:     {report.phase_coverage*100:.1f}% ({sum(1 for p in report.phases.values() if p.wgsl_handler)}/{len(report.phases)})")
...
def to_dict(report: SyncReport) -> dict:
    ...
    return {
        ...
        "phases": {
            name: {
                "value": info.value,
                "rust_handler": info.rust_handler,
                "wgsl_handler": info.wgsl_handler
            }
...
def main():
    ...
    parser.add_argument("--rust-dir", default="engine_rust_src/src",
                        help="Path to Rust source directory")
    parser.add_argument("--wgsl-dir", default="engine_rust_src/src/core",
                        help="Path to WGSL shader directory")
    parser.add_argument("--report", default="reports/game_logic_sync_report.json",
