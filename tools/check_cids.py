...
for cid, card in member_db.items():
    for ab in card.get("abilities", []):
        ...
        if trigger == 1 and 58 in bc and 81 in bc:
            print(
                f"  CID={cid} card_id={card.get('card_id', 0)} card_no={card.get('card_no', '?')} trigger={trigger} bc={bc[:24]}"
            )
...
for cid, card in member_db.items():
    for ab in card.get("abilities", []):
        ...
        if trigger == 2 and 64 in bc and 45 in bc and 12 in bc:
            print(
                f"  CID={cid} card_id={card.get('card_id', 0)} card_no={card.get('card_no', '?')} trigger={trigger} bc={bc}"
            )
