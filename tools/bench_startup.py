...
def run_bench(name, cmd_list):
    ...
    print("-" * 30)
    return {"name": name, "duration": duration, "max_rss_mb": max_rss / 1024 / 1024, "max_cpu": max_cpu}
...
results.append(
    run_bench("Translation Analysis", ["uv", "run", "python", "tools/analysis/analyze_translation_coverage.py"])
)
