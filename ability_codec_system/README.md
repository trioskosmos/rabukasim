...
These are the main files behind the system:
...
- Preserves round-trip safety for the current bytecode layout by carrying `source_words` as compatibility payload.
...
- the sparse fields are for readability and future writability
- `source_words` keeps the current runtime exact
- the engine should not trust the sparse semantic rewrite alone until the parity checks say it is safe
...
- The sparse semantic rebuild path needs parity coverage before `source_words` can be considered removable.
...
5. Keep the compatibility payload until the parity suite passes without fallback.
## Roadmap
